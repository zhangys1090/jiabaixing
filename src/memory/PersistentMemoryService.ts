...
export class PersistentMemoryService {
  private static instance: PersistentMemoryService | null = null;
  private memoryEntries: string[] = [];
  private userEntries: string[] = [];
  private readonly MEMORY_CHAR_LIMIT = 2200;
  private readonly USER_CHAR_LIMIT = 1375;
  private readonly ENTRY_SEPARATOR = '§';
  private readonly CAPACITY_WARNING_THRESHOLD = 0.8;
  private readonly storageDir: string;
  private initialized: boolean = false;
  private readonly fileSystem: FileSystem;

  /** 提示注入检测正则 */
  private readonly PROMPT_INJECTION_PATTERN =
    /(?:ignore|forget|override|disregard|skip)\s+(?:previous|all|above|prior|earlier)\s+(?:instructions?|rules?|prompts?|directives?|constraints?)/i;

  /** 凭证泄露检测正则 */
  private readonly CREDENTIAL_PATTERN =
    /(?:api[_\s-]?key|password|passwd|secret[_\s-]?key|access[_\s-]?token|private[_\s-]?key|ssh[_\s-]?key|auth[_\s-]?token|bearer)\s*[:=]\s*[\w\-+/=.]{8,}|-----BEGIN\s+(?:RSA\s+)?PRIVATE\s+KEY-----|sk-[a-zA-Z0-9]{12,}/i;

  /** 不可见Unicode字符范围: 零宽字符、控制字符等 */
  private readonly INVISIBLE_UNICODE_PATTERN =
    /[\u200B-\u200F\u2028-\u202F\uFEFF]/;
   ...
   * @returns 单例实例
   */
  public static getInstance(storageDir?: string): PersistentMemoryService {
  ...
  public async initialize(): Promise<void> {
    if (this.initialized) {
      Logger.warn(
        'PersistentMemoryService 已初始化，跳过重复初始化',
        'PersistentMemory'
      );
      return;
    }
    ...
    try {
      ...
      if (await this.fileSystem.exists(memoryPath)) {
        try {
          ...
          Logger.info(
            `MEMORY.md 加载完成，共 ${this.memoryEntries.length} 条记录`,
            'PersistentMemory'
          );
        } catch (error) {
          this.memoryEntries = [];
          Logger.warn(
            `MEMORY.md 读取失败，初始化为空: ${(error as Error).message}`,
            'PersistentMemory'
          );
        }
      } else {
        this.memoryEntries = [];
        Logger.info('MEMORY.md 不存在，初始化为空', 'PersistentMemory');
      }
      ...
      if (await this.fileSystem.exists(userPath)) {
        try {
          ...
          Logger.info(
            `USER.md 加载完成，共 ${this.userEntries.length} 条记录`,
            'PersistentMemory'
          );
        } catch (error) {
          this.userEntries = [];
          Logger.warn(
            `USER.md 读取失败，初始化为空: ${(error as Error).message}`,
            'PersistentMemory'
          );
        }
      } else {
        this.userEntries = [];
        Logger.info('USER.md 不存在，初始化为空', 'PersistentMemory');
      }
      ...
      Logger.info('PersistentMemoryService 初始化完成', 'PersistentMemory');
    } catch (error) {
      Logger.error(
        'PersistentMemoryService 初始化失败',
        error as Error,
        'PersistentMemory'
      );
      throw new Error(`持久化记忆服务初始化失败: ${(error as Error).message}`);
    }
  }
  ...
  public async add(
    ...
    if (entries.some((entry) => entry === content)) {
      Logger.warn('添加记忆失败: 条目已存在（精确重复）', 'PersistentMemory');
    ...
    if (newUsage > limit) {
      Logger.warn(
        `添加记忆失败: 超出容量限制 (${this.formatNumber(newUsage)}/${this.formatNumber(limit)})`,
        'PersistentMemory'
      );
      return this.buildResult(
        target,
        false,
        `超出容量限制 (${this.formatNumber(newUsage)}/${this.formatNumber(limit)})`
      );
    }
    ...
    if (threat.detected) {
      Logger.warn(
        `添加记忆失败: 安全威胁 - ${threat.description}`,
        'PersistentMemory'
      );
    ...
    if (updatedUsage / limit > this.CAPACITY_WARNING_THRESHOLD) {
      Logger.warn(
        `${target} 容量已超过80% (${this.getUsage(target)})，建议整合条目`,
        'PersistentMemory'
      );
    }

    Logger.info(
      `添加${target === 'memory' ? 'Agent笔记' : '用户画像'}条目成功`,
      'PersistentMemory'
    );
  ...
  public async replace(
    ...
    if (matchIndices.length > 1) {
      return this.buildResult(
        target,
        false,
        `匹配到 ${matchIndices.length} 个条目，请提供更精确的匹配文本`
      );
    }
    ...
    if (threat.detected) {
      Logger.warn(
        `替换记忆失败: 安全威胁 - ${threat.description}`,
        'PersistentMemory'
      );
      return this.buildResult(target, false, `安全威胁: ${threat.description}`);
    }
    ...
    if (newUsage > limit) {
      return this.buildResult(
        target,
        false,
        `替换后超出容量限制 (${this.formatNumber(newUsage)}/${this.formatNumber(limit)})`
      );
    }
    ...
    Logger.info(
      `替换${target === 'memory' ? 'Agent笔记' : '用户画像'}条目成功`,
      'PersistentMemory'
    );
    return this.buildResult(target, true);
  }
  ...
  public async remove(
    ...
    Logger.info(
      `删除${target === 'memory' ? 'Agent笔记' : '用户画像'}条目成功`,
      'PersistentMemory'
    );
    return this.buildResult(target, true);
  }
   ...
   * @returns 格式化的记忆快照字符串
   */
  public getSnapshot(target: MemoryTarget): string {
    this.ensureInitialized();
   ...
   */
  public getUsage(target: MemoryTarget): string {
   ...
   * @returns 条目数组的浅拷贝
   */
  public getEntries(target: MemoryTarget): string[] {
  ...
  public async save(): Promise<void> {
    ...
    try {
      ...
      await this.fileSystem.writeFile(
        this.getFilePath('memory'),
        memoryContent
      );
      await this.fileSystem.writeFile(this.getFilePath('user'), userContent);

      Logger.debug('记忆文件保存完成', 'PersistentMemory');
    } catch (error) {
      Logger.error('记忆文件保存失败', error as Error, 'PersistentMemory');
      throw new Error(`记忆文件保存失败: ${(error as Error).message}`);
    }
  }
  ...
  public async shutdown(): Promise<void> {
    ...
    try {
      ...
      Logger.info('PersistentMemoryService 已关闭', 'PersistentMemory');
    } catch (error) {
      Logger.error(
        'PersistentMemoryService 关闭失败',
        error as Error,
        'PersistentMemory'
      );
    }
  ...
  private ensureInitialized(): void {
    if (!this.initialized) {
      throw new Error(
        'PersistentMemoryService 未初始化，请先调用 initialize()'
      );
    }
  }
   ...
   * @returns 文件绝对路径
   */
  private getFilePath(target: MemoryTarget): string {
   ...
   * @returns 条目数组的直接引用
   */
  private getEntriesRef(target: MemoryTarget): string[] {
   ...
   * @returns 字符限制值
   */
  private getCharLimit(target: MemoryTarget): number {
   ...
   * @returns 当前字符数
   */
  private calculateUsage(target: MemoryTarget): number {
    const entries = this.getEntriesRef(target);
   ...
   * @returns 使用百分比整数
   */
  private getUsagePercentage(target: MemoryTarget): number {
    const current = this.calculateUsage(target);
    const limit = this.getCharLimit(target);
   ...
   */
  private async migrateFromLegacySystems(): Promise<void> {
    // 仅当 USER.md 为空时才迁移
    ...
    try {
      ...
      if (habits?.preferredLanguages?.length) {
        migrationEntries.push(
          `常用语言: ${habits.preferredLanguages.join(', ')}`
        );
      }
      if (habits?.preferredFrameworks?.length) {
        migrationEntries.push(
          `常用框架: ${habits.preferredFrameworks.join(', ')}`
        );
      }
      ...
      if (prefs?.dietaryPreferences?.length) {
        migrationEntries.push(
          `饮食偏好: ${prefs.dietaryPreferences.join(', ')}`
        );
      }
      ...
      if (prefs?.entertainmentPreferences?.length) {
        migrationEntries.push(
          `娱乐偏好: ${prefs.entertainmentPreferences.join(', ')}`
        );
      }
      ...
      Logger.warn(
        `UserProfile 迁移跳过: ${(error as Error).message}`,
        'PersistentMemory'
      );
    }
    ...
    try {
      ...
      if (summary.frameworkPreferences.length > 0) {
        migrationEntries.push(
          `框架偏好: ${summary.frameworkPreferences.join('；')}`
        );
      }
      if (summary.workflowPreferences.length > 0) {
        migrationEntries.push(
          `工作流程: ${summary.workflowPreferences.join('；')}`
        );
      }
      if (summary.recentCorrections.length > 0) {
        migrationEntries.push(
          `最近纠错: ${summary.recentCorrections.join('；')}`
        );
      }
    } catch (error) {
      Logger.warn(
        `PreferenceManager 迁移跳过: ${(error as Error).message}`,
        'PersistentMemory'
      );
    }
    ...
    if (migrationEntries.length > 0) {
      ...
      for (const entry of migrationEntries) {
        if (totalChars + entry.length <= this.USER_CHAR_LIMIT) {
          ...
          Logger.warn(
            `USER.md 迁移截断: 超出容量限制，已导入 ${this.userEntries.length}/${migrationEntries.length} 条`,
            'PersistentMemory'
          );
          break;
        }
      ...
      await this.save();
      Logger.info(
        `从旧系统迁移 ${this.userEntries.length} 条用户画像到 USER.md`,
        'PersistentMemory'
      );
    }
  }
   ...
   * @returns 安全威胁检测结果
   */
  private scanSecurity(content: string): SecurityThreat {
    // 1. 提示注入检测
