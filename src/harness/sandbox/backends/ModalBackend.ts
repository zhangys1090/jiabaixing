...
export class ModalBackend implements ITerminalBackend {
  readonly type = 'modal' as const;
  private config: ModalBackendConfig;
  private initialized = false;
  private appDeployed = false;
  ...
  async initialize(): Promise<void> {
    ...
    if (!available) {
      throw new Error(
        'Modal CLI 不可用，请确认 modal 已安装且已认证 (modal token set)'
      );
    }

    Logger.info(
      `⚡ Modal 后端已就绪${this.config.appName ? ` (App: ${this.config.appName})` : ''}`,
      'ModalBackend'
    );
  ...
  async cleanup(): Promise<void> {
    ...
    Logger.info('⚡ Modal 后端已清理', 'ModalBackend');
  }

  private buildRunScript(command: string, options?: ExecuteOptions): string {
...
MODAL_EOF`;
  ...
  private parseModalOutput(raw: string): string {
    ...
    for (const line of lines) {
      ...
      if (line.includes('─────') || line.includes('│')) {
        ...
        if (content) outputLines.push(content);
        continue;
      }
