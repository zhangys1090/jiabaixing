...
export const SKILL_SHARE_DEF: ToolDefinition = {
  name: 'skill_share',
  description:
    'Skill 分享工具：导出/导入/运行可分享的技能包。USE WHEN: 用户要分享技能、导入别人的技能、运行一个技能包。DO NOT USE WHEN: 用户要创建新技能（用skill_create）。Skill是传播单元：写完分享，别人一个命令就能跑。',
...
export function createSkillShareExecutor() {
  return async (
    params: Record<string, unknown>,
    ...
    try {
      ...
      return {
        success: false,
        output: null,
        error: `Skill 操作失败: ${(err as Error).message}`,
        duration: Date.now() - startTime,
        validated: false,
      };
    }
...
async function importSkill(
  ...
  Logger.info(`📦 技能导入: ${skill.name}`, 'SkillShare');
...
function listSkills(startTime: number): ToolResult {
  ensureSkillsDir();
