...
export const GET_ACTIVE_FILE_DEF: ToolDefinition = {
  name: 'get_active_file',
  description:
    '获取用户当前正在编辑的文件内容（需要IDE集成支持）。适用场景：用户说"改一下这个文件"、"优化这段代码"但没有提供文件路径。不适用：用户已明确提供文件路径。',
...
/** get_active_file 依赖接口 */
export interface GetActiveFileDeps {
  getActiveFilePath?: () => Promise<string | null>;
  readFileContent?: (path: string) => Promise<string>;
}

/** 创建 get_active_file 执行器 */
export function createGetActiveFileExecutor(deps: GetActiveFileDeps = {}) {
  return async (
    ...
    if (!deps.getActiveFilePath) {
      return {
        success: false,
        output:
          'IDE集成未启用。请手动提供文件路径，或安装VSCode扩展以启用此功能。',
        duration: 0,
        validated: false,
    ...
    try {
      ...
      return {
        success: false,
        output: `获取活动文件失败: ${(error as Error).message}`,
