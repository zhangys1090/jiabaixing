...
export const CONVERSATION_COMPRESSION_DEF: ToolDefinition = {
  name: 'conversation_compression',
  description:
    '压缩对话历史，减少上下文窗口占用。支持策略：summarize=LLM摘要, extract=关键信息提取, sliding_window=滑动窗口保留最近N条, hybrid=混合策略。适用场景：长对话上下文溢出、减少Token消耗、保留关键信息。',
  category: ToolCategory.SYSTEM,
  parameters: {
    action: {
      type: 'string',
      description:
        '操作类型：compress=执行压缩, estimate=估算Token, preview=预览压缩效果',
  ...
  requiredPermissions: [Permission.MEMORY_READ, Permission.MEMORY_WRITE],
  riskLevel: 'low',
  idempotent: true,
  timeout: 30000,
};

export interface ConversationCompressionDeps {
  llm?: {
    chat(
      prompt: string,
      history?: unknown[],
...
function extractKeyPoints(messages: ChatMessage[], focus?: string): string[] {
  const keyPoints: string[] = [];
  const codeBlocks: string[] = [];
  const fileMentions: Set<string> = new Set();
  const errorMentions: string[] = [];
...
function slidingWindowCompress(
  ...
  summaryParts.push(
    `用户消息: ${userMessages.length}条, 助手消息: ${assistantMessages.length}条`
  );
...
export function createConversationCompressionExecutor(
  deps: ConversationCompressionDeps = {}
) {
  return async (
    ...
    if (strategy === 'sliding_window') {
      ...
      try {
        const conversationText = toCompress
          .map(
            (m) =>
              `[${m.role}]: ${typeof m.content === 'string' ? m.content.substring(0, 300) : ''}`
          )
          .join('\n');
        ...
        const originalTokens = messages.reduce(
          (sum, m) =>
            sum +
            estimateTokens(typeof m.content === 'string' ? m.content : ''),
          0
        );
        const compressedTokens =
          estimateTokens(llmSummary) +
          preserved.reduce(
            (sum, m) =>
              sum +
              estimateTokens(typeof m.content === 'string' ? m.content : ''),
        ...
        Logger.warn(
          `LLM 摘要失败，降级到 extract 策略: ${(err as Error).message}`,
    ...
    Logger.info(
      `🗜️ conversation_compression: ${result.originalCount}→${result.compressedCount} (${savings}% 节省)`,
      'ConversationCompression'
    );
