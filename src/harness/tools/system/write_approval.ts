...
export const WRITE_APPROVAL_DEF: ToolDefinition = {
  name: 'write_approval',
  description:
    '文件写入审批工具。在执行高风险写入操作（如修改配置文件、删除文件、批量编辑）前，向用户展示变更预览并请求审批。适用场景：修改关键配置、批量文件操作、不可逆的写入操作。不适用：低风险的新建文件、临时文件操作。',
  category: ToolCategory.SYSTEM,
  parameters: {
    operation: {
      type: 'string',
      description:
        '请求审批的操作类型：file_write|file_delete|batch_edit|config_change',
...
export function createWriteApprovalExecutor(deps: WriteApprovalDeps = {}) {
  return async (
    params: Record<string, unknown>,
    ...
    void EventBus.emit('write_approval_request', {
      operation,
      target,
    ...
    if (deps.approvalManager) {
      try {
        const result = await deps.approvalManager.requestApproval(
          'write_approval',
          { operation, target, changes: changes.substring(0, 500) },
          'medium'
        );

        if (result.approved) {
          Logger.info(
            `✅ write_approval 审批通过: ${operation} → ${target}`,
            'WriteApproval'
          );
          return {
            success: true,
        ...
        Logger.info(
          `❌ write_approval 审批拒绝: ${operation} → ${target} (${result.reason || '用户拒绝'})`,
          'WriteApproval'
        );
        return {
          success: false,
        ...
        Logger.error(
          `❌ write_approval 审批流程异常: ${(err as Error).message}`,
          err as Error,
          'WriteApproval'
        );
        return {
          success: false,
          output: '',
          error: `审批流程异常: ${(err as Error).message}`,
          duration: Date.now() - startTime,
          validated: false,
        };
      }
    ...
    Logger.info(
      `⏳ write_approval 等待审批: ${operation} → ${target}`,
      'WriteApproval'
    );
