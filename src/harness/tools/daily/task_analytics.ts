...
export const TASK_ANALYTICS_DEF: ToolDefinition = {
  name: 'task_analytics',
  description:
    '任务统计分析。支持统计任务完成情况、生成日报/周报、分析任务趋势、识别瓶颈任务等操作。适用场景：项目进度跟踪、团队效率分析、任务管理报表。',
...
export type TaskAnalyticsDeps = Pick<TaskManageDeps, 'taskStore'>;
...
export function createTaskAnalyticsExecutor(deps: TaskAnalyticsDeps) {
  return async (
    ...
    try {
      ...
      switch (action) {
        ...
        case 'weekly_report': {
          ...
          const weekCreated = allTasks.filter(
            (t) => t.createdAt >= weekStartMs && t.createdAt < weekEndMs
          );
          ...
          return {
            success: true,
            output:
              `📋 本周任务报告 (${formatDate(weekStart.getTime())} ~ ${formatDate(weekEndMs - 1)})\n\n` +
        ...
        case 'trend': {
          ...
          const totalCreated = Object.values(trendData).reduce(
            (sum, s) => sum + s.created,
            0
          );
          const totalCompleted = Object.values(trendData).reduce(
        ...
        case 'bottleneck': {
          ...
          const oldTasks = pendingTasks.filter(
            (t) => t.createdAt < getDaysAgo(3)
          );

          const priorityGroups: Record<string, TaskEntry[]> = {
            urgent: [],
            high: [],
            medium: [],
            low: [],
      ...
      return {
        success: false,
        output: null,
        error: `统计分析失败: ${(err as Error).message}`,
        duration: 0,
        validated: false,
      };
    }
