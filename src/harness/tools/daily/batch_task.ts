...
export const BATCH_TASK_DEF: ToolDefinition = {
  name: 'batch_task',
  description:
    '批量操作任务。支持批量创建、批量完成、批量删除、批量更新优先级、批量添加标签等操作。适用场景：一次性处理多个任务、批量管理任务列表。',
...
export type BatchTaskDeps = Pick<TaskManageDeps, 'taskStore'>;
...
export function createBatchTaskExecutor(deps: BatchTaskDeps) {
  return async (
    ...
    try {
      ...
      return {
        success: false,
        output: null,
        error: `批量操作失败: ${(err as Error).message}`,
        duration: 0,
        validated: false,
      };
    }
