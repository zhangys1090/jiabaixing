...
const CATEGORY_INTENT_MAP: Record<string, string[]> = {
  file: ['文件', '读取', '写入', '编辑', '创建', '删除', '搜索文件', 'file', 'read', 'write', 'edit', 'create', 'delete'],
  code: ['代码', '执行', '运行', '调试', '编译', '分析', 'code', 'execute', 'run', 'debug', 'compile', 'analyze'],
  memory: ['记忆', '存储', '回忆', '搜索', '记忆检索', 'memory', 'store', 'recall', 'search'],
  system: ['系统', '状态', '配置', '管理', 'shell', 'system', 'status', 'config', 'manage'],
  network: ['网络', '请求', '下载', '搜索', 'API', 'network', 'request', 'download', 'search', 'fetch'],
  cognition: ['思考', '反思', '分析', '评估', '规划', 'cognition', 'think', 'reflect', 'analyze', 'evaluate', 'plan'],
  daily: ['日常', '提醒', '日程', '天气', '时间', 'daily', 'remind', 'schedule', 'weather', 'time'],
  desktop: ['桌面', '截图', '点击', '输入', '自动化', 'desktop', 'screenshot', 'click', 'type', 'automate'],
  perception: ['语音', '图像', '识别', '感知', 'perception', 'voice', 'image', 'recognize', 'sense'],
  meta: ['元工具', '动态', '定义', '检查', '注销', 'meta', 'dynamic', 'define', 'inspect', 'undefine'],
};

const TOOL_INTENT_OVERRIDES: Record<string, Partial<EnhancedToolMetadata>> = {
  tool_define: {
    intentKeywords: ['创建工具', '定义工具', '新工具', '自定义工具', 'define tool', 'create tool', 'new tool'],
    useCases: ['需要新能力时创建工具', '现有工具不满足时自定义', '运行时扩展Agent能力'],
    outputType: 'tool_definition',
    complexity: 'complex',
    combinableWith: ['tool_inspect', 'tool_undefine', 'execute_code'],
  },
  tool_inspect: {
    intentKeywords: ['查看工具', '检查工具', '工具列表', '工具信息', 'inspect tool', 'list tools', 'tool info'],
  ...
  tool_undefine: {
    intentKeywords: ['删除工具', '注销工具', '移除工具', 'undefine tool', 'remove tool', 'delete tool'],
  ...
  shell_exec: {
    intentKeywords: ['执行命令', 'shell', '终端', '命令行', 'run command', 'terminal', 'cli'],
  ...
  execute_code: {
    intentKeywords: ['执行代码', '运行代码', '代码执行', 'run code', 'execute', 'eval'],
  ...
  file_read: {
    intentKeywords: ['读取文件', '查看文件', '打开文件', 'read file', 'open file', 'view file'],
  ...
  file_write: {
    intentKeywords: ['写入文件', '创建文件', '保存文件', 'write file', 'create file', 'save file'],
  ...
  web_search: {
    intentKeywords: ['搜索', '查找', '查询', 'search', 'find', 'query', 'google'],
  ...
  web_fetch: {
    intentKeywords: ['获取网页', '下载', '抓取', 'fetch', 'download', 'scrape', 'crawl'],
  ...
  memory_search: {
    intentKeywords: ['搜索记忆', '回忆', '查找记忆', 'recall', 'remember', 'memory search'],
  ...
  memory_store: {
    intentKeywords: ['存储记忆', '记住', '保存记忆', 'store memory', 'remember', 'save'],
...
export class ToolMetadataEnhancer {
  ...
  private usageStats: Map<string, { count: number; successCount: number; totalDuration: number }> = new Map();
  ...
  recordUsage(toolName: string, success: boolean, durationMs: number): void {
    ...
    if (metadata) {
      ...
      metadata.successRate = stats.count > 0 ? stats.successCount / stats.count : 0;
      metadata.typicalDurationMs = stats.count > 0 ? Math.round(stats.totalDuration / stats.count) : 0;
    }
  }
  ...
  searchByIntent(query: string, options: ToolSearchOptions = {}): ToolSearchResult[] {
    ...
    for (const [toolName, definition] of this.toolDefinitions) {
      ...
      if (options.capabilityLevel && definition.capabilityLevel !== options.capabilityLevel) {
        continue;
      }
      ...
      const score = this.computeRelevanceScore(queryLower, queryTokens, definition, metadata);
  ...
  recommendTools(context: string, recentToolCalls: string[] = [], limit: number = 3): ToolSearchResult[] {
  ...
  private buildEnhancedMetadata(definition: ToolDefinition): EnhancedToolMetadata {
    ...
    const intentKeywords = override?.intentKeywords ?? [
      ...
      definition.name,
      ...(definition.tags ?? []),
      ...definition.description.split(/[,，.。;；\s]+/).filter((w) => w.length > 1),
    ];

    const useCases = override?.useCases ?? [
  ...
  private inferComplexity(definition: ToolDefinition): 'simple' | 'moderate' | 'complex' {
    const paramCount = Object.keys(definition.parameters).length;
    const requiredCount = definition.requiredParams.length;

    if (paramCount <= 2 && requiredCount <= 1 && definition.riskLevel !== 'high') {
      return 'simple';
    }
    if (paramCount > 5 || requiredCount > 3 || definition.riskLevel === 'high') {
      return 'complex';
    }
    return 'moderate';
  }

  private updateSearchIndex(toolName: string, metadata: EnhancedToolMetadata): void {
  ...
  private computeRelevanceScore(
    ...
    const nameMatch = query.includes(definition.name.toLowerCase()) ||
    ...
    for (const token of queryTokens) {
      if (metadata.intentKeywords.some((k) => k.toLowerCase().includes(token))) {
        keywordMatchCount++;
      }
    }
