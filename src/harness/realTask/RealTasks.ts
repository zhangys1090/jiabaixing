...
export const G7_TypeCoercion: RealTask = {
  ...
  description: 'String concatenation instead of numeric addition — type coercion error',
  domain: 'replan',
  ...
  setup: async (env: TaskEnvironment) => {
    fs.mkdirSync(env.tempDir, { recursive: true });
    fs.writeFileSync(path.join(env.tempDir, 'calc.js'), `function sum(a,b){return a+b;}\nmodule.exports={sum};\n`);
    fs.writeFileSync(path.join(env.tempDir, 'calc_test.js'), `const {sum}=require('./calc');\nconst assert=require('assert');\nassert.strictEqual(sum("2","3"),5,'sum("2","3") should be 5');\nconsole.log('PASS');\n`);
  },
  successCriteria: {
    ...
    check: async (env: TaskEnvironment) => {
      try {
        const { execSync } = require('child_process');
        execSync('node calc_test.js', { cwd: env.tempDir, encoding: 'utf-8', timeout: 10000 });
        return { satisfied: true, evidence: 'sum("2","3") === 5 — type coercion fixed' };
      } catch (e: any) {
        return { satisfied: false, evidence: `sum("2","3") !== 5: ${e.message}` };
      }
    },
  },
};
...
export const G8_MissingAwait: RealTask = {
  ...
  description: 'Async function missing await — returns Promise instead of value',
  domain: 'replan',
  goalDescription: 'Fix async.js so getData() returns "hello" instead of a Promise object',
  executionDomain: 'desktop',
  maxSteps: 15,
  maxTimeMs: 45000,
  allowedCapabilities: ['shell', 'file_read', 'file_write'],
  setup: async (env: TaskEnvironment) => {
    fs.mkdirSync(env.tempDir, { recursive: true });
    fs.writeFileSync(path.join(env.tempDir, 'async.js'), `async function getData(){return Promise.resolve("hello");}\nfunction getResult(){return getData();}\nmodule.exports={getResult};\n`);
    fs.writeFileSync(path.join(env.tempDir, 'async_test.js'), `const {getResult}=require('./async');\nconst assert=require('assert');\nconst r=getResult();\nassert.strictEqual(r,"hello",'getResult() should be "hello" not a Promise');\nconsole.log('PASS');\n`);
  },
  successCriteria: {
    ...
    check: async (env: TaskEnvironment) => {
      try {
        const { execSync } = require('child_process');
        execSync('node async_test.js', { cwd: env.tempDir, encoding: 'utf-8', timeout: 10000 });
        return { satisfied: true, evidence: 'getResult() === "hello" — missing await fixed' };
      } catch (e: any) {
        return { satisfied: false, evidence: `getResult() !== "hello": ${e.message}` };
      }
    },
  },
};
...
export const G9_FilePermission: RealTask = {
  ...
  domain: 'replan',
  goalDescription: 'Fix output.txt so it contains "updated" despite being read-only',
  executionDomain: 'desktop',
  maxSteps: 15,
  maxTimeMs: 45000,
  allowedCapabilities: ['shell', 'file_read', 'file_write'],
  ...
  successCriteria: {
    ...
    check: async (env: TaskEnvironment) => {
      ...
      if (content === 'updated') {
        return { satisfied: true, evidence: 'output.txt contains "updated" — permission handled' };
      }
      return { satisfied: false, evidence: `output.txt contains "${content}" not "updated"` };
    },
  },
};
...
export const G10_MissingEnvVar: RealTask = {
  ...
  description: 'Code depends on undefined environment variable — novel env dependency',
  domain: 'replan',
  goalDescription: 'Fix config.js so getPort() returns 3000 when PORT env var is not set',
  executionDomain: 'desktop',
  maxSteps: 15,
  maxTimeMs: 45000,
  allowedCapabilities: ['shell', 'file_read', 'file_write'],
  setup: async (env: TaskEnvironment) => {
    fs.mkdirSync(env.tempDir, { recursive: true });
    fs.writeFileSync(path.join(env.tempDir, 'config.js'), `function getPort(){return process.env.PORT.length>0?parseInt(process.env.PORT):3000;}\nmodule.exports={getPort};\n`);
    fs.writeFileSync(path.join(env.tempDir, 'config_test.js'), `const {getPort}=require('./config');\nconst assert=require('assert');\nassert.strictEqual(getPort(),3000,'getPort() should default to 3000');\nconsole.log('PASS');\n`);
  },
  successCriteria: {
    ...
    check: async (env: TaskEnvironment) => {
      try {
        const { execSync } = require('child_process');
        execSync('node config_test.js', { cwd: env.tempDir, encoding: 'utf-8', timeout: 10000, env: { ...process.env, PORT: undefined } });
        return { satisfied: true, evidence: 'getPort() === 3000 — missing env var handled' };
      } catch (e: any) {
        return { satisfied: false, evidence: `getPort() !== 3000: ${e.message}` };
      }
    },
  },
};
...
export const G11_LoopConditionInverted: RealTask = {
  ...
  setup: async (env: TaskEnvironment) => {
    fs.mkdirSync(env.tempDir, { recursive: true });
    fs.writeFileSync(path.join(env.tempDir, 'count.js'), `function countItems(arr){let count=0;let i=0;while(i>=arr.length){count++;i++;}return count;}\nmodule.exports={countItems};\n`);
    fs.writeFileSync(path.join(env.tempDir, 'count_test.js'), `const {countItems}=require('./count');\nconst assert=require('assert');\nassert.strictEqual(countItems([1,2,3]),3,'countItems([1,2,3]) should be 3');\nconsole.log('PASS');\n`);
  },
  successCriteria: {
    ...
    check: async (env: TaskEnvironment) => {
      try {
        const { execSync } = require('child_process');
        execSync('node count_test.js', { cwd: env.tempDir, encoding: 'utf-8', timeout: 10000 });
        return { satisfied: true, evidence: 'countItems([1,2,3]) === 3 — loop condition fixed' };
      } catch (e: any) {
        return { satisfied: false, evidence: `countItems([1,2,3]) !== 3: ${e.message}` };
      }
    },
  },
};
...
export const G12_BomEncoding: RealTask = {
  ...
  description: 'UTF-8 BOM in JSON file causes JSON.parse to fail — encoding error',
  domain: 'replan',
  ...
  setup: async (env: TaskEnvironment) => {
    ...
    fs.writeFileSync(path.join(env.tempDir, 'data.json'), Buffer.concat([bom, jsonContent]));
    fs.writeFileSync(path.join(env.tempDir, 'loader.js'), `const fs=require('fs');\nfunction loadConfig(){const raw=fs.readFileSync('data.json','utf-8');return JSON.parse(raw);}\nmodule.exports={loadConfig};\n`);
    fs.writeFileSync(path.join(env.tempDir, 'loader_test.js'), `const {loadConfig}=require('./loader');\nconst assert=require('assert');\nconst cfg=loadConfig();\nassert.strictEqual(cfg.key,"value",'config.key should be "value"');\nconsole.log('PASS');\n`);
  },
  successCriteria: {
    ...
    check: async (env: TaskEnvironment) => {
      try {
        const { execSync } = require('child_process');
        execSync('node loader_test.js', { cwd: env.tempDir, encoding: 'utf-8', timeout: 10000 });
        return { satisfied: true, evidence: 'loadConfig() parses correctly — BOM encoding fixed' };
      } catch (e: any) {
...
export const N1_ConfigTypo: RealTask = {
  ...
  domain: 'replan',
  goalDescription: 'Fix settings.js so getPort() returns 8080 instead of undefined',
  executionDomain: 'desktop',
  maxSteps: 15,
  maxTimeMs: 45000,
  allowedCapabilities: ['shell', 'file_read', 'file_write'],
  setup: async (env: TaskEnvironment) => {
    fs.mkdirSync(env.tempDir, { recursive: true });
    fs.writeFileSync(path.join(env.tempDir, 'settings.js'), `const config={prot:8080,host:"localhost"};\nfunction getPort(){return config.port;}\nmodule.exports={getPort};\n`);
    fs.writeFileSync(path.join(env.tempDir, 'settings_test.js'), `const {getPort}=require('./settings');\nconst assert=require('assert');\nassert.strictEqual(getPort(),8080,'getPort() should be 8080');\nconsole.log('PASS');\n`);
  },
  successCriteria: {
    ...
    check: async (env: TaskEnvironment) => {
      try {
        const { execSync } = require('child_process');
        execSync('node settings_test.js', { cwd: env.tempDir, encoding: 'utf-8', timeout: 10000 });
        return { satisfied: true, evidence: 'getPort() === 8080 — config typo fixed' };
      } catch (e: any) {
        return { satisfied: false, evidence: `getPort() !== 8080: ${e.message}` };
      }
    },
  },
};
...
export const N2_DataTransform: RealTask = {
  ...
  description: 'Data transform produces string concatenation instead of numeric sum — novel data error',
  domain: 'replan',
  goalDescription: 'Fix transform.js so total([10,20]) returns 30 instead of "1020"',
  executionDomain: 'desktop',
  maxSteps: 15,
  maxTimeMs: 45000,
  allowedCapabilities: ['shell', 'file_read', 'file_write'],
  setup: async (env: TaskEnvironment) => {
    fs.mkdirSync(env.tempDir, { recursive: true });
    fs.writeFileSync(path.join(env.tempDir, 'transform.js'), `function total(items){return items.reduce((a,b)=>a+b,0);}\nmodule.exports={total};\n`);
    fs.writeFileSync(path.join(env.tempDir, 'transform_test.js'), `const {total}=require('./transform');\nconst assert=require('assert');\nassert.strictEqual(total(["10","20"]),30,'total(["10","20"]) should be 30');\nconsole.log('PASS');\n`);
  },
  successCriteria: {
    ...
    check: async (env: TaskEnvironment) => {
      try {
        const { execSync } = require('child_process');
        execSync('node transform_test.js', { cwd: env.tempDir, encoding: 'utf-8', timeout: 10000 });
        return { satisfied: true, evidence: 'total(["10","20"]) === 30 — data transform fixed' };
      } catch (e: any) {
        return { satisfied: false, evidence: `total(["10","20"]) !== 30: ${e.message}` };
      }
    },
  },
};
...
export const N3_UnknownDirRestore: RealTask = {
  ...
  description: 'Output directory abc_output is deleted mid-task — novel directory name',
  domain: 'replan',
  goalDescription: 'Write output to abc_output/report.csv, but abc_output is deleted mid-task',
  executionDomain: 'desktop',
  maxSteps: 15,
  maxTimeMs: 45000,
  allowedCapabilities: ['shell', 'file_read', 'file_write'],
  ...
  successCriteria: {
    ...
    check: async (env: TaskEnvironment) => {
      ...
      if (fs.existsSync(target)) {
        return { satisfied: true, evidence: 'abc_output/report.csv exists — directory restored' };
      }
      return { satisfied: false, evidence: 'abc_output/report.csv does not exist' };
    },
  },
  disturbance: {
    ...
    verify: async (env: TaskEnvironment) => {
      ...
      return { disturbed: !exists, evidence: exists ? 'abc_output still exists' : 'abc_output deleted' };
    },
  },
};
...
export const N4_TestSetupFix: RealTask = {
  ...
  description: 'Test references wrong source module — novel test configuration error',
  domain: 'replan',
  goalDescription: 'Fix app_test.js so it requires ./app instead of ./application',
  executionDomain: 'desktop',
  maxSteps: 15,
  maxTimeMs: 45000,
  allowedCapabilities: ['shell', 'file_read', 'file_write'],
  setup: async (env: TaskEnvironment) => {
    fs.mkdirSync(env.tempDir, { recursive: true });
    fs.writeFileSync(path.join(env.tempDir, 'app.js'), `function greet(name){return "Hello "+name;}\nmodule.exports={greet};\n`);
    fs.writeFileSync(path.join(env.tempDir, 'app_test.js'), `const {greet}=require('./application');\nconst assert=require('assert');\nassert.strictEqual(greet("World"),"Hello World");\nconsole.log('PASS');\n`);
  },
  successCriteria: {
    ...
    check: async (env: TaskEnvironment) => {
      try {
        const { execSync } = require('child_process');
        execSync('node app_test.js', { cwd: env.tempDir, encoding: 'utf-8', timeout: 10000 });
        return { satisfied: true, evidence: 'app_test.js PASS — test setup fixed' };
      } catch (e: any) {
        return { satisfied: false, evidence: `app_test.js fails: ${e.message}` };
      }
    },
  },
};
...
export const N5_CrossFileDep: RealTask = {
  ...
  description: 'Cross-file dependency broken — util.js exports wrong function name',
  domain: 'replan',
  goalDescription: 'Fix util.js so main.js can call formatCurrency() successfully',
  executionDomain: 'desktop',
  maxSteps: 15,
  maxTimeMs: 45000,
  allowedCapabilities: ['shell', 'file_read', 'file_write'],
  setup: async (env: TaskEnvironment) => {
    fs.mkdirSync(env.tempDir, { recursive: true });
    fs.writeFileSync(path.join(env.tempDir, 'util.js'), `function fmtCurrency(val){return "$"+val.toFixed(2);}\nmodule.exports={fmtCurrency};\n`);
    fs.writeFileSync(path.join(env.tempDir, 'main.js'), `const {formatCurrency}=require('./util');\nfunction price(item){return formatCurrency(item.price);}\nmodule.exports={price};\n`);
    fs.writeFileSync(path.join(env.tempDir, 'main_test.js'), `const {price}=require('./main');\nconst assert=require('assert');\nassert.strictEqual(price({price:9.99}),"$9.99");\nconsole.log('PASS');\n`);
  },
  successCriteria: {
    ...
    check: async (env: TaskEnvironment) => {
      try {
        const { execSync } = require('child_process');
        execSync('node main_test.js', { cwd: env.tempDir, encoding: 'utf-8', timeout: 10000 });
        return { satisfied: true, evidence: 'price({price:9.99}) === "$9.99" — cross-file dep fixed' };
      } catch (e: any) {
        return { satisfied: false, evidence: `main_test.js fails: ${e.message}` };
      }
    },
  },
};
...
export const N6_MultiStepPipeline: RealTask = {
  ...
  description: 'Multi-step data pipeline: extract, transform, validate — novel multi-step task',
  domain: 'multi_step',
  goalDescription: 'Fix the 3-stage pipeline (extract→transform→validate) so it produces correct output',
  executionDomain: 'desktop',
  maxSteps: 20,
  maxTimeMs: 60000,
  allowedCapabilities: ['shell', 'file_read', 'file_write'],
  setup: async (env: TaskEnvironment) => {
    fs.mkdirSync(env.tempDir, { recursive: true });
    fs.writeFileSync(path.join(env.tempDir, 'extract.js'), `function extract(raw){return raw.split("\\n").map(l=>l.trim()).filter(l=>l.length>0);}\nmodule.exports={extract};\n`);
    fs.writeFileSync(path.join(env.tempDir, 'transform.js'), `function transform(lines){return lines.map(l=>{const p=l.split(",");return{name:p[0],value:p[1]};});}\nmodule.exports={transform};\n`);
    fs.writeFileSync(path.join(env.tempDir, 'validate.js'), `function validate(items){return items.every(i=>i.name&&i.value);}\nmodule.exports={validate};\n`);
    fs.writeFileSync(path.join(env.tempDir, 'pipeline.js'), `const {extract}=require("./extract");\nconst {transform}=require("./transform2");\nconst {validate}=require("./validate");\nfunction run(raw){const lines=extract(raw);const items=transform(lines);return validate(items);}\nmodule.exports={run};\n`);
    fs.writeFileSync(path.join(env.tempDir, 'pipeline_test.js'), `const {run}=require("./pipeline");\nconst assert=require("assert");\nconst input="alice,100\\nbob,200";\nassert.strictEqual(run(input),true,"pipeline should validate");\nconsole.log("PASS");\n`);
  },
  successCriteria: {
    ...
    check: async (env: TaskEnvironment) => {
      try {
        const { execSync } = require('child_process');
        execSync('node pipeline_test.js', { cwd: env.tempDir, encoding: 'utf-8', timeout: 10000 });
        return { satisfied: true, evidence: 'pipeline_test.js PASS — multi-step pipeline fixed' };
      } catch (e: any) {
        return { satisfied: false, evidence: `pipeline_test.js fails: ${e.message}` };
      }
    },
  },
};
...
export const N7_EnvDisturbFileMove: RealTask = {
  ...
  description: 'File moved to unexpected subdirectory mid-task — novel environment disturbance',
  domain: 'replan',
  goalDescription: 'Read data.csv and compute sum, but data.csv gets moved to backup/ mid-task',
  executionDomain: 'desktop',
  maxSteps: 15,
  maxTimeMs: 45000,
  allowedCapabilities: ['shell', 'file_read', 'file_write'],
  ...
  successCriteria: {
    ...
    check: async (env: TaskEnvironment) => {
      try {
        const { execSync } = require('child_process');
        execSync('node sum_test.js', { cwd: env.tempDir, encoding: 'utf-8', timeout: 10000 });
        return { satisfied: true, evidence: 'compute() === 60 — file moved and restored' };
      } catch (e: any) {
        return { satisfied: false, evidence: `sum_test.js fails: ${e.message}` };
      }
    },
  },
  disturbance: {
    ...
    verify: async (env: TaskEnvironment) => {
      ...
      return { disturbed: !fs.existsSync(src) && fs.existsSync(backup), evidence: fs.existsSync(src) ? 'data.csv still in place' : 'data.csv moved to backup/' };
    },
  },
};
...
export const N8_RecursiveSchema: RealTask = {
  ...
  description: 'Nested config with wrong key at multiple levels — novel recursive inspection task',
  domain: 'code',
  goalDescription: 'Fix config.js so deepGet("db.host") returns "localhost" instead of undefined',
  executionDomain: 'desktop',
  maxSteps: 15,
  maxTimeMs: 45000,
  allowedCapabilities: ['shell', 'file_read', 'file_write'],
  setup: async (env: TaskEnvironment) => {
    fs.mkdirSync(env.tempDir, { recursive: true });
    fs.writeFileSync(path.join(env.tempDir, 'config.js'), `const cfg={database:{hst:"localhost",port:5432},cache:{host:"127.0.0.1",port:6379}};\nfunction deepGet(path){return path.split(".").reduce((o,k)=>o&&o[k],cfg);}\nmodule.exports={deepGet};\n`);
    fs.writeFileSync(path.join(env.tempDir, 'config_test.js'), `const {deepGet}=require('./config');\nconst assert=require('assert');\nassert.strictEqual(deepGet("db.host"),"localhost",'db.host should be localhost');\nconsole.log('PASS');\n`);
  },
  successCriteria: {
    ...
    check: async (env: TaskEnvironment) => {
      try {
        ...
        return { satisfied: true, evidence: 'deepGet("db.host") === "localhost" — recursive schema fixed' };
      } catch (e: any) {
        return { satisfied: false, evidence: `config_test.js fails: ${e.message}` };
      }
    },
  },
};
