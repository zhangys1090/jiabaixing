...
export class TrajectoryFlywheel {
  ...
  private appliedOptimizations: Map<string, { timestamp: number; impact: number }>;

  constructor(trajectoryDB: TrajectoryDatabase, config?: Partial<FlywheelConfig>) {
   ...
   * 计算总体统计
   */
  private calculateOverallStats(executions: ExecutionRecord[]): TrajectoryAnalysis['overallStats'] {
    ...
    const avgDuration = executions.reduce((sum, e) => sum + (e.total_duration || 0), 0) / executions.length;
    const avgToolCalls = executions.reduce((sum, e) => sum + (e.total_tool_calls || 0), 0) / executions.length;
   ...
   * 生成时间序列数据
   */
  private generateTimeSeriesData(executions: ExecutionRecord[]): TrajectoryAnalysis['timeSeriesData'] {
    const buckets: Map<string, { successRate: number; avgDuration: number; count: number }> = new Map();
    ...
    for (const execution of executions) {
      ...
      if (!buckets.has(bucketKey.toString())) {
        buckets.set(bucketKey.toString(), { successRate: 0, avgDuration: 0, count: 0 });
      }
      ...
      bucket.successRate = (bucket.successRate * (bucket.count - 1) + isSuccess) / bucket.count;
   ...
   * 分析工具使用
   */
  private analyzeToolUsage(toolInvocations: ToolInvocationRecord[]): TrajectoryAnalysis['toolStats'] {
    ...
    for (const invocation of toolInvocations) {
      ...
      stats.avgDuration = (stats.avgDuration * (stats.totalCalls - 1) + invocation.duration) / stats.totalCalls;
  ...
  private detectPatterns(
    ...
    const optimalToolSequences: Array<{ sequence: string[]; successRate: number }> = [];
    ...
    // 分析工具序列模式
    const toolSequenceSuccess: Map<string, { total: number; success: number }> = new Map();
  ...
  private identifyBottlenecks(
    ...
    // 查找最慢的工具
    const toolTimings: Map<string, { total: number; count: number }> = new Map();
    for (const invocation of toolInvocations) {
  ...
  private generateOptimizationSuggestions(
    ...
    // 建议1：失败率高的工具优化
    for (const [toolName, stats] of Object.entries(toolStats)) {
      if (stats.errorRate > 0.2 && stats.totalCalls >= this.config.minSampleSize) {
        suggestions.push({
    ...
    // 建议2：慢工具优化
    for (const [toolName, stats] of Object.entries(toolStats)) {
      if (stats.avgDuration > 3000 && stats.totalCalls >= this.config.minSampleSize) {
        suggestions.push({
  ...
  getImprovementTrend(): {
    ...
    data: Array<{ timestamp: number; successRate: number; avgDuration: number }>;
  } {
