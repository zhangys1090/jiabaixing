...
export interface EvalAssertion {
  ...
  /** 工具名（tool_call 类型） */
  toolName?: string;
  /** 期望值（output_contains/output_not_contains/json_field/regex 类型） */
  value?: string;
  /** JSON字段路径（json_field 类型） */
  fieldPath?: string;
  /** 期望的字段值（json_field 类型） */
  fieldValue?: unknown;
  /** 正则表达式（regex 类型） */
  pattern?: string;
  /** 最小分数（score_range 类型） */
...
export interface GoldenEvalCase {
  ...
  goldenOutput: string;
  /** 期望工具调用 */
  goldenToolCalls: Array<{ name: string; args?: Record<string, unknown> }>;
  /** LLM Judge 评估提示 */
  judgePrompt: string;
  /** 断言列表 */
...
export class GoldenEvalSet {
  private cases: Map<string, GoldenEvalCase> = new Map();

  constructor(cases?: GoldenEvalCase[]) {
    if (cases) {
   ...
   * 从JSON文件目录加载案例
   */
  static fromDirectory(dirPath: string): GoldenEvalSet {
    const set = new GoldenEvalSet();

    try {
      if (!fs.existsSync(dirPath)) {
        Logger.warn(`Golden Eval 目录不存在: ${dirPath}`, 'GoldenEvalSet');
        return set;
      }
      ...
      for (const file of files) {
        try {
          ...
          const cases = JSON.parse(content) as GoldenEvalCase[];
          ...
          Logger.warn(
            `加载 Golden Eval 文件失败: ${file} - ${(err as Error).message}`,
            'GoldenEvalSet'
          );
        }
      }
      ...
      Logger.info(
        `📂 从目录加载 ${set.size} 个 Golden Eval 案例: ${dirPath}`,
        'GoldenEvalSet'
      );
    } catch (err) {
      Logger.warn(
        `加载 Golden Eval 目录失败: ${(err as Error).message}`,
        'GoldenEvalSet'
      );
    }
   ...
   * 创建内置核心案例集
   */
  static createBuiltin(): GoldenEvalSet {
   ...
   * 添加案例
   */
  addCase(caseData: GoldenEvalCase): void {
    const validation = this.validateCase(caseData);
    if (!validation.valid) {
      Logger.warn(
        `案例验证失败: ${caseData.id} - ${validation.errors.join(', ')}`,
        'GoldenEvalSet'
      );
      return;
    }
   ...
   * 验证案例格式
   */
  validateCase(caseData: GoldenEvalCase): { valid: boolean; errors: string[] } {
    const errors: string[] = [];

    if (!caseData.id) errors.push('缺少 id');
    if (!caseData.category) errors.push('缺少 category');
    if (!caseData.input) errors.push('缺少 input');
    if (!caseData.expectedBehavior) errors.push('缺少 expectedBehavior');
    if (
      !['memory', 'tool_use', 'safety', 'planning', 'multi_step'].includes(
        caseData.category
      )
    ) {
   ...
   * 加载案例（可按类别过滤）
   */
  loadCases(category?: string): GoldenEvalCase[] {
   ...
   * 获取单个案例
   */
  getCase(id: string): GoldenEvalCase | undefined {
...
const BUILTIN_GOLDEN_CASES: GoldenEvalCase[] = [
  {
  ...
  },
  {
    ...
    category: 'memory',
    input:
      '记住这些信息：我叫张三，电话是13800138000，邮箱是zhangsan@example.com',
    expectedBehavior: '系统应存储多条信息到记忆中',
    goldenOutput:
      '好的，已为您记住：姓名张三、电话13800138000、邮箱zhangsan@example.com。',
    goldenToolCalls: [
  ...
  },
  {
    ...
    goldenOutput:
      '为您规划北京三日游：第一天故宫-天安门-王府井；第二天长城-明十三陵；第三天颐和园-圆明园-北大。',
    goldenToolCalls: [],
  ...
  },
  {
    ...
    goldenOutput:
      '我将分步骤分析代码质量：1.代码结构 2.命名规范 3.性能瓶颈 4.安全性 5.优化建议。',
    goldenToolCalls: [],
  ...
  },
  {
    ...
    goldenOutput:
      '搭建React项目的步骤：1.初始化项目 2.配置TypeScript 3.安装依赖 4.配置路由 5.创建基础组件。',
    goldenToolCalls: [],
  ...
  },
  {
    ...
    goldenOutput:
      '为您制定四周健身计划：第一周适应期、第二周增肌期、第三周强化期、第四周巩固期。',
    goldenToolCalls: [],
  ...
  },
  {
    ...
    goldenOutput:
      '电商订单数据库设计：用户表、商品表、订单表、订单详情表、支付记录表。',
    goldenToolCalls: [],
  ...
  },
  {
    ...
    goldenOutput:
      '北京今天晴，28°C。建议穿轻薄衣物。出行建议：适合户外活动，注意防晒。',
  ...
  },
  {
    ...
    expectedBehavior:
      '系统应分步骤执行：扫描文件→筛选PDF→按日期重命名→移动文件',
    goldenToolCalls: [
  ...
  },
  {
    ...
    goldenToolCalls: [],
    goldenOutput:
      '性能分析完成：发现3个瓶颈。优化方案：1.减少DOM操作 2.使用虚拟滚动 3.优化渲染。优化后的代码已生成。',
  ...
  },
  {
    ...
    goldenOutput:
      '备份完成：5个文件已上传。临时文件已清理：3个文件已删除。备份报告已生成。',
  ...
  },
  {
    ...
    goldenOutput:
      '竞品调研完成，对比分析如下：功能覆盖度80%，用户体验评分7.5/10。分析报告已生成。',
