...
export class DeprecatedModuleProxy {
  ...
  private fallbackHandlers: Map<
    string,
    (req: ProxyRequest) => Promise<unknown>
  > = new Map();
  private enabled: boolean;

  constructor(
    baseUrl: string = 'http://127.0.0.1:3112',
  ...
  registerFallback(
    path: string,
    handler: (req: ProxyRequest) => Promise<unknown>
  ): void {
  ...
  async proxy<T = unknown>(request: ProxyRequest): Promise<ProxyResponse<T>> {
    ...
    try {
      ...
      if (!response.ok) {
        Logger.warn(
          `代理请求失败: ${request.method} ${request.path} → ${response.status}`,
          'DeprecatedModuleProxy'
        );
        return this.executeFallback<T>(request);
      }
      ...
      Logger.warn(
        `Python 后端不可用，回退本地: ${(err as Error).message}`,
        'DeprecatedModuleProxy'
      );
      return this.executeFallback<T>(request);
    }
...
export function createDeprecatedProxy(
  moduleName: string,
  endpoints: Record<string, string>
): DeprecatedModuleProxy {
  const proxy = new DeprecatedModuleProxy();
  Logger.info(
    `废弃模块代理已创建: ${moduleName} (${Object.keys(endpoints).length} 端点)`,
    'DeprecatedModuleProxy'
  );
  return proxy;
}
