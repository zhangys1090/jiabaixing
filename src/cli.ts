...
const COMMANDS = ['/daemon', '/help', '/status', '/chat', '/gateway', '/schedule', '/config', '/web', '/clear', '/quit', '/exit'];

const SHELL_COMMANDS = [
  'npm', 'npx', 'yarn', 'pnpm', 'bun',
  'node', 'ts-node', 'tsx', 'deno',
  'cd', 'dir', 'ls', 'pwd', 'mkdir', 'rmdir', 'rm', 'cp', 'mv',
  'cat', 'type', 'echo', 'head', 'tail', 'less', 'more',
  'git', 'docker', 'kubectl',
  'python', 'python3', 'pip', 'pip3',
  'java', 'javac', 'mvn', 'gradle',
  'go', 'cargo', 'rustc',
  'ping', 'curl', 'wget', 'ssh', 'scp',
  'taskkill', 'netstat', 'ipconfig', 'ifconfig',
  'cls', 'clear', 'exit',
...
function detectShellCommand(input: string): string | null {
  ...
  if (firstWord.endsWith('.exe') || firstWord.endsWith('.cmd') || firstWord.endsWith('.bat')) return firstWord;
  return null;
}
...
async function checkBackendHealth(): Promise<{ online: boolean; status?: string; uptime?: number; services?: Record<string, unknown> }> {
  try {
    ...
    return { online, status, uptime: data.uptime as number, services: data.services as Record<string, unknown> };
  } catch {
    return { online: false };
  }
}
...
function printStatusBar(health: { online: boolean; status?: string }): void {
  ...
  const connText = health.online ? c(COLORS.green, 'connected') : c(COLORS.red, 'disconnected');
...
async function sendChatMessage(input: string): Promise<string> {
  ...
  try {
    ...
    clearThinking();
    const data = await res.json() as Record<string, unknown>;
    return (data.response || data.message || data.text || JSON.stringify(data)) as string;
  } catch (err) {
    clearThinking();
...
async function handleStatusCommand(): Promise<void> {
  ...
  console.log(`  后端服务: ${health.online ? c(COLORS.green, '🟢 在线') : c(COLORS.red, '🔴 离线')}`);
  ...
  if (health.services) {
    ...
    for (const [name, svc] of Object.entries(health.services)) {
      ...
      const mark = info.status === 'ok' ? c(COLORS.green, '🟢') : c(COLORS.red, '🔴');
...
async function handleGatewayMenu(rl: readline.Interface): Promise<void> {
  while (true) {
    ...
    console.log(`  ${COLORS.dim}  list  查看连接状态  |  back  返回${COLORS.reset}\n`);
    ...
    switch (choice) {
      ...
      case '2': {
        ...
        console.log('  ⏳ 连接中...');
        const ok = await im.connectPlatform('wechat', { mode: 'official', appId, appSecret, token });
        console.log(ok ? '  ✅ 连接成功' : '  ❌ 连接失败');
        break;
      }
      case '3': {
        console.log('\n  🐧 QQ 机器人 (Mirai)\n');
        const host = await ask(rl, '  Mirai HTTP 地址 [localhost]: ') || 'localhost';
        const port = await ask(rl, '  Mirai HTTP 端口 [8080]: ') || '8080';
        const vk = await ask(rl, '  verifyKey: ');
        const qq = await ask(rl, '  QQ 账号: ');
        console.log('  ⏳ 连接中...');
        const ok = await im.connectPlatform('qq', { miraiHttpHost: host, miraiHttpPort: port, miraiVerifyKey: vk, qqAccount: qq });
      ...
      case '5': {
        ...
        console.log('  ⏳ 连接中...');
        const ok = await im.connectPlatform('dingtalk', { appId: clientId, appSecret: clientSecret });
        console.log(ok ? '  ✅ 连接成功' : '  ❌ 连接失败');
        break;
      }
      case 'list': {
...
async function handleScheduleMenu(rl: readline.Interface): Promise<void> {
  while (true) {
    ...
    console.log(`  内置任务: 早安简报(8:00) · 情绪检查(30min) · 任务提醒(15min) · 行为分析(2:00)\n`);
    ...
    switch (choice) {
      case 'list': {
        try {
          const tasks = await fetch(`${backendUrl}/api/automation/tasks`).then(r => r.json()) as Array<Record<string, unknown>>;
          console.log('\n  定时任务:\n');
          if (Array.isArray(tasks) && tasks.length > 0) {
            for (const t of tasks) {
              ...
              console.log(`     cron: ${t.schedule || t.cronExpression || '-'}  |  执行 ${t.executionCount || 0} 次\n`);
            }
          } else {
      ...
      case 'add': {
        ...
        try {
          const res = await fetch(`${backendUrl}/api/automation/tasks`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ name, schedule: cron, description: desc, enabled: true }),
          }).then(r => r.json()) as Record<string, unknown>;
          console.log(res.success ? `  ✅ 任务 "${name}" 已创建` : `  ❌ 创建失败`);
        } catch {
          console.log('  ⚠️ 后端服务未启动');
        }
        break;
      ...
      case 'toggle': {
        ...
        try {
          const res = await fetch(`${backendUrl}/api/automation/tasks/${id}/toggle`, { method: 'PATCH' }).then(r => r.json()) as Record<string, unknown>;
      ...
      case 'run': {
        const id = await ask(rl, '  任务ID: ');
        try {
          const res = await fetch(`${backendUrl}/api/automation/tasks/${id}/execute`, { method: 'POST' }).then(r => r.json()) as Record<string, unknown>;
          console.log(res.success ? '  ✅ 已执行' : `  ❌ 失败: ${res.error || ''}`);
        } catch {
          console.log('  ⚠️ 后端服务未启动');
        }
        break;
      ...
      case 'triggers': {
        try {
          const triggers = await fetch(`${backendUrl}/api/automation/triggers`).then(r => r.json()) as Array<Record<string, unknown>>;
          console.log('\n  触发器队列:\n');
          if (Array.isArray(triggers) && triggers.length > 0) {
            for (const t of triggers) {
              console.log(`  🔔 [${t.type}] ${t.reason} (优先级: ${t.priority})`);
            }
          } else {
            console.log('  (队列为空)\n');
          }
      ...
      case 'patterns': {
        try {
          const patterns = await fetch(`${backendUrl}/api/automation/patterns`).then(r => r.json()) as Record<string, unknown>;
          console.log('\n  用户行为模式:\n');
          if (patterns.activeHours) {
            console.log(`  活跃时段: ${(patterns.activeHours as string[]).join(', ')}`);
            console.log(`  常用话题: ${(patterns.frequentTopics as string[] || []).join(', ')}`);
            console.log(`  任务完成率: ${Math.round(((patterns.taskCompletionRate as number) || 0) * 100)}%`);
          } else {
            console.log('  (暂无足够数据)\n');
          }
        } catch {
...
async function handleConfigMenu(rl: readline.Interface): Promise<void> {
  while (true) {
    ...
    switch (choice) {
      case 'show': {
        ...
        if (fs.existsSync(envFile)) {
          ...
          for (const line of lines) {
            ...
            if (trimmed.includes('KEY=') || trimmed.includes('SECRET=') || trimmed.includes('VERIFY_KEY=')) {
      ...
      case 'model': {
        try {
          const res = await fetch(`${backendUrl}/api/llm/status`).then(r => r.json()) as Record<string, unknown>;
          console.log('\n  LLM 模型状态:\n');
          console.log(`  当前模型: ${res.currentModel || 'default'}`);
          if (res.availableModels) {
            for (const m of res.availableModels as Array<Record<string, string>>) {
...
function setupReadlineHistory(rl: readline.Interface, state: ReplState): void {
  ...
  rli.input.on('keypress', (_str: string, key: KeypressResult) => {
    ...
    } else if (key.name === 'down' && state.historyIndex < state.history.length - 1) {
      state.historyIndex++;
      readline.moveCursor(process.stdout, -rli.line.length, 0);
      readline.clearLine(process.stdout, 0);
...
async function replLoop(rl: readline.Interface, state: ReplState): Promise<void> {
async function handleDaemonMenu(rl: readline.Interface): Promise<void> {
  ...
  while (true) {
    ...
    switch (choice) {
      ...
      case 'logs': {
        ...
        console.log(logText.split('\n').map((l: string) => `  \x1b[2m${l}\x1b[0m`).join('\n'));
  ...
  while (true) {
    ...
    if (input.startsWith('/')) {
      ...
      switch (cmd) {
        ...
        case '/status':
          ...
          continue;
        case '/chat':
          console.log(`  ${COLORS.dim}已处于聊天模式，直接输入消息即可${COLORS.reset}\n`);
          continue;
        case '/gateway':
        case '/gw':
        ...
        case '/w':
          ...
          try {
            const { execSync } = require('child_process') as { execSync: (cmd: string) => Buffer };
            if (process.platform === 'win32') {
    ...
    if (shellCmd) {
      ...
      console.log(`  ${COLORS.dim}提示: 输入 ${COLORS.cyan}/help${COLORS.dim} 查看 jiabaixing CLI 支持的命令${COLORS.reset}`);
      if (input.startsWith('npm start') || input.startsWith('npm run')) {
        console.log(`  ${COLORS.dim}      启动服务: 在系统终端执行 ${COLORS.cyan}${input}${COLORS.dim}，然后在另一个终端执行 ${COLORS.cyan}npm run cli${COLORS.dim} 进入 CLI${COLORS.reset}`);
      }
      console.log();
      continue;
    }
    ...
    try {
      ...
      if (state.aborted) {
        ...
        console.log(`  ${COLORS.dim}请确认后端服务已运行: npm start${COLORS.reset}\n`);
      }
    }
  }
}
...
async function mainLoop(): Promise<void> {
  ...
  if (!health.online) {
    console.log(`  ${COLORS.yellow}⚠ 后端服务未运行，部分功能不可用。输入 /status 查看详情。${COLORS.reset}\n`);
  }

  console.log(`  ${COLORS.dim}输入消息开始对话，输入 /help 查看所有命令${COLORS.reset}\n`);
  ...
  EventBus.on('integration_message', (payload: { platform: string; fromName?: string; from?: string; content: string }) => {
    const from = payload.fromName || payload.from || '';
    console.log(`\n  ${COLORS.bold}${COLORS.magenta}📩${COLORS.reset} [${payload.platform}] ${from}: ${payload.content}`);
    process.stdout.write(`  ${COLORS.bold}${COLORS.magenta}✦${COLORS.reset} ${COLORS.bold}${COLORS.cyan}jiabaixing${COLORS.reset}${COLORS.dim} > ${COLORS.reset}`);
  });
  ...
  switch (cmd) {
    ...
    case 'status': {
      ...
      if (status.running) {
        ...
        console.log(`🟢 运行中  PID: ${status.state!.pid}  端口: ${status.state!.port}  运行: ${uptime}${status.memoryUsage ? `  内存: ${status.memoryUsage}` : ''}`);
      } else {
        console.log('🔴 未运行');
      }
