...
import { getProviderManager, ProviderConfig } from '../models/ProviderManager';
...
const PROVIDER_TEMPLATES: Record<string, { displayName: string; baseUrl: string; model: string; hint: string }> = {
  deepseek: {
...
/** 交互式添加 Provider */
async function addProviderInteractive(manager: ReturnType<typeof getProviderManager>): Promise<void> {
  ...
  names.forEach((key, i) => {
    ...
    console.log(`  ${c(COLORS.cyan, `${i + 1}.`)} ${c(COLORS.bold, t.displayName)} ${COLORS.dim}- ${t.hint}${COLORS.reset}`);
  });
  console.log();
  ...
  if (!apiKey && templateKey !== 'local') {
    console.log(`  ${c(COLORS.yellow, '⚠️ 未输入 API Key，稍后可在配置文件中补填')}\n`);
  }
...
function listProviders(manager: ReturnType<typeof getProviderManager>): void {
  ...
  if (providers.length === 0) {
    ...
    if (process.env.DEEPSEEK_API_KEY) envHints.push('DeepSeek (DEEPSEEK_API_KEY)');
    if (process.env.OPENAI_API_KEY && !process.env.DEEPSEEK_API_KEY) envHints.push('OpenAI (OPENAI_API_KEY)');

    console.log(`  ${c(COLORS.yellow, '⚠️  未配置任何 Provider')}\n`);
    if (envHints.length > 0) {
      console.log(`  ${COLORS.dim}检测到 .env 中有以下配置（可执行 npm run setup 导入）:${COLORS.reset}`);
  ...
  console.log(`  ${COLORS.dim}已注册 ${providers.length} 个 Provider，路由: ${manager.getRouting().enabled ? '启用' : '禁用'}${COLORS.reset}\n`);

  providers.forEach(p => {
    const isPrimary = primary?.name === p.name;
    const prefix = isPrimary ? c(COLORS.green, '★') : ' ';
    const status = p.healthy === undefined ? c(COLORS.dim, '? 未检测') :
                   p.healthy ? c(COLORS.green, '✓ 正常') : c(COLORS.red, '✗ 异常');
    const lastCheck = p.lastHealthCheck
      ? `${Math.round((Date.now() - p.lastHealthCheck) / 1000)}s前`
      : '';

    console.log(`  ${prefix} ${c(COLORS.bold, p.displayName)} ${isPrimary ? c(COLORS.dim, '(主)') : ''}`);
...
/** 测试所有 Provider 连接 */
async function testProviders(manager: ReturnType<typeof getProviderManager>): Promise<void> {
  const providers = manager.getAll();
  if (providers.length === 0) {
...
export async function runSetupCLI(args: string[]): Promise<void> {
  ...
  while (true) {
    ...
    console.log(`  ${c(COLORS.cyan, '4')}  切换路由 (当前: ${manager.getRouting().enabled ? '开' : '关'})`);
    ...
    switch (choice) {
      ...
      case '3': {
        ...
        providers.forEach((p, i) => {
          ...
          console.log(`  ${c(COLORS.cyan, `${i + 1}.`)} ${p.displayName}${mark}`);
        });
        const idx = parseInt(await ask('选择主模型编号', '1')) - 1;
        if (idx >= 0 && idx < providers.length) {
          manager.setPrimary(providers[idx].name);
          console.log(`  ${c(COLORS.green, `✅ 主模型已切换为 ${providers[idx].displayName}`)}\n`);
        }
        break;
      }
      case '4': {
        ...
        const store = (manager as unknown as { store: { routingEnabled: boolean } }).store;
        store.routingEnabled = newVal;
        console.log(`  ${c(COLORS.green, `✅ 路由已${newVal ? '启用' : '禁用'}`)}\n`);
        break;
      }
    }
  }
