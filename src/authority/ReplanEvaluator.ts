...
const REPLAN_ELIGIBLE_SOURCES = new Set<string>([
  'explicit',
  'workspace_context',
  'desktop_context',
  'task_context',
...
const STRONG_INVALIDATION_TYPES = new Set<string>([
  'resource_deleted',
...
export interface ReplanEvaluator {
  evaluate(
    goal: Goal,
    impact: GoalImpact,
...
class ReplanEvaluatorImpl implements ReplanEvaluator {
  private readonly recentRequests: Map<string, ReplanRequest> = new Map();
  private readonly maxRecentRequests = 200;
  ...
  evaluate(
    ...
    try {
      ...
      Logger.info(
        `[D7-2] ReplanRequest ${request.requestId} for Goal ${goal.goalId} (plan v${goal.planVersion}): ${staleResult.reason}`,
        'ReplanEvaluator'
      );
      ...
      Logger.error(
        `[D7-2] ReplanEvaluator.evaluate failed: ${(err as Error).message}`,
        err as Error,
        'ReplanEvaluator'
      );
      return null;
  ...
  private checkBindingEligibility(
    goal: Goal,
    impact: GoalImpact
  ...
  private checkStrongInvalidationSignal(
    goal: Goal,
    impact: GoalImpact
  ): { stale: boolean; reason: string } {
    if (impact.impactType === 'git_change' && goal.bindings) {
      ...
      if (binding.repository || binding.repositoryPath) {
        if (
          impact.reason.includes('goal_bindings_match') ||
  ...
  private isStructuralFileChange(reason: string): boolean {
    return (
  ...
  private checkConsecutiveNegativeEvidence(
    evidence: readonly GoalEvidence[]
  ): { stale: boolean; reason: string } {
    ...
    const negativeCount = recentEvidence.filter(
      (e) => e.progressDelta <= -0.3
    ).length;
...
let instance: ReplanEvaluator | null = null;

export function getReplanEvaluator(): ReplanEvaluator {
  if (!instance) {
...
export function resetReplanEvaluator(): void {
  instance = null;
}
