...
import { getTaskEnvironmentController, resetTaskEnvironmentController } from '../../harness/realTask/TaskEnvironmentController';
...
interface AuthenticityTrace {
  ...
  disturbanceVerified: boolean;
}

async function runTaskWithTrace(
  task: RealTask,
  tempDir: string
): Promise<AuthenticityTrace> {
  resetAll();
...
async function runT09WithDisturbance(tempDir: string): Promise<AuthenticityTrace> {
  resetAll();
...
describe('D8-2.1: Task Authenticity — T08 Recovery', () => {
  ...
  test('A2: T08 first failure must enter Observation/Evidence', async () => {
    ...
    try {
      const trace = await runTaskWithTrace(T08_TestFailAndFix, tempDir);
...
describe('D8-2.1: Task Authenticity — T09 Replan', () => {
  ...
  test('B1: T09 initial directory must actually exist', async () => {
    ...
    try {
      const env = makeEnv(tempDir);
      if (T09_EnvironmentChangeReplan.setup) await T09_EnvironmentChangeReplan.setup(env);

      const resultDir = path.join(tempDir, 'result_dir');
      expect(fs.existsSync(resultDir)).toBe(true);
      expect(fs.statSync(resultDir).isDirectory()).toBe(true);

      if (T09_EnvironmentChangeReplan.teardown) await T09_EnvironmentChangeReplan.teardown(env);
    } finally {
      cleanDir(tempDir);
    }
  }, TIMEOUT);
...
describe('D8-2.1: Three-Way Separation Verification', () => {
  ...
  test('T08 RealTask has no expectedAction field', () => {
    const task = T08_TestFailAndFix as unknown as Record<string, unknown>;
  ...
  test('T09 RealTask has no replanSolution field', () => {
    const task = T09_EnvironmentChangeReplan as unknown as Record<string, unknown>;
  ...
  test('T09 has real disturbance spec', () => {
    expect(T09_EnvironmentChangeReplan.disturbance).toBeDefined();
    expect(T09_EnvironmentChangeReplan.disturbance!.phase).toBe('after_first_step');
    expect(typeof T09_EnvironmentChangeReplan.disturbance!.execute).toBe('function');
    expect(typeof T09_EnvironmentChangeReplan.disturbance!.verify).toBe('function');
  });
