...
import { RealTaskHarness, type RecoveryProposerMode } from '../../harness/realTask';
import {
  T08_TestFailAndFix,
  G1_MulInsteadOfAdd,
  G2_OffByOne,
  ...
  G6_TwoStageError,
  ALL_REAL_TASKS,
} from '../../harness/realTask';
import type { RealTask, RealTaskResult } from '../../harness/realTask';
...
interface MainlineRecoveryTrace {
  taskId: string;
  group: RecoveryProposerMode;
...
interface MainlineAblationMetrics {
  group: RecoveryProposerMode;
  totalAttempts: number;
  declaredCompleted: number;
  verifiedRecoveries: number;
  recoveryRate: number;
  falseRecoveryCount: number;
  falseRecoveryRate: number;
  averageTimeMs: number;
  errorCount: number;
  perTask: { taskId: string; verified: boolean; time: number }[];
}

async function runMainlineAblationGroup(
  tasks: RealTask[],
  mode: RecoveryProposerMode
): Promise<MainlineAblationMetrics> {
  const harness = new RealTaskHarness();
  harness.setRecoveryProposerMode(mode);

  const traces: MainlineRecoveryTrace[] = [];
...
function formatMetrics(m: MainlineAblationMetrics): string {
  return `
...
`);
  ...
  describe('planVersion Semantic Audit', () => {
    test('planVersion > 0 reliably indicates recovery/replan phase', async () => {
      ...
      const result = await harness.runTask(T08_TestFailAndFix);
      ...
      expect(result.taskId).toBe(T08_TestFailAndFix.taskId);
    }, TIMEOUT);
  });
});
