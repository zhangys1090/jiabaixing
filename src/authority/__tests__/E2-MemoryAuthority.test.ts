...
describe('E2-V2: Memory Authority Runtime Canonicality', () => {
  ...
  describe('M1: bridge registered → write → Python owner', () => {
    test('write with bridge registered goes to Python', async () => {
      const mockWrite = jest.fn().mockResolvedValue({ success: true, memoryId: 'mem_1', operationId: 'op_1', source: 'python' as const });
      const mockRead = jest.fn().mockResolvedValue({ items: [], operationId: 'op_2', source: 'python' as const });
      ma.registerBridge(
        mockWrite as (req: MemoryWriteRequest) => Promise<MemoryWriteResult>,
        mockRead as (req: MemoryReadRequest) => Promise<MemoryReadResult>
      );
  ...
  describe('M2: bridge registered → read → Python owner', () => {
    test('read with bridge registered goes to Python', async () => {
      const mockWrite = jest.fn().mockResolvedValue({ success: true, memoryId: 'mem_1', operationId: 'op_1', source: 'python' as const });
      const mockRead = jest.fn().mockResolvedValue({ items: [{ id: '1', content: 'data', memoryType: 'short_term' as const, relevanceScore: 0.9, timestamp: Date.now() }], operationId: 'op_2', source: 'python' as const });
      ma.registerBridge(
        mockWrite as (req: MemoryWriteRequest) => Promise<MemoryWriteResult>,
        mockRead as (req: MemoryReadRequest) => Promise<MemoryReadResult>
      );
