...
async function runLoopRealityTest(
  task: RealTask,
  tempDir: string
): Promise<LoopRealityTrace> {
  resetAll();
...
describe('D7-4: AutonomousLoop Reality — Observe-Act-Verify-Replan-Complete', () => {
  test(
    'N2 data_transform: loop observes failure, replans, fixes, verifies completion',
    async () => {
      ...
      try {
        ...
        expect(trace.totalSteps).toBeGreaterThanOrEqual(1);
      } finally {
        cleanDir(tempDir);
      }
    },
  ...
  test(
    'N3 unknown_dir_restore: loop restores missing directory via replan',
    async () => {
  ...
  test(
    'N4 test_setup_fix: loop fixes require path via replan',
    async () => {
  ...
  test(
    'N6 multi_step_pipeline: loop fixes multi-step pipeline via replan',
    async () => {
  ...
  test(
    'T08 test_fail_fix: classic failure-replan-fix-verify loop',
    async () => {
      ...
      try {
        ...
        expect(trace.terminationReason).toBe(
          LoopTerminationReason.GOAL_COMPLETED
        );
      } finally {
        cleanDir(tempDir);
      }
  ...
  test(
    'D7-4 summary: at least 4/5 tasks complete via Observe-Act-Verify-Replan loop',
    async () => {
      ...
      for (const task of tasks) {
        const tempDir = makeTempDir(
          'summary_' + task.taskId.replace(/[^a-z0-9]/gi, '_')
        );
        try {
      ...
      const falseRecovery = traces.filter(
        (t) =>
          t.independentlyVerified === false && t.finalGoalStatus === 'completed'
      ).length;
      ...
      process.stderr.write(
        `total=${traces.length} verified=${verified} withReplan=${withReplan} withFailure=${withFailure} falseRecovery=${falseRecovery}\n`
      );
      for (const t of traces) {
        process.stderr.write(
          `  ${t.taskId}: verified=${t.independentlyVerified} status=${t.finalGoalStatus} steps=${t.totalSteps} replans=${t.totalReplans} failure=${t.hadFailure}\n`
        );
      }
      process.stderr.write('='.repeat(55) + '\n');
