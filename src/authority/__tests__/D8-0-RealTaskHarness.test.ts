...
describe('D8-0: Real Task Harness', () => {
      ...
      const task: RealTask = {
        ...
        successCriteria: {
          ...
          check: async (env: TaskEnvironment) => {
            ...
            return {
              satisfied: exists,
              evidence: exists ? 'marker found' : 'marker missing',
            };
          },
        },
      ...
      const tasks: RealTask[] = [
        {
          ...
          successCriteria: {
            ...
            check: async () => ({ satisfied: true, evidence: 'ok' }),
          },
        },
        {
          taskId: 'fail_1',
          ...
          successCriteria: {
            ...
            check: async () => ({ satisfied: false, evidence: 'no' }),
          },
        },
      ];
  ...
  describe('Task definitions', () => {
    test('ALL_REAL_TASKS has 36 tasks', () => {
      expect(ALL_REAL_TASKS.length).toBe(36);
    });
  ...
  describe('External verification (filesystem)', () => {
    test('T01: file create — external verification works', async () => {
      ...
      try {
        ...
        const check = await T01.successCriteria.check({
          workingDir: process.cwd(),
          tempDir,
          platform: process.platform,
          env: {} as Record<string, string>,
    ...
    test('T02: file modify — external verification works', async () => {
      ...
      try {
        ...
        const check = await T02.successCriteria.check({
          workingDir: process.cwd(),
          tempDir,
          platform: process.platform,
          env: {} as Record<string, string>,
    ...
    test('T06: code modify — external verification works', async () => {
      ...
      try {
        ...
        const check = await T06.successCriteria.check({
          workingDir: process.cwd(),
          tempDir,
          platform: process.platform,
          env: {} as Record<string, string>,
    ...
    test('T07: run test — external verification works', async () => {
      ...
      try {
        ...
        const check = await T07.successCriteria.check({
          workingDir: process.cwd(),
          tempDir,
          platform: process.platform,
          env: {} as Record<string, string>,
    ...
    test('T08: test fail and fix — verification detects unfixed bug', async () => {
      ...
      try {
        ...
        const checkBefore = await T08.successCriteria.check({
          workingDir: process.cwd(),
          tempDir,
          platform: process.platform,
          env: {} as Record<string, string>,
        ...
        const checkAfter = await T08.successCriteria.check({
          workingDir: process.cwd(),
          tempDir,
          platform: process.platform,
          env: {} as Record<string, string>,
  ...
  describe('Metrics', () => {
    test('falseCompletionRate = 0 when all declared completions are verified', () => {
      ...
      const falseCompletions = results.filter(
        (r) => r.declaredCompleted && !r.externallyVerified
      ).length;
