...
export class DynamicTaskAdjuster {
   ...
   * 开始监控任务执行
   */
  startMonitoring(taskIds: string[]): void {
    if (this.isMonitoring) return;

    this.isMonitoring = true;
    Logger.info('动态任务调整器：开始监控任务执行', 'DynamicTaskAdjuster');
  ...
  stopMonitoring(): void {
    ...
    Logger.info('动态任务调整器：停止监控', 'DynamicTaskAdjuster');
  }
  ...
  evaluateTaskState(
    ...
    if (state.startTime) {
      ...
      if (
        elapsedMinutes > this.monitoringConfig.timeThreshold &&
        state.progress < 50
      ) {
        return {
          type: 'retry',
    ...
    // 检查资源使用
    if (
      state.resourceUsage.cpu > this.monitoringConfig.resourceThreshold.cpu ||
    ...
    // 检查进度
    if (
      state.progress < this.monitoringConfig.progressThreshold &&
   ...
   * 执行调整决策
   */
  private executeAdjustment(decision: AdjustmentDecision): void {
    Logger.info(
      `动态任务调整器：执行调整 - ${decision.type}`,
      'DynamicTaskAdjuster'
    );
    Logger.info(`   原因: ${decision.reason}`, 'DynamicTaskAdjuster');
    Logger.info(
      `   影响任务: ${decision.affectedTasks.join(', ')}`,
      'DynamicTaskAdjuster'
    );
  ...
  private handleRetry(decision: AdjustmentDecision): void {
    for (const taskId of decision.affectedTasks) {
      ...
      if (state) {
        ...
        Logger.info(
          `   重试任务: ${taskId} (第${state.errorCount}次)`,
          'DynamicTaskAdjuster'
        );
      }
    }
  ...
  private handleSkip(decision: AdjustmentDecision): void {
    for (const taskId of decision.affectedTasks) {
      const state = this.executionStates.get(taskId);
      if (state) {
        ...
        this.executionStates.set(taskId, state);
        Logger.info(`   跳过任务: ${taskId}`, 'DynamicTaskAdjuster');
      }
    }
  }

  private handleReplan(decision: AdjustmentDecision): void {
    Logger.info(
      `   重新规划任务: ${decision.affectedTasks.join(', ')}`,
      'DynamicTaskAdjuster'
    );
    // 实际应用中这里应该调用任务重新规划逻辑
    for (const taskId of decision.affectedTasks) {
  ...
  private handleParallelize(decision: AdjustmentDecision): void {
    Logger.info(
      `   并行化任务: ${decision.affectedTasks.join(', ')}`,
      'DynamicTaskAdjuster'
    );
    // 实际应用中这里应该调整任务执行策略为并行
  }

  private handleSequentialize(decision: AdjustmentDecision): void {
    Logger.info(
      `   串行化任务: ${decision.affectedTasks.join(', ')}`,
      'DynamicTaskAdjuster'
    );
    // 实际应用中这里应该调整任务执行策略为串行
  }

  private handleAbort(decision: AdjustmentDecision): void {
    Logger.info(
      `   中止任务: ${decision.affectedTasks.join(', ')}`,
      'DynamicTaskAdjuster'
    );
    for (const taskId of decision.affectedTasks) {
      const state = this.executionStates.get(taskId);
   ...
   */
  adjustTaskPlan(
    currentPlan: TaskDecomposition,
    executionStates: Map<string, TaskExecutionState>
  ): TaskDecomposition {
    const adjustedSubTasks: SubTask[] = [];
    ...
    for (const subTask of currentPlan.subTasks) {
      ...
      if (!state || state.status === 'pending') {
        ...
        if (state.progress > 0) {
          ...
          adjustedTask.estimatedTime = Math.ceil(
            remainingProgress * timePerProgress
          );
        }
    ...
    return {
      ...
      totalEstimatedTime: adjustedSubTasks.reduce(
  ...
  private recalculateParallelGroups(subTasks: SubTask[]): string[][] {
    const groups: string[][] = [];
        ...
        .filter(
          (t) =>
            ...
            !t.dependencies.some((dep) =>
              subTasks.find((st) => st.id === dep)?.dependencies.includes(t.id)
            )
        )
        .map((t) => t.id);
  ...
  private recalculateCriticalPath(subTasks: SubTask[]): string[] {
    const path: string[] = [];
  ...
  adjustForRetry(
    ...
    for (const nodeId of failedNodeIds) {
      ...
      if (state) {
        ...
        if (decision.parameters?.timeout) {
          ...
          Logger.info(
            `  🔧 调整任务 [${nodeId}] 超时时间: ${node.timeout}ms`,
            'DynamicTaskAdjuster'
          );
        }
        ...
        if (decision.parameters?.retryCount !== undefined) {
          ...
          adjustedNodes.push(nodeId);
          Logger.info(
            `  🔧 调整任务 [${nodeId}] 重试次数: ${node.maxRetries}`,
            'DynamicTaskAdjuster'
          );
        }
    ...
    if (adjustedNodes.length > 0) {
      Logger.info(
        `📊 动态任务调整完成: ${adjustedNodes.length}/${failedNodeIds.length} 个节点已调整`,
        'DynamicTaskAdjuster'
      );
    }
  ...
  getExecutionStatistics(): {
    ...
    return {
      ...
      averageProgress:
        states.reduce((sum, s) => sum + s.progress, 0) / states.length || 0,
   ...
   * 获取调整历史
   */
  getAdjustmentHistory(): AdjustmentDecision[] {
...
export default DynamicTaskAdjuster;
