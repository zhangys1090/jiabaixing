...
/** 技能洞察报告生成结果 */
export interface SkillInsightReportResult extends SkillInsightReport {
  topSkills: Array<{
    name: string;
    usageCount: number;
    successRate: number;
...
export class SkillUsageTracker {
  ...
  private save(): void {
    try {
      ...
      Logger.warn(
        `SkillUsageTracker 持久化失败: ${(err as Error).message}`,
        'SkillUsageTracker'
      );
    }
  }
  ...
  getSummary(): { total: number; active: number; stale: number } {
    ...
    return {
      ...
      active: all.filter(
        (s) =>
          s.lastUsedAt &&
          now - new Date(s.lastUsedAt).getTime() <= staleThreshold
      ).length,
      stale: all.filter(
        (s) =>
          !s.lastUsedAt ||
  ...
  getAutoGeneratedSkillNames(): string[] {
    return Object.keys(this.data.skills).filter((name) =>
   ...
   */
  shareSkillInsights(agentId: string): SkillInsightReportResult {
    ...
    for (const skill of topSkills.slice(0, 3)) {
      if (skill.avgQuality < 0.7) {
        recommendations.push(
          `建议优化 ${skill.name}（质量分 ${skill.avgQuality.toFixed(2)}）`
        );
      } else {
    ...
    // 添加低使用率技能的建议
    const lowUsageSkills = allSkills.filter(
      (s) => s.useCount === 0 && s.loadCount > 0
    );
    if (lowUsageSkills.length > 0) {
      recommendations.push(
        `发现 ${lowUsageSkills.length} 个已加载但未使用的技能，建议评估是否需要`
      );
    }
   ...
   */
  integrateExternalInsights(report: SkillInsightReport): number {
    // 验证报告完整性
    if (
      !report.agentId ||
      !report.generatedAt ||
    ...
    for (const externalSkill of report.topSkills) {
      ...
      if (localWeight + externalWeight > 0) {
        const adjustedScore =
          (localRecord.qualityScore * localWeight +
