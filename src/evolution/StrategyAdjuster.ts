...
export class StrategyAdjuster {
   ...
   * @param signal - 学习信号
   */
  public recordSignal(signal: LearningSignal): void {
    this.totalSignals++;

    // 任务级信号：无 toolName，记录到任务统计
    if (
      signal.signalType === 'task_success' ||
    ...
    Logger.debug(
      `📊 策略调整器记录工具信号: ${signal.toolName} (${signal.signalType})`,
      'StrategyAdjuster'
    );
  }
   ...
   */
  public getAdjustedToolPriority(tools: string[]): string[] {
   ...
   * @returns 反思配置
   */
  public getAdjustedReflectionConfig(): ReflectionConfig {
    let toolFailures = 0;
    let toolSuccesses = 0;
