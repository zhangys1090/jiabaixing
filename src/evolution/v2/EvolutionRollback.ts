...
export class EvolutionRollback {
   ...
   * 创建回滚检查点：为所有涉及文件创建快照
   */
  createCheckpoint(planId: string, actions: EvolutionAction[]): RollbackCheckpoint {
    const snapshot: Record<string, string> = {};
    
    for (const action of actions) {
      if (action.type === 'MODIFY_FILE' || action.type === 'DELETE_FILE') {
        const filePath = (action.target as any).filePath || action.target as string;
        if (fs.existsSync(filePath)) {
          try {
            snapshot[filePath] = fs.readFileSync(filePath, 'utf-8');
            Logger.debug(`Snapshot saved: ${filePath}`, 'EvolutionRollback');
          } catch (e) {
            Logger.error(`Failed to snapshot ${filePath}`, e as Error, 'EvolutionRollback');
          }
        }
      }
    }
    ...
    Logger.info(`💾 Checkpoint created: ${checkpoint.id} (${Object.keys(snapshot).length} files)`, 'EvolutionRollback');
    return checkpoint;
  }
  ...
  async rollback(checkpointId: string): Promise<{ success: boolean; error?: string }> {
    ...
    try {
      Logger.info(`⏪ Starting rollback to checkpoint: ${checkpointId}`, 'EvolutionRollback');
      ...
      for (const [filePath, originalContent] of Object.entries(checkpoint.snapshot)) {
        if (originalContent) {
          ...
          if (fs.existsSync(filePath)) {
            ...
            Logger.debug(`Rolled back delete: ${filePath}`, 'EvolutionRollback');
          }
        }
      }

      Logger.info(`✅ Rollback completed: ${checkpointId}`, 'EvolutionRollback');
      return { success: true };
    } catch (error) {
      Logger.error(`❌ Rollback failed: ${checkpointId}`, error as Error, 'EvolutionRollback');
  ...
  cleanOldCheckpoints(daysToKeep: number = 7): void {
    ...
    if (deleted > 0) {
      Logger.info(`🧹 Cleaned up ${deleted} old checkpoints`, 'EvolutionRollback');
    }
  }
}
