...
1. 这个改动属于哪一层？(E/T/C/S/L/V 还是 core/security/server 还是 authority)?
2. 这个改动的安全影响是什么？
3. 有对应的测试文件吗？
4. **Authority 约束**：这个改动是否涉及 action 执行？如果是，必须经过 DecisionAuthority → ActionAuthority 链路，不能直接执行。详见 [docs/AUTHORITY_RECONSTRUCTION.md](docs/AUTHORITY_RECONSTRUCTION.md) §9。
...
P0 — Authority Reconstruction (2026-09-10 FROZEN):

- ✅ A-P0: ActionAuthority — 任何OS mutation必须经过ActionAuthority
- ✅ D4-I1: Python Decision → TS delegation (cross-process FINAL, no re-decision)
- ✅ D4-I2: Desktop — matchSkill/parseLLMAction → Proposer → DecisionAuthority
- ✅ D4-I3: Orchestrator — decomposeGoal → OrchestratorProposer → Plan Decision
- ✅ D4-I4: Global Authority Replay — 3 scenarios, 5 replay contracts
- ✅ D5: Learning Authority — Evidence→PredictionError→BeliefUpdate→Future Decision changes
- ❌ D6: Memory Authority — canonical owner for TS/Python/Redis memory
- ❌ D7: Long-Horizon Agency — autonomous replanning

> **详见 [docs/AUTHORITY_RECONSTRUCTION.md](docs/AUTHORITY_RECONSTRUCTION.md)** — 总方案、验收标准、阶段边界、开发标准均已冻结。
