...
function _evolutionRiskToCore(level) {
    const map = {
        safe: 'low',
        cautious: 'medium',
        restricted: 'high',
...
class SelfModificationEngine {
    ...
    /** 禁止删除的入口文件 */
    forbiddenDeletePaths = new Set([
        'src/main.ts',
        'src/index.ts',
        'package.json',
    ]);
    /** 需要确认的核心路径 */
    cautiousModifyPaths = new Set([
        'src/core/',
        'src/harness/',
    ]);
    /** 策略结果历史 */
    ...
    async executePlan(plan, _checkpointId) {
        if (!this.authorityMeta) {
            Logger_1.Logger.error(`Authority gate BLOCKED at plan level: no authorityMeta for plan ${plan.id}`, new Error('E2-3: Self-modification plan without authority is forbidden'), 'SelfModificationEngine');
            return {
                planId: plan.id,
                success: false,
                executedActions: 0,
        ...
        if (this.authorityConsumed) {
            Logger_1.Logger.error(`Authority gate BLOCKED at plan level: stale authorization for plan ${plan.id}`, new Error('E2-3: Stale authority — one-shot authorization already used'), 'SelfModificationEngine');
            return {
                planId: plan.id,
                success: false,
                executedActions: 0,
    ...
    checkAuthorityGate(actionDescription) {
        if (!this.authorityMeta) {
            Logger_1.Logger.error(`Authority gate BLOCKED: no authorityMeta — "${actionDescription}"`, new Error('E2-3: Self-modification without authority is forbidden'), 'SelfModificationEngine');
            return false;
        }
        if (this.authorityConsumed) {
            Logger_1.Logger.error(`Authority gate BLOCKED: authorityMeta already consumed (stale authorization) — "${actionDescription}"`, new Error('E2-3: Stale authority — one-shot authorization already used'), 'SelfModificationEngine');
            return false;
        }
        const task = actionDescription;
        if (!(0, AuthoritySignature_1.verifyAuthorityMeta)(this.authorityMeta, task)) {
            Logger_1.Logger.error(`Authority gate BLOCKED: HMAC verification failed — "${actionDescription}"`, new Error('E2-3: authorityMeta signature invalid'), 'SelfModificationEngine');
            return false;
        }
        return true;
    }
