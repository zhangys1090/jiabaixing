...
exports.OptimizationAdvisor = exports.OptimizationType = exports.OptimizationLevel = void 0;
/** 异常类型枚举（原 AnomalyDetector 已删除，本地定义） */
var AnomalyType;
(function (AnomalyType) {
    AnomalyType["LATENCY_SPIKE"] = "LATENCY_SPIKE";
...
(function (OptimizationLevel) {
    ...
    OptimizationLevel["L3"] = "L3";
    OptimizationLevel["L4"] = "L4";
...
(function (OptimizationType) {
    OptimizationType["PERFORMANCE"] = "performance";
    OptimizationType["MEMORY"] = "memory";
    OptimizationType["USAGE_PATTERN"] = "usage_pattern";
    OptimizationType["TASK_OPTIMIZATION"] = "task_optimization";
    OptimizationType["SECURITY"] = "security";
...
class OptimizationAdvisor {
    ...
    generateOptimizationPlans(insights) {
        const plans = [];
        const performancePlans = this.analyzePerformanceAnomalies(insights.anomalies);
    ...
    analyzePerformanceAnomalies(anomalies) {
        const plans = [];
        const latencyAnomalies = anomalies.filter((a) => a.type === AnomalyType.LATENCY_SPIKE);
    ...
    analyzeMemoryAnomalies(anomalies) {
        const plans = [];
        const memoryAnomalies = anomalies.filter((a) => a.type === AnomalyType.MEMORY_LEAK);
    ...
    analyzeTaskOptimization(patterns) {
        const plans = [];
    ...
    analyzeSecurityOptimization(_insights) {
        const plans = [];
        plans.push({
            id: `sec-${Date.now()}-L1`,
            level: OptimizationLevel.L1,
    ...
    evaluatePlanEffectiveness(plan) {
    ...
    getPlanById(planId, plans) {
        return plans.find((plan) => plan.id === planId);
    }
    filterPlansByLevel(plans, level) {
        return plans.filter((plan) => plan.level === level);
    }
    filterPlansByType(plans, type) {
