...
exports.runSetupCLI = runSetupCLI;
const readline = __importStar(require("readline"));
const ProviderManager_1 = require("../models/ProviderManager");
const COLORS = {
    reset: '\x1b[0m',
    bold: '\x1b[1m',
    dim: '\x1b[2m',
...
/** 交互式添加 Provider */
async function addProviderInteractive(manager) {
...
/** 列出当前配置 */
function listProviders(manager) {
    ...
    if (providers.length === 0) {
        ...
        if (process.env.XIAOMI_API_KEY)
            envHints.push('小米 MiMo (XIAOMI_API_KEY)');
...
/** 测试所有 Provider 连接 */
async function testProviders(manager) {
    const providers = manager.getAll();
    if (providers.length === 0) {
    ...
    for (const p of providers) {
        ...
        try {
            ...
            console.log(c(COLORS.red, `❌ ${e.message}`));
            manager.updateHealth(p.name, false);
        }
    }
    console.log();
...
async function runSetupCLI(args) {
    const manager = (0, ProviderManager_1.getProviderManager)();
