...
function makeManager() {
    const dm = new DeviceManager_1.DeviceManager();
    dm.devices.set('dev-1', makeDevice());
    dm.deviceStatuses.set('dev-1', makeStatus());
    return dm;
}
describe('DeviceManager W3 遥测（TS 入口/透传）', () => {
    ...
    it('离线状态 online=false', () => {
        ...
        dm.deviceStatuses.set('dev-1', { ...makeStatus(), status: 'offline' });
    ...
    it('publishDeviceTelemetry 推送到桥并返回 ingested 数', async () => {
        ...
        dm.setTelemetryBridge(bridge);
        const n = await dm.publishDeviceTelemetry();
        expect(n).toBe(1);
        expect(bridge.postDeviceTelemetry).toHaveBeenCalledWith(expect.arrayContaining([expect.objectContaining({ device_id: 'dev-1' })]));
    });
