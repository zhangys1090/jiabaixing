...
exports.EvidenceDrivenRecoveryProposer = void 0;
exports.getEvidenceDrivenRecoveryProposer = getEvidenceDrivenRecoveryProposer;
exports.resetEvidenceDrivenRecoveryProposer = resetEvidenceDrivenRecoveryProposer;
...
class RecoveryMetrics {
    ...
    recordRecoveryAttempt(opts) {
        ...
        if (opts.autonomous)
            this.autonomousRecoveries++;
        if (opts.usedTaskSpecificRule)
            this.taskSpecificRuleUsed++;
        this.recoveryStepsSum += opts.steps;
        this.replansSum += opts.replans;
        if (opts.success && !opts.verifiedAtEnd)
            this.falseRecoveries++;
    }
    getSnapshot() {
...
class EvidenceDrivenRecoveryProposer {
    proposerId = 'evidence_driven_recovery_proposer';
    metrics = new RecoveryMetrics();
    strategiesAblated = false;
    ablateStrategies() {
        this.strategiesAblated = true;
        Logger_1.Logger.info('EDR: STRATEGIES ABLATED — all pattern-specific recovery handlers disabled, only generic reasoning allowed', 'EvidenceDrivenRecoveryProposer');
    }
    restoreStrategies() {
        this.strategiesAblated = false;
        Logger_1.Logger.info('EDR: STRATEGIES RESTORED — pattern-specific recovery handlers re-enabled', 'EvidenceDrivenRecoveryProposer');
    }
    isStrategiesAblated() {
    ...
    async propose(context) {
        ...
        if (evidenceLog.length === 0) {
            Logger_1.Logger.info(`EDR: no evidence yet — propose initial observation`, 'EvidenceDrivenRecoveryProposer');
            return this.proposeInitialObservation(goal, tempDir);
        }
        const patterns = this.analyzeFailureEvidence(evidenceLog);
        Logger_1.Logger.info(`EDR: detected ${patterns.length} failure pattern(s): ${patterns.map(p => p.type).join(', ')} ablated=${this.strategiesAblated}`, 'EvidenceDrivenRecoveryProposer');
        ...
        if (patterns.length === 0) {
            Logger_1.Logger.info(`EDR: no failure patterns — propose goal-directed action`, 'EvidenceDrivenRecoveryProposer');
    ...
    analyzeFailureEvidence(evidenceLog) {
        const patterns = [];
    ...
    proposeInitialObservation(goal, tempDir) {
        ...
        return [{
                ...
                action: {
                    type: 'tool_call',
                    payload: toolPayload('shell_exec', `const fs=require('fs'),p=require('path');const files=fs.readdirSync(TEMP_DIR);console.log('FILES:'+JSON.stringify(files));files.forEach(f=>{try{const stat=fs.statSync(p.join(TEMP_DIR,f));if(stat.isDirectory())console.log('DIR:'+f);else console.log('FILE:'+f+':'+fs.readFileSync(p.join(TEMP_DIR,f),'utf-8').slice(0,200))}catch(e){console.log('ERR:'+f+':'+e.message)}})`, 'Observe: list directory contents and read files to understand environment'),
                },
                confidence: 0.8,
    ...
    proposeGoalDirectedAction(goal, tempDir) {
        const goalDesc = (goal.description || '').toLowerCase();
        if (goalDesc.includes('write') && goalDesc.includes('result')) {
            return [{
                    ...
                    action: {
                        type: 'tool_call',
                        payload: toolPayload('shell_exec', `const fs=require('fs'),p=require('path');const goalDesc=${JSON.stringify(goal.description)};const dirMatch=goalDesc.match(/([\\w-]+)_dir/gi);if(dirMatch){const dirName=dirMatch[0];const dirPath=p.join(TEMP_DIR,dirName);if(!fs.existsSync(dirPath)){fs.mkdirSync(dirPath,{recursive:true});console.log('CREATED_DIR:'+dirPath)}const filePath=p.join(dirPath,'result.txt');fs.writeFileSync(filePath,'result','utf-8');console.log('WRITTEN:'+filePath)}else{const filePath=p.join(TEMP_DIR,'result.txt');fs.writeFileSync(filePath,'result','utf-8');console.log('WRITTEN:'+filePath)}`, 'Goal-directed: write result file based on goal description analysis'),
                    },
                    confidence: 0.7,
        ...
        if (goalDesc.includes('test') || goalDesc.includes('fix') || goalDesc.includes('pass')) {
            return [{
                    ...
                    action: {
                        type: 'tool_call',
                        payload: toolPayload('shell_exec', `const{execSync}=require('child_process'),fs=require('fs'),p=require('path');const files=fs.readdirSync(TEMP_DIR);const testFiles=files.filter(f=>f.includes('test')||f.includes('spec'));if(testFiles.length>0){try{const o=execSync('node '+testFiles[0],{cwd:TEMP_DIR,encoding:'utf-8',timeout:10000});console.log('PASS:'+o.trim())}catch(e){console.log('FAIL:'+e.message)}}else{console.log('NO_TEST_FOUND')}`, 'Goal-directed: run tests to verify current state'),
                    },
                    confidence: 0.7,
        ...
        return [{
                ...
                action: {
                    type: 'tool_call',
                    payload: toolPayload('shell_exec', `const fs=require('fs'),p=require('path');const files=fs.readdirSync(TEMP_DIR);console.log('FILES:'+JSON.stringify(files));files.forEach(f=>{try{const stat=fs.statSync(p.join(TEMP_DIR,f));if(stat.isDirectory())console.log('DIR:'+f);else console.log('FILE:'+f+':'+fs.readFileSync(p.join(TEMP_DIR,f),'utf-8').slice(0,200))}catch(e){console.log('ERR:'+f+':'+e.message)}})`, 'Goal-directed: observe environment'),
                },
                confidence: 0.5,
    ...
    proposeTestFailureRecovery(goal, tempDir, pattern) {
    ...
    proposeAssertionErrorRecovery(goal, tempDir, pattern) {
        ...
        if (numericDiff !== null && numericDiff !== 0) {
            return [{
                    ...
                    action: {
                        type: 'tool_call',
                        payload: toolPayload('shell_exec', `const fs=require('fs'),p=require('path'),{execSync}=require('child_process');const diff=${numericDiff};const files=fs.readdirSync(TEMP_DIR);const srcFiles=files.filter(f=>f.endsWith('.js')&&!f.includes('test')&&!f.includes('spec'));for(const src of srcFiles){const content=fs.readFileSync(p.join(TEMP_DIR,src),'utf-8');const lines=content.split('\\n');let fixed=false;for(let i=0;i<lines.length;i++){const line=lines[i];if(line.includes('return')){if(diff>0&&line.includes('+ '+String(diff))){lines[i]=line.replace('+ '+String(diff),'');fixed=true;break}if(diff<0&&line.includes('- '+String(Math.abs(diff)))){lines[i]=line.replace('- '+String(Math.abs(diff)),'');fixed=true;break}}}if(fixed){fs.writeFileSync(p.join(TEMP_DIR,src),lines.join('\\n'),'utf-8');const testFile=files.find(f=>f.includes('test')||f.includes('spec'));if(testFile){try{execSync('node '+testFile,{cwd:TEMP_DIR,encoding:'utf-8',timeout:10000});console.log('FIXED_AND_PASS')}catch(e){console.log('FIXED_BUT_STILL_FAIL:'+e.message)}}else{console.log('FIXED_NO_TEST')}return}}console.log('NO_DIFF_PATTERN_MATCH')`, `Recovery: assertion error diff=${numericDiff} — find and remove arithmetic offset in source`),
                    },
                    confidence: 0.75,
    ...
    proposeMissingResourceRecovery(goal, tempDir, pattern) {
        ...
        Logger_1.Logger.warn(`EDR: FAIL-CLOSED — extractMissingPathFromEvidence returned null. Evidence insufficient for safe recovery. Refusing goal-description fallback.`, 'EvidenceDrivenRecoveryProposer');
        return [{
                candidateId: cid(),
                proposerId: this.proposerId,
                action: {
                    type: 'tool_call',
                    payload: toolPayload('shell_exec', `console.log('NO_RECOVERY_EVIDENCE: missing resource path could not be extracted from failure evidence. Requesting additional observation.')`, 'FAIL-CLOSED: evidence insufficient — no recovery candidate, request observation'),
                },
                confidence: 0.1,
    ...
    proposeGenericExploration(goal, tempDir) {
        return [{
                candidateId: cid(),
                proposerId: this.proposerId,
                action: {
                    type: 'tool_call',
                    payload: toolPayload('shell_exec', `const fs=require('fs'),p=require('path');const files=fs.readdirSync(TEMP_DIR);console.log('FILES:'+JSON.stringify(files));files.forEach(f=>{try{const stat=fs.statSync(p.join(TEMP_DIR,f));if(stat.isDirectory()){const sub=fs.readdirSync(p.join(TEMP_DIR,f));console.log('DIR:'+f+':'+JSON.stringify(sub))}else{console.log('FILE:'+f+':'+fs.readFileSync(p.join(TEMP_DIR,f),'utf-8').slice(0,200))}}catch(e){console.log('ERR:'+f+':'+e.message)}})`, 'Generic exploration: observe all files and directories in environment'),
                },
                confidence: 0.4,
    ...
    extractNumericDiffFromEvidence(evidence) {
        const patterns = [
    ...
    extractMissingPathFromEvidence(evidence) {
        ...
        if (doesNotExistMatch)
            return doesNotExistMatch[1];
        const pathSlashExtMatch = evidence.match(/([\w-]+_dir\/[\w-]+\.\w+)/);
        if (pathSlashExtMatch)
            return pathSlashExtMatch[1];
        const dirSlashFileMatch = evidence.match(/([\w-]+\/[\w-]+\.\w+)/);
        if (dirSlashFileMatch)
            return dirSlashFileMatch[1];
        const notFoundPathMatch = evidence.match(/([\w-]+(?:[\/\\][\w-]+)*)\s+not found/i);
        if (notFoundPathMatch)
            return notFoundPathMatch[1];
        return null;
    }
}
exports.EvidenceDrivenRecoveryProposer = EvidenceDrivenRecoveryProposer;
let instance = null;
function getEvidenceDrivenRecoveryProposer() {
    if (!instance) {
        instance = new EvidenceDrivenRecoveryProposer();
    }
    return instance;
}
function resetEvidenceDrivenRecoveryProposer() {
    instance = null;
}
