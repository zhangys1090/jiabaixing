...
(0, deprecationWarning_1.emitDeprecationWarning)('PersistentMemoryService', 'MemoryAuthority (memoryType: persistent_hermes -> Python)', 'V6.0', 'PersistentMemoryService is a rogue store. Use MemoryAuthority.write({ memoryType: "persistent_hermes" }) instead.');
/**
 * Hermes风格持久化记忆服务
...
class PersistentMemoryService {
    ...
    fileSystem;
    /** 提示注入检测正则 */
    PROMPT_INJECTION_PATTERN = /(?:ignore|forget|override|disregard|skip)\s+(?:previous|all|above|prior|earlier)\s+(?:instructions?|rules?|prompts?|directives?|constraints?)/i;
    /** 凭证泄露检测正则 */
    CREDENTIAL_PATTERN = /(?:api[_\s-]?key|password|passwd|secret[_\s-]?key|access[_\s-]?token|private[_\s-]?key|ssh[_\s-]?key|auth[_\s-]?token|bearer)\s*[:=]\s*[\w\-+/=.]{8,}|-----BEGIN\s+(?:RSA\s+)?PRIVATE\s+KEY-----|sk-[a-zA-Z0-9]{12,}/i;
