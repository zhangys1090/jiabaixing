...
class KnowledgeGraphBuilder {
        ...
        try {
            ...
            for (const memory of memories) {
                ...
                for (const relation of relations) {
                    const edgeKey = relation.subject + '|' + relation.predicate + '|' + relation.object;
            ...
            Logger_1.default.info('知识图谱构建完成: ' +
                filteredNodes.length +
                '个节点, ' +
                filteredEdges.length +
                '条边, 耗时' +
                duration +
                'ms', 'KnowledgeGraphBuilder');
            return {
                nodes: filteredNodes,
                edges: filteredEdges,
            };
            ...
            Logger_1.default.error('构建知识图谱失败', error, 'KnowledgeGraphBuilder');
            return { nodes: [], edges: [] };
        }
    }
    /**
    ...
    identifyGaps(memories) {
        ...
        try {
            ...
            Logger_1.default.error('识别知识缺口失败', error, 'KnowledgeGraphBuilder');
        }
        return gaps;
    }
    /** 从文本提取实体 */
     ...
     * @returns 新增的关联数量
     */
    async proactiveKnowledgeEnrichment(memory, graph) {
        const text = this.memoryToText(memory);
        // 短文本不处理
        if (text.length < 3) {
            return 0;
        ...
        if (this.llmProvider) {
            try {
                ...
                Logger_1.default.warn('主动知识补充失败', 'KnowledgeGraphBuilder');
            }
        }
        return 0;
    }
     ...
     * 跨会话知识迁移 — 识别共享实体并补全关联
     */
    async migrateCrossSessionKnowledge(sessionMemories) {
        const result = {
            migratedAt: new Date(),
            migratedNodes: 0,
            newAssociations: 0,
        ...
        // 统计各实体在会话间的出现频率
        const entitySessionCount = new Map();
...
请返回 JSON 格式:
{
  "conclusions": [
    { "conclusion": "推理结论", "confidence": 0.8 }
                ...
                Logger_1.default.warn('跨会话知识推理失败', 'KnowledgeGraphBuilder');
            }
        }
        return result;
    }
