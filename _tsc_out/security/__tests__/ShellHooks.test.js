...
const ShellHooks_1 = require("../ShellHooks");
describe('ShellHooks - Shell命令钩子系统', () => {
    let hooks;
    beforeEach(() => {
        hooks = ShellHooks_1.ShellHooks.getInstance();
        hooks['hooks'] = [];
        hooks['executionLog'] = [];
    });
    afterEach(() => {
    ...
    describe('前置钩子执行', () => {
        ...
        test('任一钩子拦截时应返回proceed=false', async () => {
            ...
            hooks.register('block', 'pre', () => ({ proceed: false, reason: '危险命令' }), 20);
            const context = {
                command: 'rm -rf /',
                backend: 'local',
                timestamp: Date.now(),
        ...
        test('被禁用的钩子不应执行', async () => {
            hooks.register('blocker', 'pre', () => ({ proceed: false, reason: 'blocked' }), 10);
    ...
    describe('内置钩子', () => {
        test('registerBuiltinShellHooks 应注册内置钩子', () => {
            (0, ShellHooks_1.registerBuiltinShellHooks)();
            const registered = hooks.getRegisteredHooks();
