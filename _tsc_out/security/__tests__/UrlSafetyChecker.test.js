...
const UrlSafetyChecker_1 = require("../UrlSafetyChecker");
describe('UrlSafetyChecker - URL安全检查器', () => {
    let checker;
    beforeEach(() => {
        checker = UrlSafetyChecker_1.UrlSafetyChecker.getInstance();
    ...
    describe('白名单机制', () => {
        test('白名单中的URL应该直接放行', () => {
            const result = checker.check('https://api.openai.com/v1/chat/completions');
            expect(result.safe).toBe(true);
