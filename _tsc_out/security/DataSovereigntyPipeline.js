...
class DataSovereigntyPipeline {
    auditDb = null;
    dbPath;
    static MAX_AUDIT_RECORDS = 50000;
    ...
    initialize() {
        try {
            ...
            try {
                ...
                Logger_1.Logger.debug(`数据主权审计 WAL 模式设置跳过: ${pragmaErr.message}`, 'DataSovereigntyPipeline');
            }
            this.auditDb.exec(`
    ...
    generateReport() {
        ...
        const encryptedCount = this.auditDb
            .prepare("SELECT COUNT(*) as count FROM data_access_audit WHERE operation = 'encrypt' OR purpose LIKE '%encrypt%'")
            .get().count;
        const writeCount = this.auditDb
            .prepare("SELECT COUNT(*) as count FROM data_access_audit WHERE operation IN ('write', 'delete', 'export')")
            .get().count;
