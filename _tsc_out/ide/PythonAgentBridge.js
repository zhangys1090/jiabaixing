...
class PythonAgentBridge {
    client;
    ws = null; // EventBus WS
    chatWs = null; // Chat streaming WS
    eventHandlers = new Map();
    healthy = false;
    tsEventBusForward = null;
    // WS 流式聊天请求管理
    _pendingRequests = new Map();
    _requestIdCounter = 0;
    _chatReconnectTimer = null;
    _chatReconnectDelay = 1000; // 初始 1s，指数退避
    _eventReconnectDelay = 1000; // EventBus 重连初始 1s
    _disconnected = false; // 标记已主动断开，阻止重连
    _wsTimeout; // WS 流式聊天超时(ms)，可通过环境变量 AGENT_WS_TIMEOUT_SEC 配置
    constructor(config) {
    ...
    async _processInputViaHttp(message, sessionId, traceId, images) {
        ...
        try {
            ...
            if (finishReason === 'budget_exceeded' && !rawContent.trim()) {
                ...
                Logger_1.Logger.warn('Python后端返回budget_exceeded，使用降级提示', 'PythonAgentBridge');
            }
            return {
                response: responseText,
                traceId: data.trace_id ?? traceId,
                intent: data.intent,
                qualityScore: typeof data.quality_score === 'number'
                    ? data.quality_score
                    : undefined,
                toolCallsMade: typeof data.tool_calls_made === 'number'
    ...
    _handleChatEvent(event) {
        ...
        const requestId = eventAny.request_id;
        if (requestId) {
            ...
            if (pending) {
                if (pending.onStream) {
                    ...
                    if (event.type === 'stream_chunk' && event.content) {
                        ...
                        const wsContent = event.content || pending.contentBuffer.join('') || '';
                        const wsFinishReason = eventAny.finish_reason ||
                            _meta.finish_reason ||
                            undefined;
                        let wsResponseText = wsContent || '抱歉，后端未返回有效内容，请稍后重试。';
                        if (wsFinishReason === 'budget_exceeded' && !wsContent.trim()) {
                            wsResponseText =
                                '抱歉，当前AI服务预算已达上限，暂时无法处理更多请求。请稍后重试。';
                            Logger_1.Logger.warn('WS通道返回budget_exceeded，使用降级提示', 'PythonAgentBridge');
                        }
                        pending.resolve({
                            response: wsResponseText,
                            traceId: event.trace_id,
                            qualityScore: typeof eventAny.quality_score === 'number'
                                ? eventAny.quality_score
                                : typeof _meta.quality_score === 'number'
                                    ? _meta.quality_score
                                    : undefined,
                            toolCallsMade: typeof eventAny.tool_calls_made === 'number'
                                ? eventAny.tool_calls_made
                                : typeof _meta.tool_calls_made === 'number'
                                    ? _meta.tool_calls_made
                                    : undefined,
                            roundsUsed: typeof eventAny.rounds_used === 'number'
                                ? eventAny.rounds_used
                                : typeof _meta.rounds_used === 'number'
                                    ? _meta.rounds_used
                                    : undefined,
                            duration: typeof eventAny.duration === 'number'
                                ? eventAny.duration
                                : typeof _meta.duration === 'number'
                                    ? _meta.duration
                                    : undefined,
                            finishReason: eventAny.finish_reason ||
                                _meta.finish_reason ||
                                undefined,
                        });
                        this._pendingRequests.delete(requestId);
