...
const cognitionForwarder_1 = require("../harness/cognition/cognitionForwarder");
...
async function startIpcServer(core) {
    ...
    return new Promise((resolve) => {
        ...
        server.on('error', (err) => {
            Logger_1.Logger.error(`IPC 服务器错误: ${err.message}`, err, 'IPC');
            if (!settled) {
                settled = true;
                try {
                    server.close();
                ...
                ipcServer = null;
                Logger_1.Logger.warn('IPC 服务器启动失败，跳过 IPC（不影响主服务）', 'IPC');
                resolve();
            }
        });
...
function printBanner() {
    Logger_1.Logger.info('jiabaixing v5.0', 'Bootstrap');
}
...
function bootOk(msg) {
    ...
    if (msg)
        Logger_1.Logger.info(msg, 'Bootstrap');
}
async function healthCheckWithRetry(bridge, maxRetries = 10, intervalMs = 2000) {
    for (let i = 0; i <= maxRetries; i++) {
        if (await bridge.healthCheck())
            return true;
        if (i < maxRetries) {
            Logger_1.Logger.info(`Python Agent 未就绪，${intervalMs / 1000}s 后重试 (${i + 1}/${maxRetries})`, 'Bootstrap');
...
async function bootstrap() {
    Logger_1.Logger.info('jiabaixing v5.0 启动', 'Bootstrap');
    let core;
    try {
        bootStep('核心引擎');
        ...
        if (usePythonBackend) {
            ...
            (0, cognitionForwarder_1.registerCognitionForwarder)();
            ...
            if (pyHealthy) {
                ...
                try {
                    const { MemoryAuthority } = await Promise.resolve().then(() => __importStar(require('../authority/MemoryAuthority')));
                    const memoryAuth = MemoryAuthority.getInstance();
                    memoryAuth.registerBridge(async (req) => {
                        ...
                        switch (req.memoryType) {
                            ...
                            case 'feedback':
                                await bridge.memoryStoreFeedback({
                                    ...
                                    toolName: req.metadata?.category ?? 'general',
                                });
                                break;
                            default:
                                memoryId = await bridge.memoryStoreShortTerm(req.content, req.scene, req.emotion);
                        }
                        return { success: true, memoryId, operationId: '', source: 'python' };
                    }, async (req) => {
                        const bridge = (0, bridgeRegistry_1.getActivePythonBridge)();
                        if (!bridge) {
                        ...
                        return {
                            items: results.map((r) => ({
                                id: r.id,
                                content: r.content,
                                memoryType: (req.memoryType ?? 'short_term'),
                    ...
                    Logger_1.Logger.info('E3-1: MemoryAuthority Python bridge registered from bootstrap', 'Bootstrap');
                }
                catch (bridgeRegErr) {
                    Logger_1.Logger.warn(`E3-1: MemoryAuthority bridge registration failed — ${bridgeRegErr.message}`, 'Bootstrap');
                }
        ...
        Logger_1.Logger.info(`后端决策锁定: ${pythonBackendLive ? 'Python' : 'TS本地'}`, 'Bootstrap');
