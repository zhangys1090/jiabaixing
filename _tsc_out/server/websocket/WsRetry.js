...
exports.emitRetryEvent = emitRetryEvent;
...
async function withRetry(operation, options = {}) {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        if (state.isAborted) {
            Logger_1.Logger.info('🛑 任务已取消，停止重试', 'WsRetry');
            break;
        }
        try {
            const result = await operation();
            if (attempt > 0) {
                Logger_1.Logger.info(`✅ 重试成功 (attempt=${attempt})`, 'WsRetry');
                options.onSuccess?.();
            }
            return result;
        }
            ...
            const canRetry = isRetryableError(state.lastError) && attempt < maxRetries;
            Logger_1.Logger.warn(`❌ 处理失败 (attempt=${attempt}/${maxRetries}, retryable=${canRetry}): ${state.lastError.message}`, 'WsRetry');
            if (!canRetry) {
 ...
 * 发送重试事件到 EventBus
 */
function emitRetryEvent(traceId, attempt) {
