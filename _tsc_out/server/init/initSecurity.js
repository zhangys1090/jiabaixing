...
async function initSecurity() {
    const { NetworkGuard } = await Promise.resolve().then(() => __importStar(require('../../security/NetworkGuard')));
    NetworkGuard.install();
    const { DataSovereigntyPipeline } = await Promise.resolve().then(() => __importStar(require('../../security/DataSovereigntyPipeline')));
    const sovereigntyPipeline = new DataSovereigntyPipeline();
    sovereigntyPipeline.initialize();
    const { SecurityCore } = await Promise.resolve().then(() => __importStar(require('../../security/SecurityCore')));
    ...
    Logger.info(`🛡️ 安全核心已初始化: ${JSON.stringify(health.details)}`, 'initSecurity');
