...
async function initMemory(core, sovereigntyPipeline) {
    const memoryEngine = new MemoryEngine_1.MemoryEngine();
    await memoryEngine.initialize();
    core.setMemoryEngine(memoryEngine);
    const { UnifiedContextPipeline } = await Promise.resolve().then(() => __importStar(require('../../core/UnifiedContextPipeline')));
