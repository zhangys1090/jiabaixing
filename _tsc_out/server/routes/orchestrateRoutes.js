...
exports.setOrchestrateCore = setOrchestrateCore;
...
const AgentRegistry_1 = require("../../harness/orchestration/AgentRegistry");
const OrchestratorAgent_1 = require("../../harness/orchestration/OrchestratorAgent");
...
function trimRateMap() {
    ...
    if (_orchestrateRateMap.size > RATE_MAP_MAX_ENTRIES) {
        ...
        const toRemove = keys.slice(0, _orchestrateRateMap.size - RATE_MAP_MAX_ENTRIES);
...
function setOrchestrateCore(core) {
    _core = core;
}
function getSharedRegistry() {
    if (!_sharedRegistry) {
        _sharedRegistry = new AgentRegistry_1.AgentRegistry();
...
function checkOrchestrateRate(clientIp) {
    const now = Date.now();
...
router.post('/orchestrate', async (req, res) => {
    try {
        ...
        Logger_1.Logger.info(`[Orchestrate] 收到编排请求: ${safeGoal.substring(0, 60)}...`, 'OrchestrateRoute');
        const registry = getSharedRegistry();
        const orchestrator = new OrchestratorAgent_1.OrchestratorAgent({
            registry,
            llm: {
                decomposeGoal: async (userGoal, ctx) => {
                    const result = await getCore().processInput(`你是家百星的任务编排模块。请将以下目标拆解为子任务列表。
...
- 子任务数量不超过10个`, 'orchestrator');
            ...
            executor: async (task) => {
                Logger_1.Logger.info(`[Orchestrate] 执行子任务: ${task.id} — ${task.goal.substring(0, 60)}`, 'OrchestrateRoute');
        ...
        const result = await Promise.race([
            ...
            new Promise((_, reject) => setTimeout(() => reject(new Error(`编排执行超时 (${ORCHESTRATE_TIMEOUT_MS / 1000}秒)`)), ORCHESTRATE_TIMEOUT_MS)),
        ]);
        ...
        Logger_1.Logger.error('[Orchestrate] 编排失败', error, 'OrchestrateRoute');
...
router.post('/evaluate', async (req, res) => {
    try {
        ...
        Logger_1.Logger.info('[Evaluate] 收到评估请求', 'EvaluateRoute');
        ...
        const stepEvaluator = new StepEvaluator_1.StepEvaluator();
        const stepResults = steps.map((s) => stepEvaluator.evaluateStep({
            stepId: s.stepId,
            toolName: s.toolName,
            args: s.args,
            result: s.result,
        ...
        // 质量评分
        const scorer = new QualityScorer_1.QualityScorer();
        ...
        res.json({
            success: true,
            score: qualityScore,
            report: `综合评分: ${qualityScore.overall}/100\n` +
        ...
        Logger_1.Logger.error('[Evaluate] 评估失败', error, 'EvaluateRoute');
        res.status(500).json({ success: false, error: error.message });
    }
});
