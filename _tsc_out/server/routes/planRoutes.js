...
function registerPlanRoutes(app) {
    app.post('/api/plan', express_1.default.json({ limit: '1mb' }), async (req, res) => {
        try {
            ...
            Logger_1.Logger.error('任务规划失败', error, 'PlanRoutes');
            res.status(500).json({ success: false, error: error.message });
        }
    });
    app.post('/api/plan/execute', express_1.default.json({ limit: '1mb' }), async (req, res) => {
        try {
            const body = req.body;
            ...
            Logger_1.Logger.error('执行计划失败', error, 'PlanRoutes');
            res
                .status(500)
                .json({ success: false, error: error.message });
        }
    });
    app.post('/api/plan/evaluate', express_1.default.json({ limit: '1mb' }), async (req, res) => {
        try {
            const body = req.body;
            ...
            Logger_1.Logger.error('评估计划失败', error, 'PlanRoutes');
            res
                .status(500)
                .json({ success: false, error: error.message });
        }
    });
    app.post('/api/plan/reflect', express_1.default.json({ limit: '1mb' }), async (req, res) => {
        try {
            const body = req.body;
            if (!body || !body.task || typeof body.task !== 'string') {
                return res
            ...
            Logger_1.Logger.error('反思计划失败', error, 'PlanRoutes');
            res
                .status(500)
                .json({ success: false, error: error.message });
        }
