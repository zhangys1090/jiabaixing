...
function registerCoreRoutes(app, core) {
    ...
    app.get('/api/models/status', async (_req, res) => {
        try {
            const { getProviderManager } = await Promise.resolve().then(() => __importStar(require('../../models/ProviderManager')));
    ...
    app.get('/api/models/health', async (_req, res) => {
        try {
            ...
            const modelHealthList = models.map((model) => {
                ...
                const successRate = totalCalls > 0 ? (health.successCount / totalCalls) * 100 : 100;
                return {
                    modelId: model.id,
    ...
    app.post('/api/models/switch', express_1.default.json({ limit: '1mb' }), async (req, res) => {
        try {
            if (!requireAdmin(req, res))
                return;
            const { targetModel, reason } = req.body;
            if (!targetModel) {
            ...
            Logger_1.Logger.info(`模型已切换为: ${targetModel}`, 'Models', {
    ...
    app.post('/api/process', express_1.default.json({ limit: '50mb' }), async (req, res) => {
        ...
        try {
            const { input: rawInput, images } = req.body;
            ...
            const processedInput = input.length > MAX_INPUT_LENGTH
                ? input.substring(0, MAX_INPUT_LENGTH) + '...[内容已截断]'
                : input;
            const traceId = Logger_1.Logger.generateTraceId();
            if ((0, bootstrap_1.isPythonBackend)()) {
                const bridge = (0, bootstrap_1.getPythonBridge)();
                const result = await bridge.processInput(processedInput, userId, traceId, Array.isArray(images) && images.length > 0
                    ? images.map((url) => ({ url }))
                    : undefined);
                res.json({
                    success: true,
            ...
            const result = await core.processInput(processedInput, userId, traceId, Array.isArray(images) && images.length > 0
                ? images.map((url) => ({ url }))
                : undefined);
            res.json({
                success: true,
            ...
            Logger_1.Logger.error('⚠️ /api/process 处理失败', error, 'Main');
            res.status(500).json({
                error: '处理失败，请稍后重试。',
            });
        }
    ...
    app.post('/api/upload', (req, res) => {
        ...
        bb.on('file', (_fieldname, file, info) => {
            ...
            file.on('end', async () => {
                try {
                    ...
                    Logger_1.Logger.error(`写入 ${filename} 失败`, err, 'coreRoutes');
                }
            });
        });
        bb.on('finish', () => {
            if (uploadedFiles.length === 0) {
                const extra = rejectedFiles.length > 0
        ...
        bb.on('error', (err) => {
            Logger_1.Logger.error('busboy 上传错误', err, 'coreRoutes');
            if (!res.headersSent) {
    ...
    app.get('/api/upload/history', async (_req, res) => {
        ...
        try {
            ...
            const versions = await Promise.all(files.map(async (f) => {
                try {
                    const content = await fs.promises.readFile(path.join(evolutionDir, f), 'utf-8');
    ...
    app.post('/api/correct', express_1.default.json({ limit: '1mb' }), (req, res) => {
        try {
            const { toolId, tool_name, correctionType, type, reason, message, severity, traceId, } = req.body;
            if (!toolId && !tool_name) {
                return res.status(400).json({ error: '缺少 toolId 或 tool_name' });
            }
            void EventBus_1.EventBus.emit('user_correction', {
                ...
                traceId: traceId || Logger_1.Logger.generateTraceId(),
            });
            res.json({
                success: true,
            ...
            Logger_1.Logger.error('❌ 处理纠正请求失败', error, 'API');
            res.status(500).json({
                error: '内部服务错误',
    ...
    app.get('/api/logs/stream', (req, res) => {
        ...
        Logger_1.Logger.on('log', logHandler);
        req.on('close', () => {
            Logger_1.Logger.off('log', logHandler);
            res.end();
        });
        Logger_1.Logger.info('📝 新的日志流客户端已连接', 'SSE');
    });
    // ── Desktop 面板 API ──
    app.post('/api/desktop/screenshot', async (_req, res) => {
        try {
            ...
            Logger_1.Logger.error('❌ 截图失败', error, 'DesktopAPI');
            res.json({ success: false, error: error.message });
        }
    });
    app.post('/api/desktop/automate', express_1.default.json({ limit: '1mb' }), async (req, res) => {
        try {
            const { task, authority } = req.body;
            if (!task)
                return res.json({ success: false, error: '缺少 task 参数' });
            const harness = core?.getHarness();
            if (!harness)
            ...
            if (!registry)
                return res.json({ success: false, error: '工具注册表不可用' });
            const executeParams = { task };
            if (authority) {
                executeParams._authority_meta = authority;
            }
            const result = await registry.execute('desktop_automate', executeParams, {
                userId: 'api',
    ...
    app.post('/api/audio/upload', (req, res) => {
        ...
        bb.on('file', (_fieldname, file, info) => {
            ...
            audioName = filename;
            audioMime = mimeType || '';
            if (!AUDIO_MIME_WHITELIST.has(audioMime) &&
        ...
        bb.on('finish', async () => {
            ...
            try {
                ...
                Logger_1.Logger.error('音频处理失败', error, 'coreRoutes');
                res.status(500).json({
                    success: false,
        ...
        bb.on('error', (err) => {
            Logger_1.Logger.error('音频上传错误', err, 'coreRoutes');
            if (!res.headersSent) {
                res.status(500).json({ error: `上传失败: ${err.message}` });
            }
        });
    ...
    app.get('/api/health/slo', async (_req, res) => {
        try {
            ...
            Logger_1.Logger.error('SLO 健康检查失败', error, 'coreRoutes');
