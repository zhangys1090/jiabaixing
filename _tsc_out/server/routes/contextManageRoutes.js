...
exports.registerContextManageRoutes = registerContextManageRoutes;
...
<!-- 列出需要特别注意的事项 -->
`;
function registerContextManageRoutes(app, core) {
    /**
     * 列出已加载的上下文文件
    ...
    app.get('/api/context/list', async (req, res) => {
        try {
            ...
            Logger_1.Logger.error('❌ 获取上下文文件列表失败', error, 'ContextAPI');
     ...
     */
    app.post('/api/context/refresh', express_1.default.json(), async (req, res) => {
        try {
            if (!core) {
                return res
                    .status(503)
            ...
            Logger_1.Logger.info(`📄 上下文文件缓存已刷新: ${count} 个文件`, 'ContextAPI');
            res.json({
                success: true,
                data: {
                    count,
            ...
            Logger_1.Logger.error('❌ 刷新上下文文件失败', error, 'ContextAPI');
            res.status(500).json({ success: false, error: error.message });
        }
    });
    /**
     ...
     */
    app.post('/api/context/load', express_1.default.json(), async (req, res) => {
        try {
            ...
            Logger_1.Logger.info(`📄 手动加载上下文文件: ${loadedFiles.length} 个`, 'ContextAPI');
            res.json({
                success: true,
                data: {
                    files: loadedFiles,
            ...
            Logger_1.Logger.error('❌ 加载上下文文件失败', error, 'ContextAPI');
            res.status(500).json({ success: false, error: error.message });
        }
    });
    /**
     ...
     */
    app.post('/api/context/create', express_1.default.json(), async (req, res) => {
        try {
            const { fileName = 'JIABAIXING.md' } = req.body;
            // 验证文件名是否在允许列表中
            ...
            Logger_1.Logger.info(`📄 创建上下文文件模板: ${fileName}`, 'ContextAPI');
            res.json({
                success: true,
                data: {
                    fileName,
            ...
            Logger_1.Logger.error('❌ 创建上下文文件失败', error, 'ContextAPI');
            res.status(500).json({ success: false, error: error.message });
        }
    });
    /**
    ...
    app.get('/api/context/read/:fileName', async (req, res) => {
        try {
            ...
            Logger_1.Logger.error('❌ 读取上下文文件失败', error, 'ContextAPI');
            res.status(500).json({ success: false, error: error.message });
        }
    });
}
