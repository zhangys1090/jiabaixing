...
function registerMCPRoutes(app) {
    app.get('/api/mcp/servers', async (_req, res) => {
        try {
            ...
            Logger_1.Logger.error('获取MCP服务器状态失败', error, 'MCPRoutes');
    ...
    app.post('/api/mcp/servers/:name/start', async (req, res) => {
        try {
            ...
            Logger_1.Logger.error('启动MCP服务器失败', error, 'MCPRoutes');
            res.status(500).json({ success: false, error: error.message });
        }
    });
    app.post('/api/mcp/servers/:name/stop', async (req, res) => {
        try {
            ...
            Logger_1.Logger.error('停止MCP服务器失败', error, 'MCPRoutes');
            res.status(500).json({ success: false, error: error.message });
        }
    });
    app.post('/api/mcp/servers/start-all', async (_req, res) => {
        try {
            ...
            Logger_1.Logger.error('启动所有MCP服务器失败', error, 'MCPRoutes');
            res.status(500).json({ success: false, error: error.message });
        }
    });
    app.get('/api/mcp/servers/:name/tools', async (req, res) => {
        try {
            ...
            Logger_1.Logger.error('获取MCP工具列表失败', error, 'MCPRoutes');
            res.status(500).json({ success: false, error: error.message });
        }
    });
    app.post('/api/mcp/servers/:name/call', express_1.default.json({ limit: '1mb' }), async (req, res) => {
        try {
            const { name } = req.params;
            ...
            Logger_1.Logger.error('MCP工具调用失败', error, 'MCPRoutes');
            res
                .status(500)
                .json({ success: false, error: error.message });
        }
    });
    app.post('/api/mcp/servers/:name/message', express_1.default.json({ limit: '1mb' }), async (req, res) => {
        try {
            const { name } = req.params;
            ...
            Logger_1.Logger.error('MCP消息发送失败', error, 'MCPRoutes');
            res
                .status(500)
                .json({ success: false, error: error.message });
        }
    });
    app.post('/api/mcp/register', express_1.default.json({ limit: '1mb' }), async (req, res) => {
        try {
            ...
            Logger_1.Logger.error('注册MCP服务器失败', error, 'MCPRoutes');
            res
                .status(500)
                .json({ success: false, error: error.message });
        }
