...
exports.createOpenAICompatibleRouter = createOpenAICompatibleRouter;
...
function validateApiKey(req) {
    ...
    return crypto_1.default.timingSafeEqual(Buffer.from(token), Buffer.from(API_KEY));
}
 ...
 */
function registerOpenAIRoutes(app, core) {
    ...
    app.post('/v1/chat/completions/tools', async (req, res) => {
        ...
        try {
            ...
            Logger_1.Logger.error('Function Calling 请求失败', err, 'OpenAICompat');
            return res.status(500).json({
 ...
 */
function createOpenAICompatibleRouter(options) {
 ...
 */
function parseToolCallsFromResponse(response, tools) {
    const toolCalls = [];
    ...
    if (toolCalls.length === 0) {
        for (const toolName of toolNames) {
            const callRegex = new RegExp(`(?:调用|call|use)\\s+${toolName}\\s*\\(([^)]*)\\)`, 'i');
