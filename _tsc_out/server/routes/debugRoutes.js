...
exports.registerDebugRoutes = registerDebugRoutes;
const express_1 = __importDefault(require("express"));
const EventBus_1 = require("../../shared/EventBus");
const Logger_1 = require("../../utils/Logger");
const DEBUG_ENABLED = process.env.JBX_DEBUG_ROUTES === '1' ||
    process.env.NODE_ENV === 'development';
function debugGuard(_req, res) {
    if (!DEBUG_ENABLED) {
...
function registerDebugRoutes(app, core, broadcast) {
    app.get('/api/debug/weights', (_req, res) => {
        if (!debugGuard(_req, res))
            return;
        try {
            if (!core) {
    ...
    app.get('/api/debug/tool-usage', (_req, res) => {
        ...
        try {
            ...
            const toolExecutor = core.toolExecutor;
            if (!toolExecutor) {
    ...
    app.post('/api/simulate_task', express_1.default.json({ limit: '1mb' }), (req, res) => {
        if (!debugGuard(req, res))
            return;
        try {
            ...
            const emitPhase = (phase, status, result) => {
                ...
                try {
                    void EventBus_1.EventBus.emit('agent_execution_update', payload);
                }
                catch {
            ...
            setTimeout(() => emitPhase('execute', 'completed', { message: '执行完成' }), 1800);
            ...
            setTimeout(() => emitPhase('output', 'completed', {
            ...
            Logger_1.Logger.error('❌ /api/simulate_task 触发失败', error, 'Main');
