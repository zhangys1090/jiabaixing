...
class DesktopMCPServer extends events_1.EventEmitter {
    ...
    async callTool(name, args) {
        ...
        try {
            ...
            this.emit('tool_error', { name, error: error.message });
            return errorResult;
        }
    }
    ...
    async handleGetWindows(_args) {
        ...
        const action = {
            type: 'shell',
            params: {
                command: 'Get-Process | Where-Object { $_.MainWindowTitle } | Select-Object MainWindowTitle | ConvertTo-Json',
            },
            description: '获取窗口列表',
        };
        const { result } = await this.authority.executeAction(action);
...
// 便捷导出
exports.desktopMCPServer = DesktopMCPServer.getInstance();
