...
class SlashCommandRegistry {
    ...
    register(cmd) {
        ...
        if (BUILT_IN_COMMANDS.has(name) && !cmd.name.startsWith('__internal__')) {
            Logger_1.Logger.warn(`🚫 不能覆盖内置命令: /${name}`, 'SlashCommandRegistry');
            return false;
        }
        if (this.commands.has(name)) {
            Logger_1.Logger.warn(`⚠️ 命令 /${name} 已存在，将被覆盖`, 'SlashCommandRegistry');
        }
        this.commands.set(name, cmd);
        Logger_1.Logger.debug(`📋 斜杠命令已注册: /${name} — ${cmd.description}`, 'SlashCommandRegistry');
        return true;
    }
    /** 解析消息是否为斜杠命令 */
    parse(input) {
    ...
    async execute(input, ctx) {
        ...
        try {
            ...
            Logger_1.Logger.error(`❌ 命令 /${name} 执行失败`, err, 'SlashCommandRegistry');
            return {
                handled: true,
                response: `❌ 命令 /${name} 执行出错: ${err.message}`,
            };
        }
    }
    /** 获取所有可见命令列表 */
    ...
    registerBuiltins() {
        ...
        this.commands.set('new', {
            ...
            handler: async (_args, ctx) => {
                try {
                    // 通过 EventBus 发送新会话信号
                    const { EventBus } = await Promise.resolve().then(() => __importStar(require('../shared/EventBus')));
                    EventBus.emit('session_reset', {
        ...
        this.commands.set('stop', {
            ...
            handler: async (_args, ctx) => {
                try {
                    const { EventBus } = await Promise.resolve().then(() => __importStar(require('../shared/EventBus')));
                    EventBus.emit('agent_stop_requested', {
                        traceId: ctx.userId,
        ...
        this.commands.set('compress', {
            ...
            handler: async (_args, ctx) => {
                try {
                    const { EventBus } = await Promise.resolve().then(() => __importStar(require('../shared/EventBus')));
                    EventBus.emit('compress_requested', {
        ...
        this.commands.set('usage', {
            ...
            handler: async (_args, ctx) => {
                ...
                // 尝试获取缓存统计
                try {
                    const { IntegrationManager } = await Promise.resolve().then(() => __importStar(require('./IntegrationManager')));
        ...
        this.commands.set('sethome', {
            ...
            handler: async (_args, ctx) => {
                try {
                    const { GatewaySessionStore } = await Promise.resolve().then(() => __importStar(require('./GatewaySessionStore')));
                    const store = new GatewaySessionStore();
                    store.saveChatSession('home_channel', ctx.platform, JSON.stringify({
                        chatId: ctx.userId,
                        setAt: Date.now(),
                    }));
        ...
        this.commands.set('title', {
            ...
            handler: async (args, ctx) => {
                ...
                try {
                    const { EventBus } = await Promise.resolve().then(() => __importStar(require('../shared/EventBus')));
                    EventBus.emit('session_title_set', {
                        title: name,
                        sessionId: ctx.userId,
                    });
                    return `📋 会话已命名为: "${name}"`;
        ...
        this.commands.set('cron', {
            ...
            handler: async (args, ctx) => {
                ...
                if (subcmd === 'list') {
                    try {
                        const { EventBus } = await Promise.resolve().then(() => __importStar(require('../shared/EventBus')));
                        EventBus.emit('cron_list_requested', {
                            userId: ctx.userId,
                        });
                ...
                if (subcmd === 'run') {
                    ...
                    if (!taskId)
                        return '❌ 用法: /cron run <task_id>';
                    try {
                        const { EventBus } = await Promise.resolve().then(() => __importStar(require('../shared/EventBus')));
                        EventBus.emit('cron_run_requested', {
                            taskId,
                            userId: ctx.userId,
                        });
                ...
                if (subcmd === 'pause') {
                    const taskId = parts[1];
                    if (!taskId)
                        return '❌ 用法: /cron pause <task_id>';
                    try {
                        const { EventBus } = await Promise.resolve().then(() => __importStar(require('../shared/EventBus')));
                        EventBus.emit('cron_pause_requested', {
                            taskId,
                            userId: ctx.userId,
                        });
            ...
            handler: async (args, _ctx) => {
                ...
                if (subcmd === 'list') {
                    try {
                        ...
                        if (routes.length === 0)
                            return '🔌 暂无已注册的 Webhook 路由。';
                        const lines = ['🔌 **Webhook 路由**', ''];
                        for (const r of routes) {
                            lines.push(`  • ${r.name} — 事件: ${r.events.join(', ')}${r.url ? ' → ' + r.url : ''}`);
                        }
                        return lines.join('\n');
                    }
                    catch {
                ...
                setImmediate(async () => {
                    try {
                        ...
                        try {
                            ...
                            await im.sendMessage({
                                platform: ctx.platform,
                                message: `❌ **Background task failed**: ${err.message}`,
                                to: ctx.userId,
                            });
                        }
                        catch {
        ...
        this.commands.set('pair', {
            ...
            handler: async (_args, ctx) => {
                try {
                    const { GatewayPairing } = await Promise.resolve().then(() => __importStar(require('./GatewayPairing')));
