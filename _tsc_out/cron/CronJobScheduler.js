...
class CronPromptInjectionScanner {
    ...
    static ALLOWED_COMMAND_PREFIXES = [
        'node ',
        'npx ',
        'npm ',
        'python ',
...
class CronJobScheduler {
    ...
    persistence;
    running = false;
    tickInterval = 60_000; // 默认每 60 秒 tick 一次
    constructor() {
    ...
    async runJob(job) {
        ...
        Logger_1.Logger.warn(`[D7-P0] CronJob "${job.name}" BLOCKED — no DecisionAuthority/ActionAuthority in cron path. Emitting scheduled_action_pending event.`);
        ...
        try {
            EventBus_1.default.emit('scheduled_action_pending', {
