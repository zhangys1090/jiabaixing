...
class ChatProvider {
    ...
    async executeWithRetry(operation, operationName, maxRetries = this.maxRetries) {
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                const isRateLimitError = ChatProvider.RATE_LIMIT_ERRORS.some((e) => errorMsg.includes(e));
                // 连接错误或其他错误：按指数退避重试
                if (attempt < maxRetries) {
                    ...
                    Logger_1.Logger.warn(`${operationName} 第${attempt}次失败${isRateLimitError ? '（速率限制）' : ''}，${delay}ms后重试: ${lastError.message}`, 'ChatProvider');
    ...
    async chat(message, history = [], systemPromptOverride) {
        ...
        const operation = async () => {
            ...
            if (!response.text) {
                Logger_1.Logger.warn('⚠️ LLM返回空内容，使用降级回复', 'ChatProvider');
    ...
    async chatWithTools(messages, tools, maxTokens = 4096, toolChoice = 'auto') {
        ...
        const tryGenerate = async () => {
            ...
            if (!response.text && !response.toolCalls?.length) {
                Logger_1.Logger.warn('⚠️ LLM工具聊天返回空内容且无tool_calls', 'ChatProvider');
            }
            return {
