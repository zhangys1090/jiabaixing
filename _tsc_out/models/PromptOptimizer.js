...
class PromptOptimizer {
    static optimizePrompt(prompt, maxLength = 2000) {
        ...
        Logger_1.Logger.warn(`Prompt过长 (${prompt.length} chars)，将被截断到 ${maxLength} chars`, 'PromptOptimizer');
        ...
        for (const line of lines) {
            if (headLen + line.length + 1 > headBudget)
                break;
            headLines.push(line);
        ...
        for (let i = lines.length - 1; i >= headLines.length; i--) {
            if (tailLen + lines[i].length + 1 > tailBudget)
                break;
    ...
    static compressHistory(history, maxTokens = 1000) {
        ...
        for (const sm of systemMessages) {
            ...
            if (systemTokens + t > systemTokenBudget)
                break;
            keptSystem.push(sm);
            systemTokens += t;
        }
        ...
        if (result.length < history.length) {
            Logger_1.Logger.warn(`历史对话已压缩: ${history.length} -> ${result.length} 条消息 (保留${keptSystem.length}条system + ${compressed.length}条对话)`, 'PromptOptimizer');
        }
        return result;
    }
}
