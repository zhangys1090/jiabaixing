...
class CuratorService {
    ...
    static getInstance(config, dataDir) {
    ...
    updateConfig(updates) {
        ...
        Logger_1.Logger.info('Curator 配置已更新', 'CuratorService');
    }
    // ─── 使用遥测 ──────────────────────────────────────────
    /**
     * 记录技能查看
    ...
    pin(skillName) {
        ...
        entry.pinned = true;
        this.saveUsage();
        Logger_1.Logger.info(`📌 技能已固定: ${skillName}`, 'CuratorService');
        return { success: true };
    }
    /**
     * 取消固定技能
    ...
    unpin(skillName) {
        ...
        entry.pinned = false;
        this.saveUsage();
        Logger_1.Logger.info(`📌 技能已取消固定: ${skillName}`, 'CuratorService');
        return { success: true };
    }
    /**
     * 获取所有固定技能列表
    ...
    register(name, skillPath, qualityScore = 0.7) {
        ...
        this.saveUsage();
        Logger_1.Logger.info(`📋 技能注册到追踪器: ${name}`, 'CuratorService');
    }
    /**
     ...
     */
    trackUseWithQuality(name, qualityScore) {
        if (this.isBundledOrHub(name))
            return;
        const entry = this.getOrCreateEntry(name);
        entry.use_count++;
        entry.last_used_at = new Date().toISOString();
        if (qualityScore !== undefined) {
            ...
            if (entry.recent_quality_scores.length > MAX_QUALITY_SCORES_HISTORY) {
                entry.recent_quality_scores = entry.recent_quality_scores.slice(-MAX_QUALITY_SCORES_HISTORY);
            }
        }
        this.saveUsage();
    }
    ...
    getRecentQualityScores(name) {
        ...
        if (!record || !record.recent_quality_scores)
            return [];
    ...
    getAutoGeneratedSkillNames() {
        return Object.keys(this.usageData).filter((name) => name.startsWith('auto-'));
    }
    /**
    ...
    getSummary() {
        ...
        return {
            total: all.length,
            active: all.filter((s) => s.last_used_at &&
                now - new Date(s.last_used_at).getTime() <= staleThreshold).length,
            stale: all.filter((s) => !s.last_used_at ||
                now - new Date(s.last_used_at).getTime() > staleThreshold).length,
        };
    }
    /**
    ...
    shareSkillInsights(agentId) {
        ...
        // 建议优化低质量高频技能
        const lowQualityHighUse = topSkills.filter((s) => s.successRate < 0.5 && s.usageCount > 3);
        for (const skill of lowQualityHighUse) {
            recommendations.push(`技能 "${skill.name}" 使用频繁但成功率较低(${(skill.successRate * 100).toFixed(0)}%)，建议优化`);
        }
        // 建议清理陈旧技能
        const staleSkills = this.getLeastUsed();
        if (staleSkills.length > 5) {
            recommendations.push(`有 ${staleSkills.length} 个不常用技能，建议评估是否需要保留`);
        }
        const report = {
            agentId,
            topSkills,
        ...
        Logger_1.Logger.info(`📊 技能洞察报告已生成: agent=${agentId}, topSkills=${topSkills.length}`, 'CuratorService');
        return report;
    }
    /**
     * 整合外部洞察（兼容 SkillUsageTracker.integrateExternalInsights）
     */
    integrateExternalInsights(insights) {
        if (!insights.agentId || !insights.generatedAt)
            return 0;
        let integratedCount = 0;
        for (const externalSkill of insights.topSkills) {
            const localRecord = this.usageData[externalSkill.name];
            if (localRecord) {
                const adjustedScore = localRecord.quality_score * 0.8 + externalSkill.successRate * 0.2;
                localRecord.quality_score = Math.round(adjustedScore * 1000) / 1000;
                integratedCount++;
            }
        }
        for (const rec of insights.recommendations) {
            Logger_1.Logger.info(`💡 外部建议(from ${insights.agentId}): ${rec}`, 'CuratorService');
        }
        this.saveUsage();
     ...
     * @returns 状态转换记录
     */
    performTransitions(dryRun = false) {
        const transitions = [];
        const now = Date.now();
        const staleThreshold = this.config.stale_after_days * 24 * 60 * 60 * 1000;
        const archiveThreshold = this.config.archive_after_days * 24 * 60 * 60 * 1000;
        for (const [name, entry] of Object.entries(this.usageData)) {
            ...
            if (!this.config.prune_builtins && this.isBundledOrHub(name))
                continue;
     ...
     * @returns 审查结果
     */
    async performLLMReview(dryRun = false) {
        const result = {
            rename_mapping: [],
            transitions: [],
            errors: [],
        ...
        const agentSkills = Object.entries(this.usageData).filter(([, entry]) => entry.state !== 'archived' &&
            !entry.pinned &&
     ...
     * 3. 描述关键词重叠度 > 60%
     */
    detectDuplicateSkills(skills) {
        const groups = [];
        const assigned = new Set();
        for (let i = 0; i < skills.length; i++) {
            const [nameA, entryA] = skills[i];
            if (assigned.has(nameA))
                continue;
            const group = [entryA];
            assigned.add(nameA);
            for (let j = i + 1; j < skills.length; j++) {
                const [nameB, entryB] = skills[j];
                if (assigned.has(nameB))
                    continue;
     ...
     * 判断两个技能是否相似
     */
    areSkillsSimilar(nameA, _entryA, nameB, _entryB) {
        // 层1：名称标准化后完全相同
        ...
        if (skillA && skillB) {
            const overlap = this.computeDescriptionOverlap(skillA.definition.description, skillB.definition.description);
            if (overlap > 0.6)
                return true;
        }
        return false;
     ...
     * 计算两个描述的关键词重叠度
     */
    computeDescriptionOverlap(descA, descB) {
        const wordsA = new Set(descA
            .toLowerCase()
            .split(/[\s,，。.、；;：:！!？?]+/)
            .filter((w) => w.length >= 2));
        const wordsB = new Set(descB
            .toLowerCase()
            .split(/[\s,，。.、；;：:！!？?]+/)
            .filter((w) => w.length >= 2));
    ...
    restore(skillName) {
        ...
        this.saveUsage();
        Logger_1.Logger.info(`📦 技能已恢复: ${skillName}`, 'CuratorService');
        return { success: true };
    }
    // ─── 完整运行 ──────────────────────────────────────────
    /**
    ...
    async run(dryRun = false) {
        ...
        Logger_1.Logger.info(`Curator 运行开始: ${runId}${dryRun ? ' (dry-run)' : ''}`, 'CuratorService');
        try {
            // 首次运行只记录时间
            if (this.state.first_observation) {
                ...
                Logger_1.Logger.info('Curator 首次观测，记录时间，推迟到下一个间隔运行', 'CuratorService');
            ...
            Logger_1.Logger.info(`Curator 运行完成: ${runId}, transitions=${report.transitions.length}`, 'CuratorService');
        }
        catch (error) {
            Logger_1.Logger.error('Curator 运行失败', error, 'CuratorService');
            report.errors.push({
                skill: '__global__',
                error: error.message,
            });
    ...
    getStatus() {
        ...
        const nextEligible = this.state.last_run_at
            ? new Date(new Date(this.state.last_run_at).getTime() +
                this.config.interval_hours * 60 * 60 * 1000).toISOString()
            : '首次观测后一个间隔';
        return {
    ...
    pause() {
        ...
        this.saveState();
        Logger_1.Logger.info('Curator 已暂停', 'CuratorService');
    }
    resume() {
        this.state.paused = false;
        this.saveState();
        Logger_1.Logger.info('Curator 已恢复', 'CuratorService');
    }
    isPaused() {
    ...
    async createBackup(reason) {
        if (!this.config.backup.enabled) {
            Logger_1.Logger.warn('备份已禁用，跳过', 'CuratorService');
            return null;
        }
        const skillsDir = path.resolve(process.cwd(), 'data', 'skills');
        if (!fs.existsSync(skillsDir)) {
            Logger_1.Logger.warn('技能目录不存在，跳过备份', 'CuratorService');
            return null;
        }
        ...
        try {
            ...
            // 裁剪旧备份
            this.pruneBackups();
            Logger_1.Logger.info(`备份已创建: ${backupId} (${(stats.size / 1024).toFixed(1)}KB)`, 'CuratorService');
            return manifest;
        }
        catch (error) {
            Logger_1.Logger.error('备份创建失败', error, 'CuratorService');
            return null;
        }
    }
    /**
    ...
    async rollback(backupId) {
        ...
        const skillsDir = path.resolve(process.cwd(), 'data', 'skills');
        const archivePath = path.join(this.dataDir, 'backups', target.id, 'skills.tar.gz');
        ...
        try {
            ...
            // 重新加载遥测数据
            this.usageData = this.loadUsage();
            Logger_1.Logger.info(`已回滚到备份: ${target.id}`, 'CuratorService');
            return { success: true };
        }
        catch (error) {
            Logger_1.Logger.error('回滚失败', error, 'CuratorService');
            return {
                success: false,
                error: `回滚失败: ${error.message}`,
            };
        }
    }
    // ─── 私有方法 ──────────────────────────────────────────
    getOrCreateEntry(skillName) {
    ...
    archiveSkillFiles(skillName) {
        ...
        try {
            ...
            Logger_1.Logger.info(`技能文件已归档: ${skillName} → .archive/`, 'CuratorService');
        }
        catch (error) {
            Logger_1.Logger.error(`归档技能文件失败: ${skillName}`, error, 'CuratorService');
        }
    }
    /**
     * 恢复已归档的技能文件
     */
    restoreSkillFiles(skillName) {
        const skillsDir = path.resolve(process.cwd(), 'data', 'skills');
        ...
        try {
            ...
            Logger_1.Logger.info(`技能文件已恢复: ${skillName} ← .archive/`, 'CuratorService');
            return true;
        }
        catch (error) {
            Logger_1.Logger.error(`恢复技能文件失败: ${skillName}`, error, 'CuratorService');
            return false;
        }
    }
    /**
     ...
     * 写入运行报告
     */
    writeRunReport(runId, report) {
        const logsDir = path.join(this.dataDir, 'logs', runId);
        try {
            fs.mkdirSync(logsDir, { recursive: true });
            // 机器可读 JSON
            fs.writeFileSync(path.join(logsDir, 'run.json'), JSON.stringify(report, null, 2), 'utf-8');
            // 人类可读 Markdown 报告
            const md = this.generateReportMarkdown(report);
            fs.writeFileSync(path.join(logsDir, 'REPORT.md'), md, 'utf-8');
        }
        catch (error) {
            Logger_1.Logger.error('写入运行报告失败', error, 'CuratorService');
        }
    }
    /**
     * 生成 Markdown 格式报告
     ...
     * 创建目录的 tar.gz 压缩
     */
    async tarGzDirectory(sourceDir, outputPath) {
    ...
    pruneBackups() {
        ...
        for (const backup of toRemove) {
            ...
            try {
                ...
                Logger_1.Logger.debug(`旧备份已裁剪: ${backup.id}`, 'CuratorService');
            }
            catch {
                Logger_1.Logger.warn(`裁剪旧备份失败: ${backup.id}`, 'CuratorService');
            }
        }
    }
    // ─── 持久化 ────────────────────────────────────────────
    loadUsage() {
        ...
        try {
            ...
            Logger_1.Logger.warn(`加载使用遥测失败: ${error.message}`, 'CuratorService');
        }
        return {};
    }
    saveUsage() {
        ...
        try {
            ...
            fs.writeFileSync(filePath, JSON.stringify(this.usageData, null, 2), 'utf-8');
        }
        catch (error) {
            Logger_1.Logger.error('保存使用遥测失败', error, 'CuratorService');
        }
    }
    loadState() {
        ...
        try {
            ...
            Logger_1.Logger.warn(`加载 Curator 状态失败: ${error.message}`, 'CuratorService');
        }
        return {
            last_run_at: null,
            paused: false,
    ...
    saveState() {
        ...
        try {
            ...
            Logger_1.Logger.error('保存 Curator 状态失败', error, 'CuratorService');
        }
    }
}
