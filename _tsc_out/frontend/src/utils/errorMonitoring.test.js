...
const errorMonitoring_1 = require("./errorMonitoring");
...
describe('ErrorMonitor', () => {
    ...
    test('should report network error', async () => {
        ...
        // 报告网络错误
        errorMonitoring_1.errorMonitor.reportNetworkError('http://example.com/api', 500, 'Internal Server Error');
        // 验证 fetch 是否被调用
        expect(global.fetch).toHaveBeenCalledWith('http://test-api.jiabaixing.com/api/error/monitoring', expect.objectContaining({
            method: 'POST',
            headers: {
    ...
    test('should report custom error', async () => {
        ...
        // 报告自定义错误
        errorMonitoring_1.errorMonitor.reportCustomError('Test error', errorMonitoring_1.ErrorLevel.ERROR, {
            foo: 'bar',
        });
        // 验证 fetch 是否被调用
        expect(global.fetch).toHaveBeenCalledWith('http://test-api.jiabaixing.com/api/error/monitoring', expect.objectContaining({
