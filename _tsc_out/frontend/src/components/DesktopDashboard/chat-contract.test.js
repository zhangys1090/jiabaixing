...
function countAssistantBubbles() {
    return document.querySelectorAll('.hermes-message-assistant:not(.hermes-message-typing)').length;
}
/** 统计含指定文本的助手气泡数量 */
function bubbleCountWithText(text) {
    return Array.from(document.querySelectorAll('.hermes-message-assistant:not(.hermes-message-typing) .message-text')).filter((el) => (el.textContent ?? '').includes(text)).length;
}
async function sendMessage(text, replyText) {
    ...
    react_1.fireEvent.change(input, { target: { value: text } });
    const sendButton = document.querySelector('.hermes-send-btn');
    if (!sendButton)
        throw new Error('未找到发送按钮 .hermes-send-btn');
    react_1.fireEvent.click(sendButton);
    await (0, react_1.waitFor)(() => {
        expect(react_1.screen.getByText(replyText)).toBeInTheDocument();
    });
}
