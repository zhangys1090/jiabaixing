...
router.post('/orchestrate', async (req, res) => {
    try {
        ...
        Logger_1.Logger.info(`[Orchestrate] 收到编排请求: ${goal.substring(0, 60)}...`, 'OrchestrateRoute');
        ...
        const orchestrator = new OrchestratorAgent_1.OrchestratorAgent({
            ...
            llm: {
                decomposeGoal: async (userGoal, ctx) => {
                    const result = await getCore().processInput(`请将以下目标拆解为子任务列表：\n目标: ${userGoal}\n上下文: ${ctx || '无'}\n\n返回JSON数组，每个元素包含: goal, dependencies[], priority`, 'orchestrator');
                    try {
...
router.post('/evaluate', async (req, res) => {
    try {
        ...
        const stepResults = (context?.steps || []).map((s) => stepEvaluator.evaluateStep({
            stepId: s.stepId,
            toolName: s.toolName,
            args: s.args,
            result: s.result,
