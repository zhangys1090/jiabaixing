...
exports.handleHermesCommandCLI = handleHermesCommandCLI;
...
async function handleHermesCommandCLI(subArgs, options) {
    ...
    switch (action) {
        case 'batch': {
            ...
            const batchPrompts = prompts.map((text, i) => ({
                id: `p${i + 1}`,
                text,
            }));
            try {
                const data = await (0, ipc_1.requestWithFallback)('hermes.batch', { prompts: batchPrompts }, { path: '/api/batch/run' });
                if (options.json) {
                ...
                Logger_1.Logger.error('批处理失败', err, 'HermesCommand');
                process.stderr.write(`批处理失败: ${err.message}\n`);
                process.exit(1);
            }
            break;
        }
        case 'ide': {
            ...
            try {
                const data = await (0, ipc_1.requestWithFallback)('hermes.ide.chat', { message }, { path: '/api/ide/chat' });
                if (options.json) {
                    process.stdout.write(JSON.stringify(data, null, 2) + '\n');
                }
                else {
                ...
                Logger_1.Logger.error('IDE 聊天失败', err, 'HermesCommand');
                process.stderr.write(`IDE 聊天失败: ${err.message}\n`);
                process.exit(1);
            }
            break;
        }
        case 'trajectory': {
            ...
            if (subAction === 'stats') {
                try {
                    const data = await (0, ipc_1.requestWithFallback)('hermes.trajectory.stats', {}, { path: '/api/trajectory/stats' });
                    if (options.json) {
                        process.stdout.write(JSON.stringify(data, null, 2) + '\n');
                    }
                    else {
                        process.stdout.write(`\n  ${constants_1.COLORS.bold}RL 轨迹统计${constants_1.COLORS.reset}\n\n`);
                        process.stdout.write(`  总轨迹:   ${data.total}\n`);
                        process.stdout.write(`  已过滤:   ${data.filtered}\n`);
                        process.stdout.write(`  平均质量: ${(data.avgQuality * 100).toFixed(1)}%\n`);
                        process.stdout.write(`  平均步数: ${data.avgSteps}\n\n`);
                    }
                }
                catch (err) {
                    Logger_1.Logger.error('轨迹统计失败', err, 'HermesCommand');
                    process.stderr.write(`轨迹统计失败: ${err.message}\n`);
                    process.exit(1);
                }
            }
            else if (subAction === 'export') {
                const format = subArgs[2] || 'jsonl';
                try {
                    const data = await (0, ipc_1.requestWithFallback)('hermes.trajectory.export', { format }, { path: '/api/trajectory/export' });
                    if (options.json) {
                        process.stdout.write(JSON.stringify(data, null, 2) + '\n');
                    }
                    else {
                    ...
                    Logger_1.Logger.error('轨迹导出失败', err, 'HermesCommand');
                    process.stderr.write(`轨迹导出失败: ${err.message}\n`);
                    process.exit(1);
                }
            }
            else {
        ...
        case 'tool': {
            ...
            if (!toolName) {
                ...
                process.stderr.write('用法: hermes tool image_generate prompt="a cat"\n');
                process.exit(1);
            }
            const params = parseKVArgs(subArgs.slice(2));
            try {
                const data = await (0, ipc_1.requestWithFallback)('hermes.tool.execute', { toolName, params }, { path: '/api/tools/execute' });
                if (options.json) {
                    process.stdout.write(JSON.stringify(data, null, 2) + '\n');
                }
                else {
                ...
                Logger_1.Logger.error('工具执行失败', err, 'HermesCommand');
                process.stderr.write(`工具执行失败: ${err.message}\n`);
                process.exit(1);
            }
            break;
        }
        case 'image': {
            ...
            try {
                const data = await (0, ipc_1.requestWithFallback)('hermes.tool.image', { toolName: 'image_generate', params: { prompt } }, { path: '/api/tools/execute' });
                if (options.json) {
                    process.stdout.write(JSON.stringify(data, null, 2) + '\n');
                }
                else {
                    process.stdout.write(data.success
                        ? `✅ ${JSON.stringify(data.output)}\n`
                        : `❌ ${data.error}\n`);
                }
            }
            catch (err) {
                Logger_1.Logger.error('图像生成失败', err, 'HermesCommand');
                process.stderr.write(`图像生成失败: ${err.message}\n`);
                process.exit(1);
            }
            break;
        }
        case 'tts': {
            ...
            try {
                const data = await (0, ipc_1.requestWithFallback)('hermes.tool.tts', { toolName: 'tts_speak', params: { text } }, { path: '/api/tools/execute' });
                if (options.json) {
                    process.stdout.write(JSON.stringify(data, null, 2) + '\n');
                }
                else {
                ...
                Logger_1.Logger.error('TTS 失败', err, 'HermesCommand');
                process.stderr.write(`TTS 失败: ${err.message}\n`);
                process.exit(1);
            }
            break;
        }
        case 'fetch': {
            ...
            try {
                const data = await (0, ipc_1.requestWithFallback)('hermes.tool.fetch', { toolName: 'web_fetch', params: { url } }, { path: '/api/tools/execute' });
                if (options.json) {
                    process.stdout.write(JSON.stringify(data, null, 2) + '\n');
                }
                else {
                    const output = data.success
                        ? typeof data.output === 'string'
                            ...
                            : JSON.stringify(data.output)
                        : data.error || '失败';
                    process.stdout.write(data.success ? `✅ ${output.substring(0, 500)}\n` : `❌ ${output}\n`);
                }
            }
            catch (err) {
                Logger_1.Logger.error('网页抓取失败', err, 'HermesCommand');
                process.stderr.write(`网页抓取失败: ${err.message}\n`);
                process.exit(1);
            }
            break;
        }
