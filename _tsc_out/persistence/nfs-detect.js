...
exports.detectNfsEnvironment = detectNfsEnvironment;
exports.resolveJournalMode = resolveJournalMode;
 ...
 */
function detectNfsEnvironment(dbPath) {
    // 1. 环境变量显式覆盖
    const envMode = process.env.JIABAIXING_JOURNAL_MODE;
    if (envMode === 'delete' || envMode === 'DELETE')
        return true;
    if (envMode === 'wal' || envMode === 'WAL')
        return false;
    ...
    try {
        ...
        const mounts = fs_1.default.readFileSync('/proc/mounts', 'utf-8');
        const mountPoints = [];
 ...
 */
function resolveJournalMode(dbPath) {
