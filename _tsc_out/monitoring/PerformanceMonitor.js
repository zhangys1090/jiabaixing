...
class PerformanceMonitor extends events_1.EventEmitter {
    ...
    static MAX_METRICS_HISTORY = 5000;
    tokenUsage = {
        totalInputTokens: 0,
        totalOutputTokens: 0,
        totalTokens: 0,
    ...
    recordRequest(responseTime, success) {
        ...
        if (this.metrics.length > PerformanceMonitor.MAX_METRICS_HISTORY) {
            this.metrics = this.metrics.slice(-PerformanceMonitor.MAX_METRICS_HISTORY);
        }
    ...
    cleanupOldMetrics() {
        ...
        for (const [id, span] of this.activeSpans) {
            if (span.startTime < spanTimeout) {
                ...
                Logger_1.Logger.debug(`🧹 清理超时span: ${span.name} (${id})`, 'PerformanceMonitor');
            }
        }
    }
