...
class OutputGuardrailEngine {
    ...
    setGuardrailSeverity(guardrailName, severity) {
        this.severityOverrides.set(guardrailName, severity);
    }
    getEffectiveSeverity(guardrailName, resultRiskLevel) {
        const override = this.severityOverrides.get(guardrailName);
        if (override)
            return override;
        if (resultRiskLevel === 'critical')
            return 'critical';
        if (resultRiskLevel === 'high')
            return 'high';
        return 'medium';
    }
    /** 注册内置 Guardrail */
            ...
            check: (output) => {
                ...
                if (!result.safe) {
                    ...
                    if (violationNames.length > 0) {
                        Logger_1.Logger.warn(`🛡️ 输出Guardrail拦截: 检测到 ${violationNames.join(', ')}`, 'OutputGuardrail');
                        return {
                            passed: false,
    ...
    check(output) {
        ...
        for (const guardrail of this.guardrails) {
            ...
            if (!result.passed) {
                const severity = this.getEffectiveSeverity(guardrail.name, result.riskLevel);
                const action = SEVERITY_ACTIONS[severity];
                if (action.warn) {
                    Logger_1.Logger.warn(`🛡️ 输出Guardrail [${guardrail.name}] 检测到问题 (严重度=${severity}): ${result.reason}`, 'OutputGuardrailEngine');
                }
