...
exports.checkSensitiveInfo = checkSensitiveInfo;
...
const SENSITIVE_PATTERNS = [
    ...
    {
        pattern: /\b\d{6}(?:18|19|20)\d{2}(?:0[1-9]|1[0-2])(?:0[1-9]|[12]\d|3[01])\d{4}\b/g,
        name: '身份证号',
        risk: 'high',
    },
    {
    ...
    // ===== 认证凭据 =====
    // 支持 "密码: xxx"、"密码=xxx"、"密码是 xxx"、"password is xxx" 等模式
    {
        pattern: /(?:password|密码|pwd|passwd)\s*(?:[:=]|是|is|was|are)\s*\S+/gi,
        name: '密码泄露',
        risk: 'critical',
    },
    {
        pattern: /(?:secret|密钥|api[_-]?key|access[_-]?token|secret[_-]?key)\s*(?:[:=]|是|is|was|are)\s*['"]?[a-zA-Z0-9-]{8,}/gi,
        name: '密钥/Token泄露',
        risk: 'critical',
        scenes: ['output', 'storage'],
    },
    {
        pattern: /(?:bearer|basic)\s+\S+/gi,
        name: '认证头泄露',
        risk: 'high',
        scenes: ['output'],
    },
    {
        pattern: /\b(?:sk-|api_)[a-zA-Z0-9]{20,}/g,
        name: 'API密钥',
        risk: 'critical',
    },
    ...
    {
        pattern: /(?:api[_-]?key|apikey|access[_-]?token|secret[_-]?key)\s*[:=]\s*['"]?[a-zA-Z0-9]{8,}/i,
        name: '密钥凭证',
        risk: 'critical',
        scenes: ['storage'],
    },
 ...
 * @returns 检测结果
 */
function checkSensitiveInfo(text, scene = 'output') {
    const violations = [];
    const applicablePatterns = SENSITIVE_PATTERNS.filter((p) => !p.scenes || p.scenes.includes(scene));
...
function sanitizeText(text) {
    return (text
        ...
        // 身份证号
        .replace(/\b\d{6}(?:18|19|20)\d{2}(?:0[1-9]|1[0-2])(?:0[1-9]|[12]\d|3[01])\d{4}\b/g, '[身份证-已脱敏]')
        ...
        // IPv4 — 使用精确模式避免误报版本号
        .replace(/\b(?:(?:25[0-5]|2[0-4]\d|1\d{2}|[1-9]?\d)\.){3}(?:25[0-5]|2[0-4]\d|1\d{2}|[1-9]?\d)\b/g, '[IP-已脱敏]')
        // IPv6
        .replace(/\b(?:[0-9a-fA-F]{1,4}:){7}[0-9a-fA-F]{1,4}\b/gi, '[IPv6-已脱敏]')
        .replace(/::(?:[0-9a-fA-F]{1,4}:){0,5}[0-9a-fA-F]{1,4}\b/gi, '[IPv6-已脱敏]')
        // 密码/密钥
        .replace(/((?:password|密码|pwd|passwd|secret|密钥|api[_-]?key|token)\s*(?:[:=]|是|is|was|are)\s*)\S+/gi, '$1[已脱敏]')
        // 认证头
