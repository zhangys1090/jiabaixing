...
class ContextManager {
    ...
    offloadedHistory = [];
    offloadIndex = [];
    static MAX_OFFLOADED_HISTORY = 5000;
    static MAX_OFFLOAD_INDEX = 10000;
    static MAX_LRU_MAP = 10000;
    compressionConfig;
    lruAccessMap = new Map();
    delegatePipeline = null;
    ...
    async buildContext(input) {
        if (this.delegatePipeline) {
            try {
                ...
                Logger_1.Logger.warn(`UnifiedContextPipeline 委托失败，回退到 TS 本地实现: ${err.message}`, 'ContextManager');
            }
        }
        this.entries = [];
        ...
        if (this.deps.personaCore) {
            try {
                ...
                if (toneInstruction) {
                    const truncated = this.allocator.truncateToBudget(toneInstruction, allocation.dynamicContext);
                    messages.push({ role: 'system', content: truncated });
                    this.entries.push({
                        id: 'persona_tone',
                        type: 'dynamic',
        ...
        try {
            ...
            if (historyTokens >
                ...
                if (nonSystemMsgs.length > 0) {
                    ...
                    const focusedMsgs = this.focusByAttention(nonSystemMsgs, input.text, attentionBudget);
                    if (focusedMsgs.length > 0 &&
                        focusedMsgs.length < nonSystemMsgs.length) {
                        Logger_1.Logger.info(`🎯 P3 注意力聚焦: ${nonSystemMsgs.length} → ${focusedMsgs.length} 条消息`, 'ContextManager');
                    }
                }
    ...
    async buildPhaseContext(input, phase) {
        ...
        if (this.deps.constitutionalBuilder) {
            try {
                ...
                messages.push({
                    role: 'system',
                    content: '你是家百星，28岁私人秘书。成熟、专业、从容。只使用已有工具，不编造工具和结果。',
                });
            }
        }
        else {
    ...
    setDelegatePipeline(pipeline) {
        this.delegatePipeline = pipeline;
        if (pipeline) {
            Logger_1.Logger.info('ContextManager: 已设置 UnifiedContextPipeline 委托', 'ContextManager');
        }
    }
    /**
     * 获取 Token 预算分配
...
请直接输出摘要内容，不要包含其他格式。`;
    ...
    offloadHistory(messages, keepCount = 10, strategy = 'oldest_first') {
        ...
        if (this.offloadedHistory.length > ContextManager.MAX_OFFLOADED_HISTORY) {
            this.offloadedHistory = this.offloadedHistory.slice(-ContextManager.MAX_OFFLOADED_HISTORY);
        }
        if (this.deps.offloadDir) {
...
${keyPoints.slice(-5).join('\n')}`;
    ...
    selectByLruRelevance(messages, keepCount) {
        ...
        const scored = messages.map((msg, idx) => {
            const content = msg.content || '';
            const accessCount = Array.from(this.lruAccessMap.entries()).filter(([, time]) => time > 0).length;
        ...
        const selectedIndices = new Set(scored.slice(0, keepCount).map((s) => messages.indexOf(s.msg)));
    ...
    persistOffloadedMessages(messages) {
        ...
        try {
            ...
            if (this.lruAccessMap.size > ContextManager.MAX_LRU_MAP) {
                const sorted = [...this.lruAccessMap.entries()].sort((a, b) => a[1] - b[1]);
                const toDelete = sorted.slice(0, sorted.length - ContextManager.MAX_LRU_MAP);
                for (const [key] of toDelete)
                    this.lruAccessMap.delete(key);
            }
            if (this.offloadIndex.length > ContextManager.MAX_OFFLOAD_INDEX) {
                this.offloadIndex = this.offloadIndex.slice(-ContextManager.MAX_OFFLOAD_INDEX);
            }
