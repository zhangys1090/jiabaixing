...
class ContextWindowManager {
    ...
    manageWindow(messages) {
        ...
        Logger_1.Logger.info(`📊 阶段2(历史压缩)后: ${result.compressedTokenCount} tokens (压缩比 ${result.compressionRatio.toFixed(2)})`, 'ContextWindowManager');
     ...
     * @returns 注入记忆后的消息列表
     */
    injectCrossSessionMemory(messages, crossSessionMemory, currentInput) {
        void currentInput;
        ...
        for (const entry of sorted) {
            ...
            if (usedTokens + entryTokens > maxMemoryTokens)
                break;
            selected.push(entry);
        ...
        if (selected.length === 0)
            return messages;
        const memoryLines = selected.map((e) => `- [${e.type}] ${e.content.substring(0, 200)}`);
        const memoryBlock = [
        ...
        Logger_1.Logger.info(`🧠 P1-3: 注入跨会话记忆 ${selected.length} 条 (${usedTokens} tokens)`, 'ContextWindowManager');
        return result;
    }
}
