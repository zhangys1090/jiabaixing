...
const ContextReferenceResolver_1 = require("./ContextReferenceResolver");
/** 模拟 Python 端 PerceptionReferenceResolver 的 provider */
function mockProvider(resolved) {
    return async (tokens) => {
        const out = [];
        for (const t of tokens) {
            const r = resolved[t];
            if (!r)
                continue; // 无法解析的 token 交由上层保留在文本
            out.push({
                token: '@' + t,
                kind: r.kind ?? 'named',
...
describe('ContextReferenceResolver 多模态 @引用 (U4)', () => {
    describe('collectMultimodalTokens', () => {
        it('提取 CJK 具名类型与 #索引样本', () => {
            const toks = ContextReferenceResolver_1.ContextReferenceResolver.collectMultimodalTokens('参考 @截图区域 和 @visual#0 以及 @设备状态 再决定');
        ...
        it('识别拉丁通道名但不劫持普通 @配置/@文件', () => {
            const toks = ContextReferenceResolver_1.ContextReferenceResolver.collectMultimodalTokens('@visual @audio @config @server.ts');
        ...
        it('不误伤 URL 中的 https', () => {
            const toks = ContextReferenceResolver_1.ContextReferenceResolver.collectMultimodalTokens('见 @https://example.com 文档');
            expect(toks).not.toContain('https');
        });
    });
    describe('resolve 多模态引用', () => {
        it('有 provider 时解析并替换文本为 [ref#N] 标记', async () => {
            const resolver = new ContextReferenceResolver_1.ContextReferenceResolver({ projectRoot: '/tmp' });
            resolver.setMultimodalReferenceProvider(mockProvider({
                截图区域: {
                    modality: 'visual',
        ...
        it('文件/URL 引用与多模态引用可共存', async () => {
            ...
            const res = await resolver.resolve('读取 @https://example.com 并参考 @截图区域');
            // 文件/URL 引用
