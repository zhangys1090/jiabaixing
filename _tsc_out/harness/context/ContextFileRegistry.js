...
exports.ContextFileRegistry = exports.SOUL_SEARCH_PATHS = exports.SOUL_FILE_NAME = exports.CONTEXT_FILE_PRIORITY = void 0;
...
/** SOUL.md 文件名 */
exports.SOUL_FILE_NAME = 'SOUL.md';
/** SOUL.md 搜索路径（按优先级） */
exports.SOUL_SEARCH_PATHS = [
...
class ContextFileRegistry {
    ...
    async loadAll() {
        const now = Date.now();
        if (now - this._cacheTimestamp < this._cacheTtlMs &&
        ...
        Logger_1.Logger.info(`📄 上下文文件已加载: ${entries.length} 个` +
            (projectEntry
                ? ` [项目: ${projectEntry.fileName}]`
                : ' [无项目上下文]') +
            (soulEntry ? ` [SOUL: 已加载]` : ' [无SOUL]'), 'ContextFileRegistry');
        return entries;
    }
    /**
     * 强制刷新缓存
     ...
     */
    getAllowedFileNames() {
        return [...exports.CONTEXT_FILE_PRIORITY];
    }
    /**
     * 检查文件名是否在允许列表中
     */
    isAllowedFileName(fileName) {
        return exports.CONTEXT_FILE_PRIORITY.includes(fileName);
    }
    // ==================== 项目上下文加载 ====================
    /**
     * 按优先级加载项目上下文文件
     ...
     */
    loadProjectContext() {
        for (const fileName of exports.CONTEXT_FILE_PRIORITY) {
            const filePath = path_1.default.join(this._projectRoot, fileName);
            try {
                if (fs_1.default.existsSync(filePath)) {
                    const rawContent = fs_1.default.readFileSync(filePath, 'utf-8').trim();
                    if (rawContent.length === 0)
                        continue;
                    // 安全扫描
                    const scanResult = this.scanForInjection(rawContent, fileName);
                    if (!scanResult.safe) {
                        Logger_1.Logger.warn(`⚠️ 上下文文件 ${fileName} 被拦截: ${scanResult.threats.join(', ')}`, 'ContextFileRegistry');
                        continue;
                    }
                    // 截断
                ...
                Logger_1.Logger.debug(`跳过上下文文件 ${fileName}: ${error.message}`, 'ContextFileRegistry');
            }
        }
        return null;
    }
     ...
     */
    loadSoulContext() {
        for (const searchPath of exports.SOUL_SEARCH_PATHS) {
            const filePath = path_1.default.join(this._projectRoot, searchPath);
            try {
                if (fs_1.default.existsSync(filePath)) {
                    const rawContent = fs_1.default.readFileSync(filePath, 'utf-8').trim();
                    if (rawContent.length === 0)
                    ...
                    if (!scanResult.safe) {
                        Logger_1.Logger.warn(`⚠️ SOUL文件 ${searchPath} 被拦截: ${scanResult.threats.join(', ')}`, 'ContextFileRegistry');
                        continue;
                    }
                    // 截断
     ...
     * @returns 扫描结果
     */
    scanForInjection(content, fileName) {
        const threats = [];
        ...
        if (threats.length > 0) {
            Logger_1.Logger.warn(`[BLOCKED: ${fileName} contained potential prompt injection (${threats.join(', ')}). Content not loaded.]`, 'ContextFileRegistry');
        }
     ...
     */
    truncateContent(content, fileName) {
...
exports.ContextFileRegistry = ContextFileRegistry;
