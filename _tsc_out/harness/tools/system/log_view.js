...
exports.LOG_VIEW_DEF = void 0;
exports.createLogViewExecutor = createLogViewExecutor;
const types_1 = require("../../types");
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
exports.LOG_VIEW_DEF = {
    name: 'log_view',
    description: '查看应用日志。USE WHEN: 用户想看日志、查错误、排查问题、了解系统运行状态。支持按级别(error/warn/info)、模块名、关键词过滤。DO NOT USE WHEN: 用户要修改日志配置或清理日志文件（用log_clean）。',
...
function createLogViewExecutor() {
    return async (params, _context) => {
        ...
        try {
            ...
            if (keyword) {
                ...
                filtered = filtered.filter((log) => (log.message && log.message.toLowerCase().includes(kw)) ||
                    (log.module && log.module.toLowerCase().includes(kw)));
            }
            if (module) {
                const mod = module.toLowerCase();
                filtered = filtered.filter((log) => log.module && log.module.toLowerCase().includes(mod));
            }
            // 取最后 N 条
...
function formatLogOutput(logs, logFile, level, keyword, module) {
    const lines = [];
