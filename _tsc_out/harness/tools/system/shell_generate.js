...
exports.SHELL_GENERATE_DEF = void 0;
exports.createShellGenerateExecutor = createShellGenerateExecutor;
const Logger_1 = require("../../../utils/Logger");
const types_1 = require("../../types");
exports.SHELL_GENERATE_DEF = {
    name: 'shell_generate',
    description: '将自然语言描述转换为 Shell 命令。USE WHEN: 用户用自然语言描述想做什么（如"查看端口占用"、"找大文件"、"清理缓存"），而不是给出具体命令。DO NOT USE WHEN: 用户已经给出了具体命令（用 shell_exec）。自动适配 Windows/Linux/macOS。',
...
function isLowRisk(command) {
    const trimmed = command.trim();
    return LOW_RISK_PATTERNS.some((p) => p.test(trimmed));
}
function createShellGenerateExecutor(deps = {}) {
...
5. 只输出 JSON，不要其他内容`;
            const response = await deps.llm.chat(prompt, [], '你是家百星的命令行助手，只输出 JSON。');
            // 解析 LLM 返回的 JSON
            let parsed;
            try {
            ...
            Logger_1.Logger.info(`💻 shell_generate: "${intent}" → "${command}"`, 'ShellGenerate');
            return {
                success: true,
                output,
                duration: Date.now() - startTime,
