...
3. 如果是列表/表格，提取最重要的几项
4. 不要重复原始输出`;
        return await llm.chat(prompt, [], '你是家百星的命令行输出解读模块。简洁回答。');
    }
    catch {
        return '';
    }
        ...
        try {
            ...
            if (bridgeForShell) {
                try {
                    ...
                    return fail(pyRes?.error || 'Python shell_exec 执行失败', duration, String(pyRes?.output ?? '').substring(0, 5000));
                }
                catch (pyErr) {
