...
function checkJsCodeSafety(code) {
...
function checkShellCodeSafety(code) {
    const normalized = code
        .replace(/["'`\\]/g, ' ')
        .replace(/\s+/g, ' ')
        .trim()
        .toLowerCase();
    const danger = (0, shell_exec_1.isShellCommandDangerous)(normalized);
    if (danger.blocked) {
...
async function executeJavaScript(code, deps, timeout, startTime) {
    ...
    if (!safetyCheck.safe) {
        Logger_1.Logger.warn(`🛡️ execute_code JS 安全拦截: ${safetyCheck.reason}`, 'ExecuteCode');
        return fail(`JavaScript 代码被安全策略拦截: ${safetyCheck.reason}。如需执行系统操作，请使用 shell_exec 工具。`, Date.now() - startTime);
    }
    const logs = [];
...
with (_guarded) {
  ${code}
        ...
        const asyncFn = new Function('console', 'globals', wrappedCode);
...
function runSpawnAsync(command, timeout, cwd) {
...
async function executeSubprocess(code, language, timeout, startTime) {
    let command;
    let cleanupFile = null;
    try {
        if (language === 'python') {
            const randSuffix = crypto.randomBytes(8).toString('hex');
            const tmpFile = path_1.default.join(os_1.default.tmpdir(), `jbx_exec_${randSuffix}.py`);
            fs_1.default.writeFileSync(tmpFile, code, { encoding: 'utf-8', mode: 0o600 });
            cleanupFile = tmpFile;
        ...
        const { stdout, stderr, exitCode, timedOut } = await runSpawnAsync(command, timeout, process.cwd());
        if (cleanupFile) {
            try {
        ...
        if (timedOut) {
            return fail(`${language} 执行超时 (>${timeout}ms)`, Date.now() - startTime, (stdout || stderr).substring(0, 5000));
        }
        ...
        return fail(`${language} 执行失败 (exit ${exitCode}): ${stderr.substring(0, 200)}`, Date.now() - startTime, output.substring(0, 5000));
    }
    catch (error) {
        if (cleanupFile) {
            try {
        ...
        const err = error;
        return fail(`${language} 执行失败: ${err.message.substring(0, 200)}`, Date.now() - startTime);
    }
}
function createExecuteCodeExecutor(deps = {}) {
    return async (params, _context) => {
        ...
        if (language === 'shell' || language === 'bash' || language === 'sh') {
            ...
            if (!shellSafety.safe) {
                Logger_1.Logger.warn(`🛡️ execute_code 拦截危险 shell 代码: ${shellSafety.reason}`, 'ExecuteCode');
                return fail(`代码被安全策略拦截: ${shellSafety.reason}`, Date.now() - startTime);
            }
        }
        ...
        if (language === 'python' || language === 'python3') {
            ...
            if (!pySafety.safe) {
                Logger_1.Logger.warn(`🛡️ execute_code Python 安全拦截: ${pySafety.reason}`, 'ExecuteCode');
                return fail(`Python 代码被安全策略拦截: ${pySafety.reason}。如需执行系统操作，请使用 shell_exec 工具。`, Date.now() - startTime);
            }
