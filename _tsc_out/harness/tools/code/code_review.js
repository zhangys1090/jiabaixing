...
exports.CODE_REVIEW_DEF = {
    name: 'code_review',
    description: '审查代码文件，从规则检查(质量/风格)、安全模式扫描、可选 LLM 逻辑审查三个层面发现问题。USE WHEN: 用户要求代码审查、找bug、安全检查、代码质量分析。DO NOT USE WHEN: 用户要修改代码（用code_fix）或生成新代码（用code_generate）。返回结构化审查报告（注：非真实语法树/性能剖析，仅模式匹配 + 可选 LLM）。',
        ...
        try {
            ...
            if (deps.llm && content.length < 10000) {
                try {
                    ...
                    llmFailed = true;
                    Logger_1.Logger.warn(`⚠️ code_review LLM 逻辑审查失败，仅返回规则检查+安全扫描: ${llmErr.message}`, 'CodeReview');
                }
            }
...
只输出 JSON，不要其他内容。最多输出 5 个最重要的问题。`;
    const response = await llm.chat(prompt, [], '你是家百星的代码审查模块。只输出 JSON 数组。');
    try {
