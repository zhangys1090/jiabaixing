...
exports.CODE_GENERATE_DEF = void 0;
exports.createCodeGenerateExecutor = createCodeGenerateExecutor;
const Logger_1 = require("../../../utils/Logger");
const types_1 = require("../../types");
const codeShared_1 = require("./codeShared");
exports.CODE_GENERATE_DEF = {
    name: 'code_generate',
    description: '根据需求描述生成代码。适用场景：用户需要新建函数、类、模块、脚本等代码。不适用：修改已有代码（用 incremental_edit）、分析代码（用 code_analyze）。',
    category: types_1.ToolCategory.CODE,
    parameters: {
        requirements: {
            type: 'string',
            description: '代码需求描述，如"实现一个快速排序函数"、"创建 Express 路由处理器"',
        },
        language: {
            type: 'string',
        ...
        complexity: {
            type: 'string',
            description: '代码复杂度: simple=简单函数, medium=中等模块, complex=复杂系统',
...
function createCodeGenerateExecutor(deps) {
    return async (params, _context) => {
        ...
        if (!deps.generateCode) {
            ...
            return {
                success: false,
                output: '',
                error: 'code_generate 不可用：代码生成已由 Python 真后端承接，TS harness 未注入 generateCode 实现。',
                duration: 0,
                validated: false,
        ...
        try {
            ...
            const scan = (0, codeShared_1.scanGeneratedCode)(result.code);
            if (scan.warnings.length > 0) {
                Logger_1.Logger.warn(`🔍 D3: code_generate 安全扫描告警: ${scan.warnings.join('; ')}`, 'CodeGenerate');
            }
            return {
                success: true,
                output,
...
function generateCodeTemplate(requirements, language, framework, complexity) {
...
function extractIdentifierName(requirements) {
...
function generateTsTemplate(name, requirements, framework, complexity) {
...
function generatePyTemplate(name, requirements, complexity) {
    const isComplex = complexity === 'complex';
    const className = toPascalCase(name);
    if (isComplex) {
        return `"""${requirements}"""
...
function generateGenericTemplate(name, requirements, language) {
    const fnName = toCamelCase(name);
    return `// ${requirements}
// Language: ${language}
