        ...
        try {
            ...
            if (bridge) {
                try {
                    ...
                    if (pyRes?.success) {
                        Logger_1.Logger.info(`🔍 web_search(Python): "${query}" (${Date.now() - startTime}ms)`, 'WebSearch');
                        return {
                            success: true,
                    ...
                    Logger_1.Logger.warn(`⚠️ Python web_search 逻辑失败, 降级 TS 本地: ${pyRes?.error || 'unknown'}`, 'WebSearch');
                }
                catch (pyErr) {
                    Logger_1.Logger.warn(`⚠️ Python web_search 代理失败, 降级 TS 本地: ${pyErr.message}`, 'WebSearch');
                }
            }
            let results;
