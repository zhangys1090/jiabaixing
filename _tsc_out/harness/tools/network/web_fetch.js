...
const WEB_FETCH_MAX_BUFFER_CHARS = Number(process.env.WEB_FETCH_MAX_BUFFER_CHARS) || 16 * 1024 * 1024; // 16MB 硬上限, 防内存 DoS
...
function isSafeUrl(rawUrl) {
    ...
    if (numeric !== null)
        return isSafeIPv4(numeric);
    // IPv6 字面量
    if (host.includes(':')) {
        const mapped = host.match(/^::ffff:(\d+\.\d+\.\d+\.\d+)$/);
        if (mapped)
            return isSafeIPv4(mapped[1]); // IPv4-mapped IPv6
        if (host === '::1' || // 回环
            host === '::' || // 未指定
 ...
 */
function parseMixedBaseIpv4(host) {
...
function isSafeIPv4(ip) {
    ...
    const [a, b] = parts;
    if (a === 0)
        return false; // 0.0.0.0/8 未指定
    if (a === 127)
        return false; // 127.0.0.0/8 回环
    if (a === 10)
        return false; // 10.0.0.0/8 私有
    if (a === 172 && b >= 16 && b <= 31)
        return false; // 172.16.0.0/12 私有
    if (a === 192 && b === 168)
        return false; // 192.168.0.0/16 私有
    if (a === 169 && b === 254)
        return false; // 169.254.0.0/16 链路本地(含元数据)
    if (a === 100 && b >= 64 && b <= 127)
...
function createWebFetchExecutor(deps = {}) {
    return async (params, _context) => {
        ...
        try {
            ...
            if (!isSafeUrl(url)) {
                Logger_1.Logger.warn(`🚫 web_fetch SSRF 拦截: ${url}`, 'WebFetch');
                return fail('拒绝访问本地/内网/链路本地/云元数据地址（SSRF 防护）', Date.now() - startTime);
            }
            let html;
            if (deps.httpClient) {
                // 注入的 httpClient 为不透明实现, 无法保证其自管超时与尺寸限制;
                // 此处加竞速超时(避免永久挂起)与事后缓冲截断(降低内存 DoS 面)。
                html = await withTimeout(deps.httpClient.get(url), WEB_FETCH_TIMEOUT_MS, 'httpClient 请求');
            }
            else {
            ...
            if (html && html.length > WEB_FETCH_MAX_BUFFER_CHARS) {
                Logger_1.Logger.warn(`⚠️ web_fetch 响应超长(${html.length}字符), 截断至 ${WEB_FETCH_MAX_BUFFER_CHARS}`, 'WebFetch');
