...
exports.NATURAL_SCHEDULE_DEF = void 0;
exports.createNaturalScheduleExecutor = createNaturalScheduleExecutor;
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
const Logger_1 = require("../../../utils/Logger");
const types_1 = require("../../types");
exports.NATURAL_SCHEDULE_DEF = {
    name: 'natural_schedule',
    description: '用自然语言创建定时任务。USE WHEN: 用户说"每天早上9点"、"每周一"、"每小时"、"工作日提醒我"。DO NOT USE WHEN: 用户要设置一次性提醒（用reminder_set）。支持: 每天/每周/每小时/工作日/周末/自定义时间。',
...
function loadTasks() {
    try {
        ...
        Logger_1.Logger.warn(`定时任务加载失败，已忽略损坏的 scheduled-tasks.json: ${String(e)}`, 'NaturalSchedule');
    }
}
function saveTasks() {
    try {
...
function createNaturalScheduleExecutor() {
    return async (params, _context) => {
        ...
        try {
            ...
            tasks.set(id, task);
            saveTasks();
            Logger_1.Logger.info(`📅 自然语言调度: "${description}" → ${parsed.cron} (${parsed.description})`, 'NaturalSchedule');
            return {
                success: true,
