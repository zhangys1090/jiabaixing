...
exports.CALENDAR_DEF = void 0;
exports.createCalendarExecutor = createCalendarExecutor;
const types_1 = require("../../types");
const storeLock_1 = require("./storeLock");
exports.CALENDAR_DEF = {
    name: 'calendar',
...
function createCalendarExecutor(deps) {
    return async (params, _context) => {
        ...
        if (!deps.calendarStore) {
            return {
                success: false,
                output: null,
                error: 'calendar 不可用: calendarStore 未注入，请在 initHarness 中配置日程存储依赖',
                duration: 0,
                validated: false,
            };
        }
            ...
            switch (action) {
                case 'create_event': {
                    ...
                    const event = {
                        ...
                        reminderMinutes: typeof params.reminder_minutes === 'number'
                            ? params.reminder_minutes
                            : 15,
                        createdAt: Date.now(),
                    };
                    return (0, storeLock_1.withStoreLock)(deps.calendarStore, async () => {
                        ...
                        return {
                            success: true,
                            output: `📅 日程已创建: [${event.id}] ${event.title}\n` +
                                `时间: ${timeDisplay}\n` +
                ...
                case 'get_today': {
                    ...
                    const sorted = [...todayEvents].sort((a, b) => new Date(a.startTime).getTime() - new Date(b.startTime).getTime());
                    const eventList = sorted
                        .map((e) => {
                        const timeDisplay = e.allDay
                            ? '全天'
                ...
                case 'get_week': {
                    ...
                    for (const day of weekdays) {
                        ...
                        if (dayEvents.length === 0) {
                            ...
                            const sorted = [...dayEvents].sort((a, b) => new Date(a.startTime).getTime() -
                ...
                case 'check_conflict': {
                    ...
                    const conflictList = conflicts
                        .map((e) => `${formatDateTime(e.startTime)} - ${formatDateTime(e.endTime)}: ${e.title}`)
                        .join('\n');
                    return {
                        success: true,
                ...
                case 'set_reminder': {
                    const eventId = String(params.event_id || '');
                    const minutes = typeof params.reminder_minutes === 'number'
                        ? params.reminder_minutes
                        : 15;
                    if (!eventId) {
                        return {
