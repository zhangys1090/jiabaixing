...
exports.ToolReliabilityTracker = exports.ToolRegistry = exports.ToolHealAction = void 0;
const crypto_1 = require("crypto");
const CapabilityMetrics_1 = require("../../../monitoring/CapabilityMetrics");
...
const ToolCallGuard_1 = require("./ToolCallGuard");
const ToolMetadataEnhancer_1 = require("./ToolMetadataEnhancer");
const ToolRuntimeState_1 = require("./ToolRuntimeState");
/** P1-3/B1: 仅对瞬时网络/限流类错误重试 */
const RETRYABLE_ERROR_RE = /(ECONNRESET|ETIMEDOUT|ENOTFOUND|EAI_AGAIN|ECONNREFUSED|429|5\d{2}|timeout|timed out|rate.?limit|too many requests|service unavailable|bad gateway|gateway timeout|connection reset)/i;
/** B1: 默认启用重试的工具类别（网络/LLM 类） */
const RETRY_DEFAULT_TOOL_CATEGORIES = new Set([
    types_1.ToolCategory.NETWORK,
]);
/** B3: 强制熔断覆盖的付费/外部 API 工具 */
const PAID_EXTERNAL_TOOLS = new Set([
    'image_generate',
    'tts_speak',
    'web_search',
]);
...
var ToolHealAction;
(function (ToolHealAction) {
    ToolHealAction["PARAM_FIX"] = "param_fix";
    ToolHealAction["ALT_TOOL"] = "alt_tool";
    ToolHealAction["DEGRADE"] = "degrade";
    ToolHealAction["NONE"] = "none";
})(ToolHealAction || (exports.ToolHealAction = ToolHealAction = {}));
class ToolRegistry {
    tools = new Map();
    /** toOpenAITools() 缓存 */
    ...
    permissionGuard = new PermissionGuard_1.PermissionGuard();
    /** 是否启用执行前置校验（默认 true，生产环境可按需关闭） */
    enablePreChecks = true;
    // P2-4: 工具熔断器（Circuit Breaker）
    static CB_FAILURE_THRESHOLD = 5;
    static CB_RESET_TIMEOUT_MS = 60_000;
    runtimeState = new ToolRuntimeState_1.InMemoryToolRuntimeState();
    // C1: 统一结果缓存/去重/限速守卫（复用既有 ToolCallGuard；此前为死代码未接线）。
    // 仅对幂等且属 NETWORK/FILE/MEMORY 类的工具生效（逻辑在 execute 内按工具判定）。
    callGuard = new ToolCallGuard_1.ToolCallGuard();
    // B2: 自愈处理器（由 Python 后端经 bridge 注入；默认无 → 诚实失败）
    selfHealHandler = null;
    // C2: 每-agent 并发信号量（默认 4）
    static MAX_CONCURRENT_TOOLS = (() => {
        const v = parseInt(process.env['TOOL_MAX_CONCURRENT'] || '4', 10);
        return Number.isFinite(v) && v > 0 ? v : 4;
    })();
    metadataEnhancer = new ToolMetadataEnhancer_1.ToolMetadataEnhancer();
    /**
     * 注册工具
     */
    ...
    async execute(name, params, context) {
        ...
        if (cbState && cbState.state === 'open') {
            ...
            if (elapsed < ToolRegistry.CB_RESET_TIMEOUT_MS) {
                Logger_1.Logger.warn(`⚡ P2-4: 工具 ${name} 已熔断 (连续失败 ${cbState.failureCount} 次，${Math.ceil((ToolRegistry.CB_RESET_TIMEOUT_MS - elapsed) / 1000)}s 后半开)`, 'ToolRegistry');
                return {
                    success: false,
                    output: null,
            ...
            Logger_1.Logger.info(`⚡ P2-4: 工具 ${name} 进入半开状态，尝试恢复`, 'ToolRegistry');
        }
        ...
        if (this.enablePreChecks &&
            ...
            permResult = this.permissionGuard.check(name, tool.definition.requiredPermissions, tool.definition.riskLevel, context);
        ...
        if (this.enablePreChecks && permResult?.needsConfirmation) {
            const decision = await this.requestApprovalForTool(name, tool, params, context, permResult.reason);
            if (!decision.approved) {
                Logger_1.Logger.warn(`🔒 审批拒绝: ${name} (${decision.method}) — ${decision.reason}`, 'ToolRegistry');
                return {
                    success: false,
                    output: null,
        ...
        if (tool.definition.idempotent && name !== 'result_cache') {
            ...
            const c1Eligible = cat === types_1.ToolCategory.NETWORK ||
                cat === types_1.ToolCategory.FILE ||
                cat === types_1.ToolCategory.MEMORY;
            if (c1Eligible) {
        ...
        if (PAID_EXTERNAL_TOOLS.has(name)) {
            ...
            if (!quota.allowed) {
                Logger_1.Logger.warn(`💰 C2: ${quota.reason}`, 'ToolRegistry');
                return {
                    success: false,
                    output: null,
            ...
            if (cached) {
                Logger_1.Logger.info(`♻️ C2: ${name} 命中去重缓存，直接返回`, 'ToolRegistry');
                const deduped = {
                    ...cached,
                    duration: Date.now() - startTime,
        ...
        try {
            ...
            for (let attempt = 0; attempt <= maxRetries; attempt++) {
                ...
                if (!this.isRetryableError(attemptResult.error)) {
                    Logger_1.Logger.info(`🚫 B1: ${name} 错误不可重试，停止重试: ${attemptResult.error}`, 'ToolRegistry');
                    break;
                }
                ...
                Logger_1.Logger.info(`🔄 P1-4: 工具 ${name} 第 ${attempt + 1} 次重试 (延迟=${jitteredDelay.toFixed(2)}s)`, 'ToolRegistry');
    ...
    reliabilityTracker = new ToolReliabilityTracker();
    /**
     * 获取工具可靠性追踪器
     */
    ...
    updateCircuitBreaker(toolName, success) {
        ...
        if (success) {
            if (state.state === 'half-open') {
                ...
                Logger_1.Logger.info(`⚡ P2-4: 工具 ${toolName} 恢复正常，熔断器关闭`, 'ToolRegistry');
            }
            ...
            if (state.state === 'half-open' ||
                ...
                state.state = 'open';
                Logger_1.Logger.error(`⚡ P2-4: 工具 ${toolName} 连续失败 ${state.failureCount} 次，熔断器打开`, undefined, 'ToolRegistry');
            }
        }
    }
    /**
    ...
    resetCircuitBreaker(toolName) {
        ...
        if (state) {
            ...
            Logger_1.Logger.info(`⚡ P2-4: 工具 ${toolName} 熔断器已手动重置`, 'ToolRegistry');
            return true;
        }
        return false;
    }
     ...
     * - 上限 3（B1 约束）
     */
    computeMaxRetries(name, category) {
        const envRaw = process.env[`TOOL_RETRY_${name}`] ?? process.env['TOOL_RETRY_DEFAULT'];
     ...
     */
    async runSingleAttempt(name, params, context) {
        const tool = this.tools.get(name);
        if (!tool) {
            return {
                success: false,
        ...
        const startTime = Date.now();
        try {
            const result = await PerformanceMonitor_1.perf.measure(`tool.${name}`, () => Promise.race([
            ...
            if (tool.definition.category === types_1.ToolCategory.COGNITION) {
                // D2: 从 ToolContext 捕获会话标识, 便于转发到对应 Python 会话(缺失则 null, 诚实降级不转发)
                const sessionId = context.sessionId ??
                    context.metadata?.sessionId ??
                    null;
                EventBus_1.EventBus.emit('cognition_result', {
                    tool: name,
            ...
            if (finalResult.success &&
                ...
                name !== 'result_cache') {
                const cat = tool.definition.category;
                if (cat === types_1.ToolCategory.NETWORK ||
                    cat === types_1.ToolCategory.FILE ||
                    cat === types_1.ToolCategory.MEMORY) {
     ...
     */
    async maybeSelfHeal(name, params, context, failed) {
        if (failed.success || !this.selfHealHandler)
            return failed;
        try {
            ...
            if (heal.action === ToolHealAction.PARAM_FIX && heal.params) {
                Logger_1.Logger.info(`🩹 B2: ${name} 参数修正后重试一次`, 'ToolRegistry');
            ...
            if (heal.action === ToolHealAction.ALT_TOOL && heal.alternativeTool) {
                Logger_1.Logger.info(`🩹 B2: ${name} 降级至替代工具 ${heal.alternativeTool}`, 'ToolRegistry');
                return this.execute(heal.alternativeTool, heal.params ?? params, context);
            }
            if (heal.action === ToolHealAction.DEGRADE) {
                Logger_1.Logger.warn(`🩹 B2: ${name} 降级执行（部分能力可用）`, 'ToolRegistry');
                return {
                    ...failed,
                    success: true,
                    error: undefined,
            ...
            Logger_1.Logger.warn(`🩹 B2: 自愈处理异常 ${name}: ${e.message}`, 'ToolRegistry');
        }
        return failed;
    }
    /**
    ...
    sessionKey(context) {
        ...
        return (m.sessionId || context.userId || context.traceId || 'default');
    }
    agentKey(context) {
        const m = context.metadata || {};
        return m.agentId || 'default';
    }
    /**
     * C2: 每-agent 信号量（默认 4 并发），返回释放函数。
     */
    async acquireSemaphore(agentKey) {
     ...
     */
    checkQuota(toolName, context) {
        if (!PAID_EXTERNAL_TOOLS.has(toolName))
            return { allowed: true };
        const sessionKey = this.sessionKey(context);
        const today = new Date().toISOString().slice(0, 10);
        const key = `${sessionKey}:${toolName}`;
        const rawLimit = process.env[`TOOL_QUOTA_${toolName}`] ||
    ...
    bumpQuota(toolName, context) {
        ...
        if (rec && rec.date === today)
            ...
            this.runtimeState.setQuota(key, { date: today, count: 1 });
    }
    /**
     * C2: 付费工具成功后计配额 + 写去重缓存（按 日期+工具+参数哈希）。
     */
    afterPaidSuccess(toolName, params, context, result) {
        if (!PAID_EXTERNAL_TOOLS.has(toolName))
            return;
     ...
     */
    async requestApprovalForTool(name, tool, params, context, reason) {
        try {
            ...
            Logger_1.Logger.error('❌ ApprovalEngine 调用失败，fail-closed 拒绝执行', err, 'ToolRegistry');
            return {
                approved: false,
                method: 'deny',
    ...
    extractApprovalTarget(name, params) {
        const pick = (...keys) => {
            for (const k of keys) {
                const v = params[k];
                if (typeof v === 'string')
     ...
     */
    recordCapability(toolName, category, success) {
        try {
            CapabilityMetrics_1.capMetrics.record(category, success);
        }
        catch (err) {
            Logger_1.Logger.debug(`⚠️ D4: capMetrics 记录失败 (${toolName}): ${err.message}`, 'ToolRegistry');
        }
    }
