...
exports.SceneToToolsetMapper = void 0;
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
const Logger_1 = require("../../../utils/Logger");
const SCENE_TOOLSET_MAP = {
    coding: {
        toolsetId: 'coding',
        disclosureLevel: 3,
...
class SceneToToolsetMapper {
        ...
        try {
            if (fs.existsSync(jsonPath)) {
                ...
                if (data.toolsetMap && data.keywords) {
                    Logger_1.Logger.info(`从 ${jsonPath} 加载场景-工具集映射`, 'SceneToToolsetMapper');
                    return { toolsetMap: data.toolsetMap, keywords: data.keywords };
                }
            }
        }
        catch (err) {
            Logger_1.Logger.warn(`加载场景配置失败 (${jsonPath}): ${err.message}，使用内置默认值`, 'SceneToToolsetMapper');
        }
    ...
    detectScene(input, env) {
        ...
        if (this.envState.foregroundApp) {
            ...
            if ([
                ...
                Logger_1.Logger.debug(`环境感知: 前台进程 "${this.envState.foregroundApp}" → coding`, 'SceneToToolsetMapper');
                return 'coding';
            }
        }
        // 关键词加权匹配
        ...
        Logger_1.Logger.debug(`场景检测: "${text.slice(0, 40)}" → ${best[0]} (score=${best[1]})`, 'SceneToToolsetMapper');
        return best[0];
    }
    /**
     * 场景 → 工具集配置
     ...
     * @returns 调整后的配置
     */
    applyDisclosureLevel(input, baseConfig) {
        ...
        // 中等任务: 暴露到 Level 2
        // 复杂任务: 暴露到 Level 3
        const adjustedLevel = Math.min(baseConfig.disclosureLevel, complexity === 1 ? 1 : complexity === 2 ? 2 : 3);
        const result = {
            ...baseConfig,
            disclosureLevel: adjustedLevel,
        };
        Logger_1.Logger.debug(`渐进式披露: complexity=${complexity}, level ${baseConfig.disclosureLevel}→${adjustedLevel}`, 'SceneToToolsetMapper');
        return result;
    }
    /**
...
exports.SceneToToolsetMapper = SceneToToolsetMapper;
