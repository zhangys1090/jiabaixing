...
const blockedProvider = async (action) => {
...
describe('ToolCallGuard 宪法守卫 (U4 第2项)', () => {
    it('未设置 provider 时 guard 等价于 check', async () => {
        const g = new ToolCallGuard_1.ToolCallGuard();
    ...
    it('provider 拦截破坏性动作', async () => {
        ...
        expect(r.result?.metadata?.constitutionBlocked).toBe(true);
        expect(r.result?.metadata?.violations).toContain('danger_blocks_destructive');
    });
    ...
    it('provider 放行后,check 的去重逻辑仍然生效', async () => {
        ...
        expect(second.result?.metadata?.deduplicated).toBe(true);
    });
});
