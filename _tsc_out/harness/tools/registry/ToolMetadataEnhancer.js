...
exports.ToolMetadataEnhancer = void 0;
...
class ToolMetadataEnhancer {
    enhancedMetadata = new Map();
    ...
    recordUsage(toolName, success, durationMs) {
    ...
    getEnhancedMetadata(toolName) {
        return this.enhancedMetadata.get(toolName) ?? null;
    }
    getAllEnhancedMetadata() {
        return new Map(this.enhancedMetadata);
    }
    recommendTools(context, recentToolCalls = [], limit = 3) {
    ...
    buildEnhancedMetadata(definition) {
        ...
        const inputPatterns = Object.entries(definition.parameters).map(([name, param]) => `${name}: ${param.description}`);
        return {
    ...
    findRelatedTools(definition) {
        const related = [];
        for (const [name, otherDef] of this.toolDefinitions) {
            if (name === definition.name)
                continue;
            const sharedTags = (definition.tags ?? []).filter((t) => (otherDef.tags ?? []).includes(t));
    ...
    updateSearchIndex(toolName, metadata) {
    ...
    computeRelevanceScore(query, queryTokens, definition, metadata) {
        let score = 0;
        const reasons = [];
        ...
        const tagMatchCount = (definition.tags ?? []).filter((tag) => queryTokens.some((t) => tag.toLowerCase().includes(t))).length;
        if (tagMatchCount > 0) {
...
exports.ToolMetadataEnhancer = ToolMetadataEnhancer;
