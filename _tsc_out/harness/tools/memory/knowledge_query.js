                ...
                try {
                    ...
                    if (web.length > 0) {
                        ...
                        for (const item of web.slice(0, maxResults)) {
                            results.push(`  • ${item.content.substring(0, 200)}${item.source ? ` (${item.source})` : ''}`);
                        }
                        results.push('');
                        ...
                        if (deps.memoryStore) {
                            ...
                            try {
                                ...
                                Logger_1.Logger.debug(`⚠️ D1: 记忆回填失败: ${be.message}`, 'KnowledgeQuery');
                            }
                        }
                    }
                }
                catch (we) {
                    Logger_1.Logger.warn(`⚠️ D1: web_search 降级失败: ${we.message}`, 'KnowledgeQuery');
                }
            }
            if (results.length === 0) {
                return {
