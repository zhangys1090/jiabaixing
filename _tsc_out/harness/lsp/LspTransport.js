...
class LspTransport extends events_1.EventEmitter {
    ...
    buffer = '';
    requestTimeout;
    static MAX_BUFFER_SIZE = 10 * 1024 * 1024;
    ...
    handleData(data) {
        ...
        if (this.buffer.length > LspTransport.MAX_BUFFER_SIZE) {
            Logger_1.Logger.warn('LspTransport', `缓冲区超过 ${LspTransport.MAX_BUFFER_SIZE} 字节，截断`);
            this.buffer = this.buffer.substring(this.buffer.length - Math.floor(LspTransport.MAX_BUFFER_SIZE / 2));
        }
