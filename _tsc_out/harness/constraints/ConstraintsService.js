...
exports.ConstraintsService = void 0;
...
class ConstraintsService {
     ...
     * 检查权限
     */
    checkPermission(toolName, requiredPermissions, riskLevel, context) {
        const result = this.deps.permissionGuard.check(toolName, requiredPermissions, riskLevel, context);
        return {
     ...
     */
    checkSafetyBoundary(input, action) {
        // 检查输入长度
        if (input.length > 10000) {
            return { allowed: false, reason: '输入过长，可能存在注入攻击' };
        }
        // 委托给统一危险命令检测器
        const cmdCheck = (0, SensitiveDetector_1.checkDangerousCommand)(action);
        if (cmdCheck.dangerous) {
            return { allowed: false, reason: cmdCheck.reason || '禁止执行危险操作' };
        }
        return { allowed: true };
    }
    static LIFECYCLE_TO_HOOK_EVENT = {
    ...
    registerHook(event, hook) {
        ...
        if (this.deps.hookManager) {
            const mappedEvent = ConstraintsService.LIFECYCLE_TO_HOOK_EVENT[event];
            if (mappedEvent) {
                this.deps.hookManager.registerHook(mappedEvent, hook);
                Logger_1.Logger.debug(`委托 HookManager 注册钩子: ${event} → ${mappedEvent}`, 'ConstraintsService');
                return;
            }
            // 不可映射的事件回退到本地注册
            Logger_1.Logger.debug(`事件 ${event} 无 HookManager 映射，回退本地注册`, 'ConstraintsService');
        }
        const existing = this.hooks.get(event) || [];
        existing.push(hook);
        this.hooks.set(event, existing);
        Logger_1.Logger.debug(`注册生命周期钩子: ${event}`, 'ConstraintsService');
    }
    /**
     * 执行生命周期钩子
     */
    async executeHooks(event, context) {
        ...
        for (const hook of hooks) {
            try {
                ...
                if (!result.proceed) {
                    Logger_1.Logger.info(`🛑 钩子拦截: ${event} - ${result.reason || '未提供原因'}`, 'ConstraintsService');
                    return result;
                }
                // 应用修改
                ...
                Logger_1.Logger.warn(`钩子执行失败: ${event} - ${err.message}`, 'ConstraintsService');
            }
        }
        return { proceed: true };
    }
     ...
     */
    enforceBehaviorConstraint(constraint, context) {
        const ctx = context;
        switch (constraint) {
            case 'no-unbounded-recursion': {
                const recursionDepth = ctx?.params?.recursionDepth || 0;
                const maxDepth = 10;
            ...
            case 'no-sensitive-data-leak': {
                ...
                if (output) {
                    ...
                    // 委托给统一敏感信息检测器
                    const result = (0, SensitiveDetector_1.checkSensitiveInfo)(outputStr, 'output');
                    if (!result.safe) {
                        const topViolation = result.violations[0];
                        // 密码和密钥类违规合并显示为"密码/密钥"
                        const displayName = topViolation?.name === '密码泄露' ||
            ...
            case 'no-sensitive-storage': {
                ...
                const allParamValues = Object.values(ctx?.params || {})
                    .map((v) => String(v))
                    .join(' ');
                const content = String(ctx?.params?.content || ctx?.params?.text || '') +
                    ' ' +
                    allParamValues;
                if (!content.trim())
                    return { compliant: true };
                // 委托给统一敏感信息检测器（storage 场景使用更严格的模式）
                const result = (0, SensitiveDetector_1.checkSensitiveInfo)(content, 'storage');
                if (!result.safe) {
                    ...
                    const topViolation = result.violations.find((v) => specificNames.includes(v.name)) ||
                        result.violations[0];
                    return {
                        compliant: false,
            ...
            case 'no-dangerous-commands': {
                const cmd = ctx?.params?.command ||
                    ctx?.params?.script ||
                    '';
                // 委托给统一危险命令检测器
                const result = (0, SensitiveDetector_1.checkDangerousCommand)(cmd);
                if (result.dangerous) {
                    return {
                        compliant: false,
            ...
            case 'resource-limit-check': {
                const memoryUsage = ctx?.params?.memoryMB || 0;
                const maxMemoryMB = 512;
                const cpuTime = ctx?.params?.cpuTimeMs || 0;
     ...
     * 获取约束等级
     */
    getConstraintLevel(constraintName) {
        const def = ConstraintsService.CONSTRAINT_DEFINITIONS.find((d) => d.name === constraintName);
     ...
     * 分级执行约束 — 硬约束拦截，软约束仅警告
     */
    enforceWithLevel(constraint, context) {
        ...
        if (!result.compliant && level === 'soft') {
            Logger_1.Logger.warn(`⚠️ 软约束建议: ${result.violation}`, 'ConstraintsService');
    ...
    // ===== 自适应预算 =====
    /** 默认自适应预算配置 */
    static DEFAULT_ADAPTIVE_BUDGET = {
        simple: {
            maxRounds: 4,
            maxToolCalls: 5,
            maxTokens: 3000,
    ...
    /** 默认创造性探索配置 */
    static DEFAULT_CREATIVE_CONFIG = {
        enabled: true,
        maxExtraToolCalls: 4,
        maxExtraRounds: 3,
     ...
     * @returns 预算分配
     */
    resolveAdaptiveBudget(complexity, enableCreative = false) {
     ...
     */
    canExploreCreatively(currentQuality, budgetState) {
     ...
     * @returns 压力评估结果
     */
    getBudgetPressure(budget) {
        const rounds = budget.hardRoundLimit > 0 ? budget.roundsUsed / budget.hardRoundLimit : 0;
        const tokens = budget.tokenHardLimit > 0 ? budget.tokensUsed / budget.tokenHardLimit : 0;
        const toolCalls = budget.maxToolCalls > 0 ? budget.toolCallsUsed / budget.maxToolCalls : 0;
        const elapsed = Date.now() - budget.startTime;
        const duration = budget.maxDurationMs > 0 ? elapsed / budget.maxDurationMs : 0;
    ...
    buildCautionWarning(dimension, budget) {
        const labels = {
    ...
    buildCriticalWarning(dimension, budget) {
        const labels = {
            rounds: `轮次 ${budget.roundsUsed}/${budget.hardRoundLimit}`,
            tokens: `Token ${budget.tokensUsed}/${budget.tokenHardLimit}`,
            toolCalls: `工具调用 ${budget.toolCallsUsed}/${budget.maxToolCalls}`,
