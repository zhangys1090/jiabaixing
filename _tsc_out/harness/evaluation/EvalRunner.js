...
class EvalRunner {
    ...
    async runCategory(cases, category) {
    ...
    async runSingleCase(testCase) {
        const startTime = Date.now();
        let output = '';
        let toolCallsUsed = 0;
        let stepEvaluatorScore;
        try {
    ...
    async judgeWithLLM(testCase, output) {
...
请评估并返回JSON结果。`;
        try {
            const judgeResponse = await this.llm.chat(judgeUserPrompt, judgeSystemPrompt);
            ...
            return {
                passed: false,
                score: 0,
                reasoning: `Judge LLM调用失败: ${error.message}`,
            };
        }
    }
    ...
    generateReport(runId, results, duration) {
