...
describe('SubAgentFanout W7/W8 traceId + 感知模板', () => {
    it('透传指定 traceId 到 FanoutResult 与每个 SubTaskResult', async () => {
        const fanout = new SubAgentFanout_1.SubAgentFanout(makeRegistry());
    ...
    it('把 traceId 注入执行器收到的任务元数据', async () => {
        let received = null;
        const fanout = new SubAgentFanout_1.SubAgentFanout(makeRegistry(), async (t) => {
            received = t;
            return { ok: true };
        });
        const result = await fanout.fanout('parent', [makeTask()], undefined, {
        ...
        expect(received.metadata?.traceId).toBe('trace-inject');
    ...
    it('预置感知型子 Agent 模板齐全', () => {
        expect(Object.keys(SubAgentFanout_1.PERCEPTION_AGENT_TEMPLATES).sort()).toEqual([
            'desktop_automation',
            'device_control',
            'visual_operator',
        ]);
        expect(SubAgentFanout_1.PERCEPTION_AGENT_TEMPLATES.visual_operator.modalities).toContain('visual');
        expect(SubAgentFanout_1.PERCEPTION_AGENT_TEMPLATES.device_control.modalities).toContain('environment');
    });
});
