...
请生成一段100字以内的执行摘要。不要编造不存在的任务或结果。`;
    ...
    detectConflicts(agentResults, taskNodes) {
        ...
        for (const task of taskNodes) {
            ...
            const resource = resultObj.resource || resultObj.resourceId || resultObj.target;
        ...
        for (let i = 0; i < completedTasks.length; i++) {
            for (let j = i + 1; j < completedTasks.length; j++) {
                ...
                if (resultA &&
                    ...
                    const a = resultA;
                    const b = resultB;
                    const sharedKeys = Object.keys(a).filter((k) => k in b && k !== 'taskId' && k !== 'completedAt');
...
请以 JSON 格式返回仲裁结果，包含 winnerTaskId（获胜任务ID）和 reasoning（仲裁理由）。`;
            try {
                ...
                const winnerTaskId = typeof rawWinner === 'string' &&
