...
class SubAgentFanout {
    ...
    async fanout(parentTaskId, subTasks, configOverride, options) {
        ...
        const truncatedTaskIds = [];
        if (subTasks.length > runConfig.maxFanout) {
            const truncated = subTasks.slice(runConfig.maxFanout);
            for (const t of truncated) {
                truncatedTaskIds.push(t.id);
            }
            Logger_1.Logger.warn(`⚠️ 子任务数 ${subTasks.length} 超过扇出限制 ${runConfig.maxFanout}，截断 ${truncatedTaskIds.length} 个任务: [${truncatedTaskIds.join(', ')}]`, 'SubAgentFanout');
        }
        ...
        switch (strategy) {
            ...
            default:
                {
                    const independentTasks = limitedTasks.filter((t) => t.dependencies.length === 0);
                    const dependentTasks = limitedTasks.filter((t) => t.dependencies.length > 0);
                    const allResults = [];
                    if (independentTasks.length > 0) {
                        const parallelResults = await this.executeParallel(parentTaskId, independentTasks, runConfig, traceId);
                        allResults.push(...parallelResults);
                    }
                    if (dependentTasks.length > 0) {
                        const sequentialResults = await this.executeSequential(parentTaskId, dependentTasks, runConfig, traceId);
        ...
        return {
            ...
            traceId,
            truncatedTaskIds: truncatedTaskIds.length > 0 ? truncatedTaskIds : undefined,
        };
    }
    /**
     * 并行执行所有子任务
    ...
    async executeSubTask(parentTaskId, task, config, traceId) {
        ...
        try {
            const timeoutPromise = new Promise((_, reject) => {
                setTimeout(() => reject(new Error(`子任务超时 (${config.taskTimeoutMs}ms)`)), config.taskTimeoutMs);
            });
