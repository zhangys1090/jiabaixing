...
class AgentRegistry {
    static KNOWLEDGE_MAX_ENTRIES = 5000;
    static KNOWLEDGE_TRIM_TO = 4000;
    static NEGOTIATION_MAX_SESSIONS = 200;
    static NEGOTIATION_TRIM_TO = 150;
    agents = new Map();
    healthMap = new Map();
    /**
     * 注册Agent
    ...
    knowledgeSubscriptions = new Map();
    publishKnowledge(input) {
        ...
        if (this.knowledgeEntries.size > AgentRegistry.KNOWLEDGE_MAX_ENTRIES) {
            const sorted = Array.from(this.knowledgeEntries.entries()).sort(([, a], [, b]) => a.createdAt - b.createdAt);
            const toRemove = sorted.slice(0, this.knowledgeEntries.size - AgentRegistry.KNOWLEDGE_TRIM_TO);
            for (const [removeId] of toRemove) {
                this.knowledgeEntries.delete(removeId);
            }
            Logger_1.Logger.info(`💾 P1-5: 知识库已裁剪 ${toRemove.length} 条旧条目 (当前=${this.knowledgeEntries.size})`, 'AgentRegistry');
        }
    ...
    persistKnowledgeEntry(entry) {
        if (!this.knowledgePersistDir)
            return;
        try {
            const filePath = path.join(this.knowledgePersistDir, `${entry.id}.json`);
            fs.writeFileSync(filePath, JSON.stringify(entry, null, 2), 'utf-8');
            Logger_1.Logger.debug(`💾 P1-7: 知识条目已持久化: ${entry.id}`, 'AgentRegistry');
        }
        catch (err) {
            Logger_1.Logger.warn(`💾 P1-7: 知识条目持久化失败: ${err.message}`, 'AgentRegistry');
        }
    }
    /**
     * P1-7: 从持久化目录加载知识条目
    ...
    loadPersistedKnowledge() {
        ...
        try {
            ...
            for (const file of files) {
                try {
                    const raw = fs.readFileSync(path.join(this.knowledgePersistDir, file), 'utf-8');
            ...
            if (loaded > 0) {
                Logger_1.Logger.info(`💾 P1-7: 从持久化目录加载 ${loaded} 条知识条目`, 'AgentRegistry');
            }
            return loaded;
        }
        catch {
    ...
    startNegotiation(initiatorId, participants, topic, payload) {
        ...
        if (this.negotiationSessions.size > AgentRegistry.NEGOTIATION_MAX_SESSIONS) {
            const completedOrFailed = Array.from(this.negotiationSessions.entries())
                .filter(([, s]) => s.status !== 'open')
                .sort(([, a], [, b]) => (a.id > b.id ? 1 : -1));
            const toRemove = completedOrFailed.slice(0, this.negotiationSessions.size - AgentRegistry.NEGOTIATION_TRIM_TO);
...
class A2AProtocolManager {
     ...
     * 实现双端 A2A 能力统一。
     */
    setPythonBridge(client, preferPython = true) {
        this.pythonBridge = client;
        this.preferPython = preferPython;
        Logger_1.Logger.info(`🔗 A2AProtocolManager: Python 桥接已注入 (preferPython=${preferPython})`, 'A2AProtocol');
    }
    /**
     * 发布 Agent Card
     */
    async publishAgentCard(card) {
        ...
        if (this.pythonBridge && this.preferPython) {
            try {
                ...
                Logger_1.Logger.warn(`⚠️ A2A Agent Card 同步到 Python 失败: ${e.message}`, 'A2AProtocol');
            }
        }
    }
    /**
    ...
    async createTask(input) {
        ...
        if (this.pythonBridge && this.preferPython) {
            try {
                ...
                Logger_1.Logger.info(`📋 A2A Task 创建(Python): ${remoteTask.id} (${input.fromAgentId} → ${input.toAgentId})`, 'A2AProtocol');
                return remoteTask;
            }
            catch (e) {
                Logger_1.Logger.warn(`⚠️ Python A2A Task 创建失败，降级到本地: ${e.message}`, 'A2AProtocol');
            }
        }
        // 本地兜底
        ...
        Logger_1.Logger.info(`📋 A2A Task 创建(本地): ${task.id} (${input.fromAgentId} → ${input.toAgentId})`, 'A2AProtocol');
