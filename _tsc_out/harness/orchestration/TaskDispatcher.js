        ...
        if (task.assignedTo) {
            ...
            if (agent) {
                if (agent.status === 'idle') {
                    Logger_1.Logger.debug(`使用 OrchestratorAgent 分配: ${task.id} → ${agent.name}`, 'TaskDispatcher');
                    return agent;
                }
                Logger_1.Logger.warn(`OrchestratorAgent 分配的 Agent 不空闲: ${task.assignedTo} (${agent.status})，回退自动分配`, 'TaskDispatcher');
            }
            else {
                Logger_1.Logger.warn(`OrchestratorAgent 分配的 Agent 不存在: ${task.assignedTo}，回退自动分配`, 'TaskDispatcher');
            }
        }
        if (task.agentId) {
