...
class JiabaixingCore {
    ...
    performanceMonitor;
    securityAuditor;
    traeOptimizationIntegrator = null;
    // 反馈收集器 — 闭合 Loop B（进化反馈经 Python 后端 python/agent/evolution 采集）
    feedbackCollector = new FeedbackCollector_1.FeedbackCollector();
    optimizationSchedulerManager;
    ...
    // RL 训练轨迹导出器
    trajectoryExporter = new TrajectoryExporter_1.TrajectoryExporter();
    trajectoryBuffer = [];
    memoryLeakGuard = MemoryLeakGuard_1.MemoryLeakGuard.getInstance();
    // 项目上下文文件缓存
    _contextFileCache = [];
    ...
    async processInput(input, userId, traceId, images) {
        ...
        try {
            ...
            // ═══════════════════════════════════════════════════════════════
            Logger_1.Logger.warn('⚠️ Harness 不可用，尝试直接LLM调用', 'JiabaixingCore');
            let fallbackResponse = '';
            try {
                const llm = this.getLLM();
                if (llm && llm.isServiceAvailable()) {
                    fallbackResponse = await llm.chat(input);
                    Logger_1.Logger.info('✅ 直接LLM调用成功', 'JiabaixingCore');
                }
            }
            catch (_llmErr) {
                Logger_1.Logger.warn('⚠️ 直接LLM调用也失败，使用硬编码降级', 'JiabaixingCore');
            }
