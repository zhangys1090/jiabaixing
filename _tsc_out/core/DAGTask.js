...
class TaskNode {
    ...
    startTime; // 开始执行时间
    endTime; // 结束执行时间
    estimatedTime = 0; // 预计执行时间（秒）
    priority = TaskPriority.MEDIUM; // 任务优先级
    maxRetries = 3; // 最大重试次数
    currentRetry = 0; // 当前重试次数
    timeout = 300; // 超时时间（秒）
    retryDelay = 1; // 重试延迟（秒）
    metadata = {}; // 任务元数据
    isEssential = true; // 是否为关键步骤（默认是）
...
class DAGTask {
    name; // 任务图名称
    nodes = new Map(); // 任务节点集合
    adjacencyList = new Map(); // 邻接表（依赖关系）
    eventHandlers = new Map(); // 事件处理器
    maxParallelTasks = 4; // 最大并行任务数
    runningTasks = new Set(); // 正在执行的任务
    _dirty = false;
    _cachedTopoOrder = null;
