...
class TaskComplexityAnalyzer {
    ...
    /** 实际执行轮次历史 — key 为任务描述 */
    actualRoundsHistory = new Map();
    /** 实际执行时长历史 — key 为任务描述 */
    actualDurationHistory = new Map();
    /** 预测准确率记录 */
    predictionRecords = [];
    /** 置信度校准数据 — key 为置信度桶（如 "0.9"） */
    confidenceCalibrationMap = new Map();
    /** LLM 依赖 */
    llmDeps = null;
    static HISTORY_MAX_PER_TASK = 50;
    static PREDICTION_RECORDS_MAX = 500;
    static HISTORY_MAX_KEYS = 200;
    /**
     * 分析任务复杂度
     */
    ...
    _trimHistoryKeys(map) {
        if (map.size <= TaskComplexityAnalyzer.HISTORY_MAX_KEYS)
            return;
        const keys = Array.from(map.keys());
        const toRemove = keys.slice(0, map.size - TaskComplexityAnalyzer.HISTORY_MAX_KEYS);
    ...
    recordPredictionAccuracy(task, predicted, actual) {
        ...
        if (this.predictionRecords.length >
            TaskComplexityAnalyzer.PREDICTION_RECORDS_MAX) {
            this.predictionRecords.splice(0, this.predictionRecords.length -
                TaskComplexityAnalyzer.PREDICTION_RECORDS_MAX);
        }
    }
    /**
    ...
    async analyzeComplexityWithLLM(task) {
        ...
        try {
            ...
            const estimatedSteps = typeof parsed.estimatedSteps === 'number' &&
