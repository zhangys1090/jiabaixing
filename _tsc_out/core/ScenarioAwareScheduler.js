...
const GoalImpactEvaluator_1 = require("../authority/GoalImpactEvaluator");
const ReplanEvaluator_1 = require("../authority/ReplanEvaluator");
...
class ScenarioAwareScheduler {
    ...
    // 环境感知缓存
    lastSnapshot = null;
    lastForegroundCheck = 0;
    FOREGROUND_CHECK_INTERVAL = 15000;
    lastEnv = '';
    // Git 感知
    WATCHED_DIRS = [
    ...
    lastGitState = new Map();
    gitCheckCount = 0;
    GIT_CHECK_INTERVAL = 10; // 每10次检查（约5分钟）做一次git感知
    projectChangeHistory = [];
    MAX_CHANGE_HISTORY = 50;
    // 文件变更监听
    fileWatchers = [];
    watchedDirectories = [];
    fileChangeRules = new Map();
    fileDebounceMap = new Map();
    ...
    fileChangeLog = [];
    MAX_CHANGE_LOG = 200;
    lastEvaluatedFileChanges = new Set();
    lastEvaluatedEnv = null;
    lastEvaluatedGitKey = null;
    constructor() {
    ...
    proactiveCheckCount = 0;
    PROACTIVE_COOLDOWN_MS = 10 * 60 * 1000; // 10分钟冷却
    updateUserActivity() {
    ...
    start() {
        ...
        this.checkAndExecuteTasks().catch((err) => Logger_1.Logger.warn(`调度任务执行异常: ${err?.message ?? String(err)}`, 'ScenarioAwareScheduler'));
    ...
    async evaluateActiveGoalImpacts(envSnapshot) {
        try {
            ...
            if (currentEnvKey !== this.lastEvaluatedEnv && this.lastEvaluatedEnv !== null) {
                observations.push({
                    observationId: (0, GoalImpactEvaluator_1.generateObservationId)(),
                    source: 'environment',
            ...
            const evaluator = (0, GoalImpactEvaluator_1.getGoalImpactEvaluator)();
            const allImpacts = [];
            ...
            for (const impact of allImpacts) {
                if (impact.affected) {
                    Logger_1.Logger.info(`[D7-1] Goal ${impact.goalId} affected by ${impact.impactType}: ${impact.reason} (confidence=${impact.confidence.toFixed(2)})`, 'ScenarioAwareScheduler');
                    EventBus_1.EventBus.emit('goal_replan_suggested', {
                    ...
                    if (affectedGoal) {
                        ...
                        const replanEvaluator = (0, ReplanEvaluator_1.getReplanEvaluator)();
                        const replanRequest = replanEvaluator.evaluate(affectedGoal, impact, evidenceLog);
                        if (replanRequest) {
                            Logger_1.Logger.info(`[D7-2] ReplanRequest ${replanRequest.requestId} for Goal ${replanRequest.goalId} (plan v${replanRequest.planVersion})`, 'ScenarioAwareScheduler');
                            EventBus_1.EventBus.emit('goal_replan_requested', {
                            ...
                            try {
                                const autonomousStep = (0, AutonomousStep_1.getAutonomousStep)();
                                const stepResult = await autonomousStep.execute(replanRequest);
                                if (stepResult.success) {
                                    Logger_1.Logger.info(`[D7-3C] Autonomous step completed: Goal ${stepResult.goalId} evidence_delta=${stepResult.evidence.progressDelta.toFixed(2)}`, 'ScenarioAwareScheduler');
                                }
                                else {
                                    Logger_1.Logger.warn(`[D7-3C] Autonomous step partial failure: Goal ${stepResult.goalId} reason=${stepResult.reason}`, 'ScenarioAwareScheduler');
                                }
                                if (stepResult.replan.decision) {
                                    EventBus_1.EventBus.emit('goal_replan_executed', {
                                ...
                                Logger_1.Logger.error(`[D7-3C] AutonomousStep failed: ${stepErr.message}`, stepErr, 'ScenarioAwareScheduler');
                            }
                        }
                    }
                }
            ...
            Logger_1.Logger.error(`[D7-1] evaluateActiveGoalImpacts failed: ${err.message}`, err, 'ScenarioAwareScheduler');
        }
    }
    ...
    async executeTask(task) {
        ...
        try {
            ...
            if (task.naturalDescription) {
                Logger_1.Logger.info(`[D7-P0] Scheduled task due: "${task.naturalDescription}"${task.targetPlatform ? ' → ' + task.targetPlatform : ''} — emitting event, NOT executing`, 'ScenarioAwareScheduler');
                EventBus_1.EventBus.emit('scheduled_task_due', {
    ...
    onFileEvent(dirPath, eventType, filename) {
        ...
        const timer = setTimeout(() => {
            ...
            this.handleFileChange(filePath, changeType).catch((err) => Logger_1.Logger.warn(`文件变更处理异常: ${filePath} -> ${err?.message ?? String(err)}`, 'ScenarioAwareScheduler'));
    ...
    async executeRuleAction(rule, filePath, changeType, timestamp) {
        ...
        switch (rule.action) {
            case 'notify':
                ...
                break;
            case 'auto_fix':
                Logger_1.Logger.info(`[D7-P0] auto_fix candidate: ${path.basename(filePath)} ${changeType} — emitting event, NOT executing`, 'ScenarioAwareScheduler');
                EventBus_1.EventBus.emit('auto_fix_candidate', {
                    source: 'ScenarioAwareScheduler',
                    filePath,
                    changeType,
                    ruleName: rule.name,
                ...
                break;
            case 'run_tests':
                Logger_1.Logger.info(`[D7-P0] test_request_pending: ${path.basename(filePath)} ${changeType} — emitting event, NOT executing`, 'ScenarioAwareScheduler');
                EventBus_1.EventBus.emit('test_request_pending', {
                    source: 'ScenarioAwareScheduler',
                    filePath,
                    changeType,
                    ruleName: rule.name,
                ...
                break;
            case 'custom':
                Logger_1.Logger.info(`[D7-P0] custom_rule_triggered: ${path.basename(filePath)} ${changeType} — emitting event, NOT executing`, 'ScenarioAwareScheduler');
                EventBus_1.EventBus.emit('custom_rule_triggered', {
                    source: 'ScenarioAwareScheduler',
                    filePath,
                    changeType,
                    ruleName: rule.name,
