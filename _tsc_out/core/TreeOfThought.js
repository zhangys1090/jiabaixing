...
class TreeOfThoughtEngine {
    ...
    async reason(problem, options) {
        ...
        Logger_1.Logger.info(`🌳 ToT 推理启动: depth=${maxDepth}, branches=${branchCount}`, 'TreeOfThought');
        const root = {
            id: 'root',
            thought: problem,
            score: 0,
        ...
        const answer = bestPath.length > 0
            ...
            : '无法通过多路径推理得出结论';
        Logger_1.Logger.info(`🌳 ToT 推理完成: ${allNodes.length} 节点, 最佳路径深度=${bestPath.length}`, 'TreeOfThought');
