        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    log_ignored(log, "kernel_isolation.GVisorBackend.spawn", _exc)
                return KernelSandboxResult(
                    success=False,
        ...
        for key in ("rootfs", "config", "jailer_chroot"):
            ...
            if path and Path(path).exists():
                try:
                    ...
                    log_ignored(log, "kernel_isolation.FirecrackerBackend._cleanup_vm_files", _exc)
    ...
    async def destroy(self, vm_id: str) -> None:
        ...
        if proc is not None and proc.returncode is None:
            try:
                ...
                log_ignored(log, "kernel_isolation.FirecrackerBackend.destroy", _exc)
        self._cleanup_vm_files(vm_id)
...
class WindowsSandboxBackend(KernelIsolationBackend):
    ...
    async def is_available(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                log_ignored(log, "kernel_isolation.WindowsSandboxBackend.is_available", _exc)
    ...
    async def _check_windows_feature(self) -> bool:
        try:
            ...
            log_ignored(log, "kernel_isolation.WindowsSandboxBackend._check_windows_feature", _exc)
        try:
            import winreg
            key = winreg.OpenKey(
            ...
            while elapsed < config.timeout_sec:
                ...
                if exitcode_file.exists():
                    ...
                    try:
                        ...
                        log_ignored(log, "kernel_isolation.WindowsSandboxBackend.spawn", _exc)
                    if exit_code != 0:
                        try:
        ...
        for key in ("shared_dir", "wsb_path"):
            ...
            if path and Path(path).exists():
                try:
                    ...
                    log_ignored(log, "kernel_isolation.WindowsSandboxBackend._cleanup_sandbox_files", _exc)
    ...
    async def destroy(self, vm_id: str) -> None:
        ...
        if proc is not None and proc.returncode is None:
            try:
                ...
                log_ignored(log, "kernel_isolation.WindowsSandboxBackend.destroy", _exc)
        self._cleanup_sandbox_files(vm_id)
        ...
        for info in cls.list_backends():
            ...
            if backend is not None:
                try:
                    ...
                    log_ignored(log, "kernel_isolation.KernelIsolationProvider.destroy", _exc)
        duration_ms = (time.time() - start) * 1000
