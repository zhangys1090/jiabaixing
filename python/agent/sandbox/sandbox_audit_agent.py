...
# AuditSeverity 唯一定义在 agent/loop/evolution_feedback_auditor.py（修复：此前使用但未导入）
from agent.loop.evolution_feedback_auditor import AuditSeverity
...
class AuditReport(BaseAuditReport):
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            # findings.severity 是 AuditSeverity（info/warning/critical），
            # 不能与 RiskLevel（none/low/medium/high/critical）直接比较（值域不同）。
            "critical_count": sum(1 for f in self.findings if f.severity == AuditSeverity.CRITICAL),
            "warning_count": sum(1 for f in self.findings if f.severity == AuditSeverity.WARNING),
            "findings": [
                {
    ...
    async def stop(self) -> None:
        ...
        if self._task is not None:
            ...
            try:
                ...
                log_ignored(log, "sandbox_audit_agent.SandboxAuditAgent.stop", _exc)
            self._task = None
