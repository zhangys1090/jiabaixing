...
describe('P0: Independent Evaluator Agent', () => {
  ...
  describe('Evaluator Adapter - Loop Integration', () => {
    ...
    describe('evaluateFull - Full Trace Integration', () => {
      it('should extract tool results from trace trajectory', async () => {
        ...
        const result = await evaluator.evaluateFull('列出文件', [], trace);
      ...
      it('should handle empty trajectory', async () => {
        ...
        const result = await evaluator.evaluateFull('测试', [], trace);
    ...
    describe('Budget-based Evaluation', () => {
      it('should abort when hard round limit reached', async () => {
        const context = {
      ...
      it('should abort when tool call limit reached', async () => {
        const context = {
          messages: [] as ChatMessage[],
          plan: null,
          currentStepIndex: 0,
  ...
  describe('getIndependentEvaluationService - External Access', () => {
    it('should expose independent evaluation service', () => {
      const evaluator = new Evaluator({});
      const service = evaluator.getIndependentEvaluationService();

      expect(service).toBeInstanceOf(IndependentEvaluationService);
    });

    it('should allow direct service evaluation', async () => {
      const evaluator = new Evaluator({});
      const service = evaluator.getIndependentEvaluationService();
