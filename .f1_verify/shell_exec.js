...
exports.isShellCommandDangerous = isShellCommandDangerous;
 ...
 *      检测不到的绕过。
 */
function isShellCommandDangerous(rawCommand) {
    ...
    if ((cmdName === 'powershell' || cmdName === 'pwsh') &&
        tokens.some((t) => t === '-enc' || t === '-encodedcommand' || t.startsWith('-enc:'))) {
    ...
    if (destructive.has(cmdName)) {
        ...
        const hitsRootOrWild = targetStr === '/' ||
            targetStr === '.' ||
    ...
    if (cmdName === 'chmod' || cmdName === 'chown') {
        ...
        const hitsRoot = targetTokens.some((t) => t === '/' ||
            t === '.' ||
            t.startsWith('/') ||
            /^[a-z]:[\\/]/i.test(t) ||
            t.includes('~'));
    ...
    if ((cmdName === 'cacls' || cmdName === 'icacls') &&
 ...
 * 但已满足“取消挂起命令”的核心目标。
 */
function runSpawn(command, timeout, cwd) {
    return new Promise((resolve) => {
        ...
        (_d = child.stderr) === null || _d === void 0 ? void 0 : _d.on('data', (d) => {
            if (!timedOut)
                stderr += d;
        });
        timer = setTimeout(() => {
        ...
        child.on('close', (code) => finish(code !== null && code !== void 0 ? code : -1));
        // 兜底：close 未触发时（极少数情况）用 exit 收尾
        child.on('exit', (code) => finish(code !== null && code !== void 0 ? code : -1));
    });
}
...
function createShellExecExecutor(deps = {}) {
    return async (params, _context) => {
        ...
        try {
            ...
            if (danger.blocked) {
                Logger_1.Logger.warn(`🛡️ shell_exec 拦截危险命令: ${danger.reason} — "${command}"`, 'ShellExec');
                return fail(`命令被安全策略拦截: ${danger.reason}`, Date.now() - startTime);
            }
            ...
            const bridgeForShell = (0, bridgeRegistry_1.getActivePythonBridge)();
            if (bridgeForShell) {
                try {
                    ...
                    if (pyRes === null || pyRes === void 0 ? void 0 : pyRes.success) {
                        let finalOutput = String((_a = pyRes.output) !== null && _a !== void 0 ? _a : '').substring(0, 10000);
                        if (interpret && deps.llm) {
                            const interp = await interpretOutput(deps.llm, command, String((_b = pyRes.output) !== null && _b !== void 0 ? _b : ''));
                            if (interp)
                                finalOutput += `\n\n📖 解读:\n${interp}`;
                        }
                    ...
                    return fail((pyRes === null || pyRes === void 0 ? void 0 : pyRes.error) || 'Python shell_exec 执行失败', duration, String((_c = pyRes === null || pyRes === void 0 ? void 0 : pyRes.output) !== null && _c !== void 0 ? _c : '').substring(0, 5000));
                }
                catch (pyErr) {
                    // 仅 transport 错误才安全降级到 TS 本地执行
                    Logger_1.Logger.warn(`⚠️ F1 shell_exec Python 代理传输失败, 降级本地: ${pyErr.message}`, 'ShellExec');
                }
            }
            ...
            const { stdout, stderr, exitCode, timedOut } = await runSpawn(command, timeout, cwd);
            if (timedOut) {
                Logger_1.Logger.warn(`⏱️ shell_exec 超时已取消: "${command.substring(0, 50)}"`, 'ShellExec');
                return fail(`命令执行超时（>${timeout}ms）已被取消`, Date.now() - startTime, (stdout + stderr).substring(0, 5000));
            }
            if (exitCode !== 0) {
                return fail(`命令退出码: ${exitCode}`, Date.now() - startTime, (stdout || stderr || '').substring(0, 5000));
            }
