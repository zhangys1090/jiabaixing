...
/** 重复提问模式 — 短时间内相似输入 */
const RETRY_WINDOW_MS = 60000; // 1分钟内
class FeedbackCollector {
     ...
     * 分析用户输入，检测是否为纠正反馈
     */
    analyzeUserInput(currentInput, previousResponse, userId, scene) {
        const now = Date.now();
        // 检测明确纠正
        ...
        // 检测重复提问
        const recentForUser = this.recentInputs.filter((r) => r.userId === userId && now - r.timestamp < RETRY_WINDOW_MS);
        const isRetry = recentForUser.some((r) => this.calculateSimilarity(r.input, currentInput) > 0.7);
        if (isRetry) {
            const record = {
                id: `fb_${now}_${Math.random().toString(36).substring(2, 8)}`,
                type: 'retry',
     ...
     * 记录工具调用失败反馈
     */
    recordToolFailure(toolName, errorMessage, input, userId) {
        const record = {
     ...
     * 记录低质量反馈
     */
    recordLowQuality(input, response, qualityScore, userId, scene) {
        const record = {
            id: `fb_${Date.now()}_${Math.random().toString(36).substring(2, 8)}`,
            type: 'low_quality',
            input: input.substring(0, 500),
     ...
     * 记录明确反馈（用户主动评分等）
     */
    recordExplicitFeedback(input, response, correction, userId) {
        const record = {
            id: `fb_${Date.now()}_${Math.random().toString(36).substring(2, 8)}`,
            type: 'explicit',
            input: input.substring(0, 500),
    ...
    recordFeedback(record) {
        ...
        Logger_1.Logger.info(`📝 反馈收集: type=${record.type} input="${record.input.substring(0, 30)}..."`, 'FeedbackCollector');
        // 通知外部消费者
        if (this.deps.onFeedback) {
            try {
                ...
                Logger_1.Logger.warn(`反馈通知失败: ${err.message}`, 'FeedbackCollector');
            }
        }
    }
     ...
     * 检测情绪转变 — 分析用户输入中的情绪变化
     */
    detectEmotionShift(currentInput, _previousResponse) {
        // 消极情绪关键词
        ...
        for (const pattern of negativePatterns) {
            if (pattern.regex.test(currentInput)) {
                ...
                return {
                    ...
                    suggestedResponseAdjustment: '建议采用更谨慎、安抚性的回复风格，先确认问题再提供解决方案',
                };
            }
        }
        // 检测积极情绪
     ...
     * 记录情绪模式
     */
    recordEmotionPattern(emotionType) {
     ...
     */
    getEmotionPatterns() {
        return Array.from(this.emotionPatterns.values()).sort((a, b) => b.frequency - a.frequency);
    }
}
