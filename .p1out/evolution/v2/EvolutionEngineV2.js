...
exports.EvolutionEngineV2 = void 0;
...
class EvolutionEngineV2 {
    constructor(llmClient, checkpointDir = './.evolution-checkpoints') {
        this.history = [];
        ...
        this.MAX_STRATEGY_WEIGHTS = 100;
        /** 能力记录 — key 为领域 */
        this.capabilityOutcomes = new Map();
        this.MAX_CAPABILITY_OUTCOMES = 100;
        this.rollback = new EvolutionRollback_1.EvolutionRollback(checkpointDir);
        this.modifier = new SelfModificationEngine_1.SelfModificationEngine();
        this.planner = new EvolutionPlanner_1.EvolutionPlanner(llmClient);
        Logger_1.Logger.info('🧬 EvolutionEngineV2 initialized', 'EvolutionEngineV2');
    }
    /**
     * 主入口：触发进化
     */
    async triggerEvolution(cause) {
        if (this.isRunning) {
            Logger_1.Logger.warn('Evolution already in progress, skipping', 'EvolutionEngineV2');
            return null;
        }
        this.isRunning = true;
        try {
            Logger_1.Logger.info(`🚀 Evolution started: ${cause.type} - ${cause.description}`, 'EvolutionEngineV2');
            const plan = await this.planner.generateEvolutionPlan(cause);
            return await this.executePlan(plan);
        }
        catch (error) {
            Logger_1.Logger.error('Evolution failed', error, 'EvolutionEngineV2');
            return null;
        }
        finally {
            this.isRunning = false;
    ...
    async executePlan(plan) {
        Logger_1.Logger.info(`📋 Plan: ${plan.title} (${plan.actions.length} actions, risk: ${plan.estimatedRisk})`, 'EvolutionEngineV2');
        let result;
        // 即使是空计划也要记录历史
        const emptyPlan = plan.actions.length === 0;
        if (emptyPlan) {
            Logger_1.Logger.info('No actions in plan, skipping execution', 'EvolutionEngineV2');
            result = {
                planId: plan.id,
                success: true,
                executedActions: 0,
            ...
            try {
                ...
                // Step 3: 验证效果
                if (result.success) {
                    Logger_1.Logger.info('🔍 Validating evolution...', 'EvolutionEngineV2');
                    const validationResult = await this.validateEvolution(plan);
                    result.validationResult = validationResult;
                    if (!validationResult.passed) {
                        Logger_1.Logger.warn('Validation failed, initiating rollback', 'EvolutionEngineV2');
            ...
            if (result.rollbackNeeded) {
                ...
                if (rollbackResult.success) {
                    Logger_1.Logger.info('⏪ Evolution rolled back successfully', 'EvolutionEngineV2');
                }
                else {
                    Logger_1.Logger.error('❌ Rollback failed!', new Error(rollbackResult.error), 'EvolutionEngineV2');
                }
            }
        }
        // Step 5: 记录历史（包括空计划）
    ...
    recordStrategyOutcome(record) {
        ...
        if (this.strategyWeights.size > this.MAX_STRATEGY_WEIGHTS) {
            const oldestKey = this.strategyWeights.keys().next().value;
            this.strategyWeights.delete(oldestKey);
        }
    }
    /**
     * 预测最优策略 — 基于历史权重和上下文
     */
    predictOptimalStrategy(_context) {
        ...
        // 计算置信度 — 基于权重和样本数
        const sampleCount = this.strategyRecords.filter((r) => r.strategyType === topStrategy).length;
    ...
    getStrategyTrends() {
        const trends = [];
        const strategyTypes = new Set(this.strategyRecords.map((r) => r.strategyType));
        for (const strategyType of strategyTypes) {
            ...
            if (records.length >= 4) {
                ...
                const firstRate = firstHalf.filter((r) => r.outcome === 'success').length /
                    firstHalf.length;
                const secondRate = secondHalf.filter((r) => r.outcome === 'success').length /
                    secondHalf.length;
    ...
    getResourcePreloadHints() {
        ...
        for (const record of this.strategyRecords) {
            strategyFrequency.set(record.strategyType, (strategyFrequency.get(record.strategyType) || 0) + 1);
        }
     ...
     * 记录能力结果 — 跟踪各领域的成功/失败
     */
    recordCapabilityOutcome(domain, success) {
        let record = this.capabilityOutcomes.get(domain);
        if (!record) {
            if (this.capabilityOutcomes.size >= this.MAX_CAPABILITY_OUTCOMES) {
                const oldestKey = this.capabilityOutcomes.keys().next().value;
                this.capabilityOutcomes.delete(oldestKey);
            }
...
exports.EvolutionEngineV2 = EvolutionEngineV2;
exports.default = EvolutionEngineV2;
