...
const StepEvaluator_1 = require("./StepEvaluator");
const IndependentEvaluationService_1 = require("./IndependentEvaluationService");
...
class EvaluationPipeline {
     ...
     * @param weight    权重 (0-1)
     */
    addStage(name, evaluator, weight) {
        // 检查是否已存在同名阶段，存在则更新
            ...
            try {
                ...
                if (result.data) {
                    if (stage.name === 'step_evaluation') {
                        ...
                        independentResult = result.data;
                        allSuggestions.push(...(independentResult.overall.summary
                            ? [independentResult.overall.summary]
                            : []));
                    }
                    else if (stage.name === 'quality_scoring') {
                        qualityScore = result.data;
    ...
    getReport(pipelineResult) {
        ...
        for (const stage of pipelineResult.stages) {
            ...
            lines.push(`  ${icon} [${stage.stageName}] (权重 ${(stage.weight * 100).toFixed(0)}%)`);
        ...
        if (pipelineResult.independentResult) {
            ...
            lines.push(`  任务完成: ${ir.taskCompletion.completed ? '✅' : '❌'} (置信度: ${(ir.taskCompletion.confidence * 100).toFixed(0)}%)`);
     ...
     * 执行单个阶段
     */
    async runStage(stage, context, previousStepResults) {
        ...
        switch (name) {
            case 'step_evaluation': {
                ...
                const results = context.stepParams.map((params) => {
                    ...
                    // 默认 StepEvaluator
                    return new StepEvaluator_1.StepEvaluator().evaluateStep(params);
                });
            ...
            case 'independent_evaluation': {
                const service = stage.evaluator;
                const evaluator = service || new IndependentEvaluationService_1.IndependentEvaluationService();
                const result = await evaluator.evaluate(context.evalInput);
                const score = result.quality.overall * 60 +
                ...
                return {
                    stageName: name,
                    weight,
                    passed: normalizedScore >= 50 && result.safety.riskLevel !== 'critical',
            ...
            case 'quality_scoring': {
                const scorer = stage.evaluator;
                const scorerInstance = scorer || new QualityScorer_1.QualityScorer();
                const stepResults = previousStepResults ||
                    context.stepParams.map((p) => new StepEvaluator_1.StepEvaluator().evaluateStep(p));
                const qualityResult = scorerInstance.score(stepResults, context.scorerMetadata);
                return {
                    stageName: name,
                    weight,
            ...
            default:
                ...
                if (stage.evaluator) {
                    const evaluator = stage.evaluator;
                    if (evaluator instanceof QualityScorer_1.QualityScorer) {
                        const stepResults = previousStepResults ||
                            context.stepParams.map((p) => new StepEvaluator_1.StepEvaluator().evaluateStep(p));
                    ...
                    if (evaluator instanceof IndependentEvaluationService_1.IndependentEvaluationService) {
                        const result = await evaluator.evaluate(context.evalInput);
...
exports.default = EvaluationPipeline;
