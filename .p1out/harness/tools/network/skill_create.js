...
exports.SKILL_CREATE_DEF = void 0;
exports.createSkillCreateExecutor = createSkillCreateExecutor;
const types_1 = require("../../types");
const SkillRegistry_1 = require("../../../skills/SkillRegistry");
exports.SKILL_CREATE_DEF = {
    name: 'skill_create',
    ...
    parameters: {
        ...
        skill_json: {
            type: 'string',
            description: '技能JSON数据（import action时使用，符合agentskills.io格式）',
        },
        keyword: {
            type: 'string',
...
function renderTemplate(template, variables, params) {
...
function createSkillCreateExecutor(deps) {
        ...
        try {
            switch (action) {
                ...
                case 'list': {
                    ...
                    const output = skills
                        .map((s) => `• ${s.name} - ${s.description} (使用${s.usageCount}次, 评分${s.averageRating.toFixed(1)}, v${s.version})`)
                        .join('\n');
                ...
                case 'update': {
                    ...
                    if (params.template)
                        skill.template = String(params.template);
                    if (params.variables)
                        skill.variables = params.variables;
                    skill.updatedAt = Date.now();
                    await deps.skillStore.saveSkill(skill);
                    return {
                        success: true,
                ...
                case 'auto_improve': {
                    ...
                    if (rating > 0) {
                        ...
                        const ratingCount = skill.usageHistory.filter((r) => r.rating !== undefined).length;
                        skill.averageRating =
...
请分析问题并生成改进后的模板。只输出改进后的模板内容，不要其他解释。`;
                        try {
                            const improvedTemplate = await deps.llm.chat(improvementPrompt, []);
                ...
                case 'suggest_improvements': {
                    ...
                    const suggestions = candidates
                        .slice(0, 5)
                        .map((s) => {
                        const failCount = (s.usageHistory || []).filter((r) => !r.success).length;
                ...
                case 'usage_stats': {
                    ...
                    const stats = skills
                        ...
                        .map((s) => {
                        const avgTime = (s.usageHistory || []).length > 0
                            ? Math.round(s.usageHistory.reduce((sum, r) => sum + r.executionTime, 0) / s.usageHistory.length)
                            : 0;
                ...
                case 'import': {
                    ...
                    if (!skillJson) {
                        return {
                            success: false,
                            output: null,
                            error: '导入技能需要提供skill_json（符合agentskills.io格式的JSON字符串）',
                            duration: 0,
                            validated: false,
                        };
                    }
                ...
                case 'search_hub': {
                    ...
                    const output = results
                        .map((s) => `• ${s.name} (hubId: ${s.hubId || 'N/A'}) - ${s.description} [v${s.version}, ${s.license || 'MIT'}]`)
                        .join('\n');
                    return {
                        success: true,
