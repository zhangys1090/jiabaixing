...
exports.TASK_PRIORITY_DEF = void 0;
exports.createTaskPriorityExecutor = createTaskPriorityExecutor;
exports.dynamicPriorityScore = dynamicPriorityScore;
const Logger_1 = require("../../../utils/Logger");
const types_1 = require("../../types");
exports.TASK_PRIORITY_DEF = {
    name: 'task_priority',
    description: '管理任务优先级。支持提升优先级、降低优先级、批量调整优先级、设置截止日期提醒等操作。适用场景：用户需要按优先级排序任务、处理紧急任务、管理任务截止日期。',
...
function getPrevPriority(current) {
    const index = priorityOrder.indexOf(current);
    if (index > 0) {
        return priorityOrder[index - 1];
    }
...
function formatTaskByPriority(tasks) {
    const sorted = [...tasks]
    ...
    return grouped
        ...
        const items = g.tasks.map((t) => `  - [${t.id}] ${t.title}${t.dueDate ? ` 📅${new Date(t.dueDate).toLocaleDateString('zh-CN')}` : ''}`);
        return [header, ...items].join('\n');
    })
        .join('\n\n');
}
function createTaskPriorityExecutor(deps) {
                ...
                case 'dynamic_score': {
                    ...
                    for (const item of scored) {
                        if (item.task.priority !== item.suggestedPriority) {
                            ...
                            changed++;
                            Logger_1.Logger.debug(`动态优先级: [${item.task.id}] ${item.task.title} (${oldP} → ${item.suggestedPriority}, score=${item.score.toFixed(2)})`, 'TaskPriority');
                        }
                    }
                    const summary = scored
                        .sort((a, b) => b.score - a.score)
                        .slice(0, 10)
                        .map((s, i) => `${i + 1}. [${s.task.id}] ${s.task.title} → ${s.suggestedPriority} (score=${s.score.toFixed(2)}, urgency=${s.factors.urgency.toFixed(2)}, impact=${s.factors.impact.toFixed(2)})`)
                        .join('\n');
                    return {
                        success: true,
...
function dynamicPriorityScore(tasks) {
    const now = Date.now();
    return tasks.map((task) => {
        const factors = computeScoreFactors(task, now);
        const score = factors.urgency * 0.35 +
            factors.impact * 0.25 +
...
function computeScoreFactors(task, now) {
    // 紧急度：基于截止日期
    let urgency = 0.3;
    if (task.dueDate) {
        ...
        if (timeUntilDue <= 0)
            urgency = 1.0;
        else if (timeUntilDue <= 3600000)
            urgency = 0.9;
        else if (timeUntilDue <= 86400000)
            urgency = 0.7;
        else if (timeUntilDue <= 259200000)
            urgency = 0.5;
        else
            urgency = 0.3;
    }
