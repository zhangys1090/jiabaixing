...
exports.TOOL_INSPECT_DEF = void 0;
exports.createToolInspectExecutor = createToolInspectExecutor;
const types_1 = require("../../types");
exports.TOOL_INSPECT_DEF = {
    name: 'tool_inspect',
    description: '检查已注册工具的详细信息。可查看工具定义、参数、使用统计、动态工具的剩余TTL等。适用场景：了解可用工具能力、调试工具调用、查看动态工具状态。不适用：执行工具（请直接调用工具）。',
    ...
    parameters: {
        ...
        filter: {
            type: 'string',
            description: '过滤方式：all(全部) | dynamic(仅动态) | static(仅静态) | category:xxx(按分类)',
            default: 'all',
        },
    },
    requiredParams: [],
...
function createToolInspectExecutor(deps) {
    return async (params, _context) => {
        ...
        if (name) {
            ...
            for (const [pName, pDef] of Object.entries(def.parameters)) {
                const pd = pDef;
                result += `  - ${pName} (${pd.type}): ${pd.description}${pd.default !== undefined ? ` [默认: ${pd.default}]` : ''}\n`;
            }
            result += `必填: ${def.requiredParams.join(', ') || '无'}\n`;
            result += `权限: ${def.requiredPermissions.join(', ') || '无'}\n`;
            result += `类型: ${isDynamic ? '动态工具' : '系统内置'}\n`;
            return { success: true, output: result, duration: Date.now() - startTime, validated: false };
        }
        if (!deps.toolRegistry) {
            return { success: false, output: null, error: 'ToolRegistry 不可用', duration: Date.now() - startTime, validated: false };
        }
        ...
        const dynamicCount = tools.filter(t => t.definition.tags?.includes('dynamic')).length;
        ...
        for (const [cat, catTools] of byCategory) {
            ...
            for (const tool of catTools) {
                ...
                result += `  ${isDyn} ${tool.definition.name} - ${tool.definition.description.substring(0, 60)}${tool.definition.description.length > 60 ? '...' : ''}\n`;
            }
            result += '\n';
        }
        return { success: true, output: result, duration: Date.now() - startTime, validated: false };
