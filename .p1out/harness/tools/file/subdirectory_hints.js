...
exports.SUBDIRECTORY_HINTS_DEF = void 0;
exports.createSubdirectoryHintsExecutor = createSubdirectoryHintsExecutor;
...
exports.SUBDIRECTORY_HINTS_DEF = {
    name: 'subdirectory_hints',
    description: '分析项目目录结构，返回智能导航提示。适用场景：Agent需要了解项目布局、定位特定功能的目录、减少盲目搜索。不适用：已知目标路径的情况。',
...
function scoreRelevance(dirName, query) {
    let score = 0;
...
async function scanDirectory(rootDir, maxDepth, query, currentDepth = 0) {
    ...
    for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
            ...
            const childHints = await scanDirectory(fullPath, maxDepth, query, currentDepth + 1);
    ...
    const dirName = path_1.default.basename(rootDir);
...
function createSubdirectoryHintsExecutor(deps) {
    return async (params, _context) => {
        const directory = params.directory || '.';
        const query = params.query;
        const maxDepth = Number(params.max_depth) || 3;
        const rootDir = path_1.default.isAbsolute(directory)
            ? directory
        ...
        try {
            ...
            const output = [
                ...
                .filter(Boolean)
                .join('\n');
            Logger_1.Logger.info(`📂 subdirectory_hints: ${hints.length}个目录 (${directory})`, 'SubdirectoryHints');
            return {
                success: true,
                output,
                duration: 0,
            ...
            Logger_1.Logger.error('❌ subdirectory_hints 失败', error, 'SubdirectoryHints');
            return {
                success: false,
                output: `目录分析失败: ${error.message}`,
                error: error.message,
                duration: 0,
                validated: false,
            };
