...
exports.INCREMENTAL_EDIT_DEF = void 0;
exports.createIncrementalEditExecutor = createIncrementalEditExecutor;
const fs = __importStar(require("fs"));
const path_1 = __importDefault(require("path"));
const EventBus_1 = require("../../../shared/EventBus");
const Logger_1 = require("../../../utils/Logger");
const types_1 = require("../../types");
exports.INCREMENTAL_EDIT_DEF = {
    ...
    parameters: {
        ...
        edits: {
            type: 'array',
            description: '修改列表，每项包含 {search: "要替换的代码", replace: "新代码", description: "修改说明"}',
            items: {
                type: 'object',
                description: '修改项',
                properties: {
...
function createIncrementalEditExecutor(deps) {
        ...
        try {
            ...
            if (validateSyntax && !previewOnly) {
                ...
                if (['.ts', '.tsx', '.js', '.jsx'].includes(ext)) {
                    ...
                    if (syntaxErrors.length > 0) {
                        Logger_1.Logger.warn(`⚠️ 语法验证发现问题: ${syntaxErrors.length}个`, 'IncrementalEdit');
                        return {
                            success: false,
                            output: null,
            ...
            if (deps.checkpointService) {
                try {
                    ...
                    await deps.checkpointService.createCheckpoint(`incremental_edit:${fileName}`);
                }
                catch (cpErr) {
                    Logger_1.Logger.warn(`检查点创建失败: ${cpErr.message}`, 'IncrementalEdit');
                }
            }
            ...
            Logger_1.Logger.info(`✏️ 增量修改: ${filePath} (${appliedEdits.filter((e) => e.found).length}/${edits.length}个修改已应用)`, 'IncrementalEdit');
            void EventBus_1.EventBus.emit('file_modified', {
                traceId,
                filePath,
                changeType: 'modified',
            ...
            Logger_1.Logger.error(`增量修改失败: ${filePath}`, err, 'IncrementalEdit');
            return {
                success: false,
                output: null,
