...
exports.SECURITY_GUIDANCE_DEF = void 0;
exports.createSecurityGuidanceExecutor = createSecurityGuidanceExecutor;
const fs_1 = __importDefault(require("fs"));
const path_1 = __importDefault(require("path"));
const Logger_1 = require("../../../utils/Logger");
const types_1 = require("../../types");
exports.SECURITY_GUIDANCE_DEF = {
    name: 'security_guidance',
    description: '安全指导工具。提供安全编码建议、敏感信息检测、安全配置审计。支持操作：check=安全检查, scan_secrets=敏感信息扫描, audit_config=配置审计, best_practices=最佳实践建议。适用场景：代码安全审查、敏感信息泄露检测、安全配置验证。',
    ...
    parameters: {
        ...
        category: {
            type: 'string',
            description: '安全类别：all|injection|xss|auth|crypto|secrets|config|deps',
            enum: [
                'all',
                'injection',
                'xss',
...
function scanForSecrets(projectRoot, minSeverity) {
    const findings = [];
    ...
    function scanFile(filePath, relPath) {
        try {
            ...
            for (const { pattern, name, severity } of SECRET_PATTERNS) {
                if (SEVERITY_ORDER[severity] < minOrder)
                    continue;
    ...
    function walkDir(dir, depth = 0) {
        ...
        for (const entry of entries) {
            ...
            else if (/\.(ts|tsx|js|jsx|json|env|yaml|yml|toml|ini|cfg|conf)$/.test(entry.name) ||
                entry.name === '.env') {
...
function auditGitignore(projectRoot) {
    const findings = [];
...
function auditEnvFiles(projectRoot) {
    const findings = [];
    const envExample = path_1.default.join(projectRoot, '.env.example');
    const envLocal = path_1.default.join(projectRoot, '.env');
    if (fs_1.default.existsSync(envLocal) && !fs_1.default.existsSync(envExample)) {
        findings.push({
            category: 'config',
            severity: 'warning',
    ...
    if (fs_1.default.existsSync(envLocal)) {
        try {
            const lines = fs_1.default.readFileSync(envLocal, 'utf-8').split('\n');
            for (let i = 0; i < lines.length; i++) {
                ...
                if (line && !line.startsWith('#') && line.includes('=')) {
                    ...
                    if (value &&
                        ...
                        findings.push({
                            ...
                            file: '.env',
                            line: i + 1,
                            suggestion: '确保 .env 已在 .gitignore 中排除，使用 .env.example 提供模板',
                        });
                        break;
                    }
                }
...
function getBestPractices(category) {
    ...
    const categories = category === 'all' ? Object.keys(allPractices) : [category];
...
function createSecurityGuidanceExecutor(deps) {
    return async (params, _context) => {
        ...
        try {
            Logger_1.Logger.info(`🛡️ security_guidance ${action}: ${projectRoot}`, 'SecurityGuidance');
            let findings = [];
            switch (action) {
                case 'scan_secrets':
            ...
            const minOrder = SEVERITY_ORDER[minSeverity] || 2;
            findings = findings.filter((f) => (SEVERITY_ORDER[f.severity] || 1) >= minOrder);
            ...
            if (counts.critical > 0)
                lines.push('🔴 存在严重安全问题，请立即修复！');
            Logger_1.Logger.info(`🛡️ security_guidance 完成: ${findings.length}个发现 (严重:${counts.critical})`, 'SecurityGuidance');
            return {
                success: true,
                output: lines.join('\n'),
                duration: Date.now() - startTime,
            ...
            Logger_1.Logger.error('❌ security_guidance 失败', error, 'SecurityGuidance');
            return {
                success: false,
                output: '',
