...
exports.DELEGATE_TASK_DEF = void 0;
exports.createDelegateTaskExecutor = createDelegateTaskExecutor;
const types_1 = require("../../types");
const Logger_1 = require("../../../utils/Logger");
exports.DELEGATE_TASK_DEF = {
    name: 'delegate_task',
    description: '将子任务委托给独立的子 Agent 执行。USE WHEN: 需要并行处理多个独立任务、将复杂任务拆分给专门的执行者、或需要隔离上下文执行子任务。DO NOT USE WHEN: 任务简单可直接用单个工具完成。子 Agent 有独立上下文，通过 goal 和 context 接收信息，返回执行结果摘要。',
 ...
 */
async function runSubAgent(goal, context, allowedTools, maxIterations, deps) {
    ...
    for (let i = 0; i < maxIterations; i++) {
        ...
        let response;
        try {
            response = await deps.llm.chat(messages.map((m) => m.content).join('\n'), [], systemPrompt);
        }
        catch (err) {
            return {
                goal,
                output: `子 Agent LLM 调用失败: ${err.message}`,
                toolsUsed,
                iterations,
...
function createDelegateTaskExecutor(deps) {
    return async (params, _context) => {
        ...
        const tools = params.tools ? params.tools : undefined;
        if (!goal.trim()) {
            return {
                success: false,
                output: null,
        ...
        try {
            Logger_1.Logger.info(`🎯 delegate_task 启动: "${goal.substring(0, 60)}"`, 'DelegateTask');
            ...
            const output = [
                ...
                .filter(Boolean)
                .join('\n');
            Logger_1.Logger.info(`🎯 delegate_task 完成: ${result.success ? '成功' : '失败'} (${result.iterations}轮, ${(result.duration / 1000).toFixed(1)}s)`, 'DelegateTask');
            return {
            ...
            return {
                success: false,
                output: null,
                error: `子 Agent 委托失败: ${err.message}`,
                duration: Date.now() - startTime,
                validated: false,
            };
        }
