...
exports.VOICE_INTERACT_DEF = void 0;
exports.createVoiceInteractExecutor = createVoiceInteractExecutor;
const fs = __importStar(require("fs"));
const Logger_1 = require("../../../utils/Logger");
const EventBus_1 = require("../../../shared/EventBus");
const SpeechRecognizer_1 = require("../../../multimodal/SpeechRecognizer");
const SpeechSynthesizer_1 = require("../../../multimodal/SpeechSynthesizer");
const types_1 = require("../../types");
exports.VOICE_INTERACT_DEF = {
    name: 'voice_interact',
    ...
    parameters: {
        action: {
            type: 'string',
            description: '操作类型：start_session=开始语音会话，stop_session=停止语音会话，speak=将文本转为语音，listen=获取语音输入并识别，status=获取语音会话状态',
        ...
        audioPath: {
            type: 'string',
            description: 'listen 操作：待识别的音频文件路径（wav）。提供后执行真实 ASR，替换原先的模拟音频。',
        },
        ttsBackend: {
            type: 'string',
...
let _sharedRecognizer = null;
function getSharedRecognizer() {
    if (!_sharedRecognizer)
        _sharedRecognizer = new SpeechRecognizer_1.SpeechRecognizer();
    return _sharedRecognizer;
}
/**
 * 创建 voice_interact 执行器
 */
function createVoiceInteractExecutor(deps = {}) {
    return async (params, _context) => {
        const startTime = Date.now();
        const action = params.action;
        const text = params.text;
        const language = params.language || 'zh-CN';
        const emotion = params.emotion || '平静';
        try {
            switch (action) {
                case 'start_session':
                ...
                default:
                    return fail(`不支持的操作: ${action}。支持: start_session, stop_session, speak, listen, status`, Date.now() - startTime);
            }
        }
        catch (error) {
            Logger_1.Logger.error('❌ voice_interact 失败', error, 'VoiceInteract');
            return fail(`语音交互失败: ${error.message}`, Date.now() - startTime);
        }
    };
}
/**
 * 处理 start_session 操作
 */
function handleStartSession(deps, language, startTime) {
    if (deps.interactionEngine) {
        const session = deps.interactionEngine.startVoiceSession(language);
        Logger_1.Logger.info(`🎤 voice_interact: 语音会话已启动 id=${session.id}`, 'VoiceInteract');
        return ok(`语音会话已启动 (id=${session.id}, language=${language})`, Date.now() - startTime, {
    ...
    Logger_1.Logger.info(`🎤 voice_interact (模拟): 语音会话已启动 id=${mockSessionId}`, 'VoiceInteract');
    return ok(`语音会话已启动 (模拟模式, id=${mockSessionId}, language=${language})`, Date.now() - startTime, { sessionId: mockSessionId, language, status: 'idle', simulated: true });
}
/**
 * 处理 stop_session 操作
 */
function handleStopSession(deps, startTime) {
    if (deps.interactionEngine) {
        ...
        Logger_1.Logger.info('🎤 voice_interact: 语音会话已停止', 'VoiceInteract');
        return ok(`语音会话已停止 (轮次=${turnCount})`, Date.now() - startTime, {
            turnCount,
        });
    }
    Logger_1.Logger.info('🎤 voice_interact (模拟): 语音会话已停止', 'VoiceInteract');
 ...
 * 处理 speak 操作
 */
async function handleSpeak(deps, text, emotion, params, startTime) {
    if (!text || text.trim().length === 0) {
        return fail('speak 操作需要提供 text 参数', Date.now() - startTime);
    }
    // P1-3：TTS 后端切换（real 走真实合成器，mock 仅记录意图）
    const ttsBackend = (params.ttsBackend ||
        (deps.interactionEngine?.speechSynthesizer ? 'real' : 'mock'));
    const synthesizer = new SpeechSynthesizer_1.SpeechSynthesizer(ttsBackend, deps.interactionEngine?.speechSynthesizer
    ...
    Logger_1.Logger.info(`🔊 voice_interact speak [${result.backend}]: "${text.substring(0, 30)}..."`, 'VoiceInteract');
    return ok(result.backend === 'real'
        ? `语音已生成并播放 (${(result.duration ?? 0)}ms)`
        : `语音指令已接收: "${text.substring(0, 50)}${text.length > 50 ? '...' : ''}"`, Date.now() - startTime, {
        emotion,
 ...
 */
async function handleListen(deps, params, startTime) {
    const language = params.language || 'zh-CN';
    ...
    if (audioPath) {
        try {
            ...
            return fail(`无法读取音频文件: ${audioPath} (${readErr.message})`, Date.now() - startTime);
        }
    }
        ...
        try {
            ...
            Logger_1.Logger.warn(`🎤 麦克风采集失败: ${capErr.message}`, 'VoiceInteract');
        }
    }
    if (audioBuffer && audioBuffer.length > 0) {
        ...
        EventBus_1.EventBus.emit('voice_recognized', {
            text: result.text,
            language,
        ...
        if (deps.interactionEngine && result.text) {
            try {
                deps.interactionEngine.handleUserSpeech?.(result.text);
            }
            catch {
                /* 交互引擎不一定实现 handleUserSpeech，忽略 */
            }
        }
        Logger_1.Logger.info(`🎤 voice_interact listen: 真实识别="${result.text}"`, 'VoiceInteract');
        return ok(result.text ? `语音识别结果: "${result.text}"` : '未检测到语音输入', Date.now() - startTime, {
            text: result.text,
            confidence: result.confidence,
            language,
            hasAudio: true,
    ...
    return fail('未提供音频源（audioPath 参数），且当前环境无可用的麦克风采集，无法执行真实语音识别', Date.now() - startTime);
}
/**
 * 处理 status 操作
 */
function handleStatus(deps, startTime) {
    if (deps.interactionEngine) {
        ...
        return ok(`语音会话状态: id=${session.id}, status=${session.status}, language=${session.language}, turns=${session.turnCount}`, Date.now() - startTime, {
            active: true,
            sessionId: session.id,
            status: session.status,
            language: session.language,
