...
class MessageSanitizer {
    ...
    sanitizeMessagesForAPI(messages) {
        ...
        for (const msg of messages) {
            ...
            const sanitized = { role: msg.role };
            if (msg.role === 'assistant') {
                if (msg.tool_calls && msg.tool_calls.length > 0) {
                    sanitized.tool_calls = this.repairToolCallsInMessage(msg.tool_calls);
                ...
                for (let k = nonSystemMessages.length - 1; k >= 0; k--) {
                    const prev = nonSystemMessages[k];
                ...
                if (!hasAssistantToolCalls) {
                    Logger_1.Logger.warn(`⚠️ tool 消息前无 assistant+tool_calls，跳过（tool_call_id=${msg.tool_call_id?.substring(0, 20)}）`, 'MessageSanitizer');
                    continue;
                }
    ...
    /**
     * 修复消息中的 tool_calls 参数
     *
     * 遍历 tool_calls，对每个 arguments 做 JSON 合法性检查，
     * 不合法则尝试 repairToolCallArguments 修复。
     */
    repairToolCallsInMessage(toolCalls) {
        return toolCalls.map((tc) => {
            ...
            try {
                ...
                const repaired = this.repairToolCallArguments(tc.function.arguments);
                if (repaired) {
                    return {
                        ...tc,
                        function: {
                ...
                Logger_1.Logger.warn(`⚠️ tool_call参数修复失败: ${tc.function?.name}`, 'MessageSanitizer');
                return tc;
            }
        });
    }
    ...
    repairToolCallArguments(raw) {
        ...
        // 步骤3: 单引号转双引号（仅键名位置的单引号，避免破坏字符串值中的单引号如 O'Brien）
        ...
        try {
            ...
            if (trimmed) {
                try {
                    ...
                    Logger_1.Logger.debug(`JSON修复步骤8-1失败: ${err2?.message}`, 'MessageSanitizer');
                }
            }
            // 步骤9: 平衡括号 — 补全缺失的闭合括号
            ...
            try {
                ...
                Logger_1.Logger.warn(`⚠️ 工具调用参数 JSON 修复失败: ${err?.message}，原始: ${raw.slice(0, 200)}`, 'MessageSanitizer');
                return null;
            }
        }
    }
