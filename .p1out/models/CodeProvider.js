...
class CodeProvider {
    ...
    async executeWithRetry(operation, operationName, maxRetries = this.maxRetries) {
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                const isConnectionError = CONNECTION_ERRORS.some((e) => errorMsg.includes(e));
                const isAuthError = errorMsg.includes('401') ||
                    errorMsg.includes('invalid') ||
                    errorMsg.includes('authentication');
                if (isConnectionError || isAuthError) {
                    Logger_1.Logger.warn(`🚫 ${operationName} ${isAuthError ? '认证失败' : '连接错误'}，跳过重试: ${lastError.message}`, 'CodeProvider');
                    break;
                }
                if (attempt < maxRetries) {
                    const delay = this.baseRetryInterval * Math.pow(2, attempt - 1);
                    Logger_1.Logger.warn(`${operationName} 第${attempt}次失败，${delay}ms后重试: ${lastError.message}`, 'CodeProvider');
    ...
    async analyzeCode(filePath, content, userQuery) {
        const systemPrompt = (0, PreferenceInjector_1.injectPreferences)((0, prompt_templates_1.getPromptTemplate)('analyzeCode'));
...
请分析并给出专业、温柔的回答。`;
        ...
        try {
            ...
            Logger_1.Logger.error(`⚠️ LLM分析失败`, error, 'CodeProvider');
            throw error;
        }
    }
    /**
    ...
    async devGenerateCode(userRequest, filePath, existingContent) {
        const systemPrompt = (0, prompt_templates_1.getPromptTemplate)('devGenerateCode');
        ...
        try {
            ...
            Logger_1.Logger.error('开发副驾代码生成失败', error, 'CodeProvider');
            throw error;
        }
    }
    /**
    ...
    async generateModificationPlan(filePath, content, userQuery) {
        const systemPrompt = (0, PreferenceInjector_1.injectPreferences)((0, prompt_templates_1.getPromptTemplate)('generateModificationPlan'));
...
请给出修改方案。`;
        ...
        try {
            ...
            Logger_1.Logger.error(`⚠️ LLM生成修改方案失败`, error, 'CodeProvider');
            throw error;
        }
    }
    /**
    ...
    async generateModifiedFileContent(filePath, currentContent, userRequest, fileExists) {
        const rawPrompt = (0, prompt_templates_1.getPromptTemplate)('generateModifiedFileContent');
        const systemPrompt = (0, PreferenceInjector_1.injectPreferences)(rawPrompt.replace('{{fileState}}', fileExists ? '' : '（文件当前不存在）'));
...
请给出修改后的完整文件内容。`;
        ...
        try {
            ...
            Logger_1.Logger.error(`⚠️ 生成修改文件内容失败`, error, 'CodeProvider');
            throw error;
        }
    }
}
