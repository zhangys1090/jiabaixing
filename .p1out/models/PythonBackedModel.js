...
class PythonBackedModel {
    ...
    async generate(input) {
        ...
        if (!bridge) {
            throw new Error('PythonAgentBridge 不可用，无法委派 LLM 调用（local 模式不应使用 PythonBackedModel）');
        }
        try {
            if (input.tools && input.tools.length > 0) {
                const result = await bridge.llmChatWithTools(input.messages ?? [
                    { role: 'user', content: input.prompt || input.text || '' },
                ], input.tools, input.maxTokens ?? 4096, (input.toolChoice ?? 'auto'));
                return { text: result?.content ?? '', toolCalls: result?.toolCalls };
            }
            const text = await bridge.llmChat(input.prompt || input.text || '', [], input.systemPrompt);
            return { text };
        }
        catch (err) {
            Logger_1.Logger.error('🐍 PythonBackedModel 委派 LLM 失败', err, 'PythonBackedModel');
            throw err;
        }
    }
    async *stream(input) {
...
exports.PythonBackedModel = PythonBackedModel;
