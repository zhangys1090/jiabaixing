...
exports.MemoryAssistant = void 0;
...
class MemoryAssistant {
     ...
     * 构建用户画像摘要
     */
    async buildUserProfileSummary(userId) {
     ...
     */
    async autoRetrieveMemories(input, _userId) {
        ...
        try {
            ...
            if (!memories || memories.length === 0)
                return [];
            const formatted = memories.map((m) => `[${m.type || '记忆'}] ${m.content}${m.timestamp ? `（${new Date(m.timestamp).toLocaleDateString('zh-CN')}）` : ''}`);
     ...
     */
    deduplicateMemoryStrings(memories) {
    ...
    async retrieveContext(input, _userId) {
        if (!this.memoryEngine?.retrieveRelevant) {
            return {
                memories: [],
     ...
     */
    async autoExtractKnowledge(input, response, _userId) {
        if (!this.memoryEngine?.storeShortTermMemory)
            return;
        const extracted = [];
        const preferencePatterns = [
            ...
            {
                pattern: /我(?:明天|下周|后天|过几天|待会|稍后)\s*(?:要|需要|准备|打算)\s*([^。，！？]+)/,
                category: 'task',
                importance: 0.9,
            },
            {
        ...
        const errorMatch = input.match(/(?:报错|错误|异常|bug|问题|失败)\s*[:：]?\s*([^。，！？]{3,50})/);
        if (errorMatch) {
            extracted.push({
        ...
        const solutionMatch = response.match(/(?:修复|解决|方案|建议)\s*[:：]?\s*([^。，！？]{5,60})/);
        if (solutionMatch) {
            extracted.push({
        ...
        for (const item of extracted) {
            try {
                ...
                if (item.importance >= 0.8 && this.memoryEngine.storeLongTermMemory) {
                    try {
                        await this.memoryEngine.storeLongTermMemory(item.content, item.category);
                        Logger_1.Logger.info(`🧠 Hindsight长期记忆: [${item.category}] ${item.content} (importance=${item.importance})`, 'MemoryAssistant');
                    }
                    catch {
                        // 长期记忆存储失败不影响短期记忆
                    }
                }
                Logger_1.Logger.info(`🧠 自动提取知识: [${item.category}] ${item.content}`, 'MemoryAssistant');
            }
            catch (error) {
                Logger_1.Logger.debug(`知识存储失败（非关键）: ${error.message}`, 'MemoryAssistant');
            }
        }
    }
}
exports.MemoryAssistant = MemoryAssistant;
