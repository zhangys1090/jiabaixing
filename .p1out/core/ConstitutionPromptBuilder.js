...
exports.ConstitutionPromptBuilder = void 0;
 ...
 * - 项目上下文变化时更新
 */
class ConstitutionPromptBuilder {
    constructor(deps) {
        this.deps = deps;
        this._profileSyncCounter = 0;
        /** 项目上下文缓存内容 */
     ...
     */
    async buildConstitutionPrompt(userId) {
        const now = new Date();
...
【反幻觉纪律 — 严禁编造】
1. **只说有依据的话** — 回答必须基于工具返回的真实数据或用户提供的上下文，不得凭空编造
2. **数据溯源** — 引用数据时，必须能追溯到具体的工具调用结果或用户输入
3. **承认不知道** — 如果工具没有返回相关信息，明确说"我没有找到相关信息"，而不是猜测或编造
4. **禁止虚构引用** — 不得编造论文、研究、报告、数据来源；不得编造不存在的文件路径、URL、版本号
5. **禁止幻觉填充** — 遇到信息缺失时，用"需要进一步确认"代替编造内容
6. **区分事实与推断** — 推断性内容必须标注"根据已有信息推断"，并说明推断依据
7. **工具优先于记忆** — 记忆可能过时，工具数据更可靠；两者矛盾时以工具数据为准
8. **输出自检** — 回复前自问：这段内容有数据支撑吗？是否在编造？如果是，删掉或标注不确定性
...
**工具调用规则**：每轮最多调2个工具。调完工具后，如果已经有了答案就直接回复用户——不需要继续调更多工具来"确认"结果。`;
     ...
     */
    async buildUserProfileSection(userId) {
        ...
        try {
            ...
            // 开发偏好
            if (devHabits.preferredLanguages &&
                devHabits.preferredLanguages.length > 0) {
                parts.push(`编程语言偏好：${devHabits.preferredLanguages.slice(0, 3).join('、')}`);
            }
            if (devHabits.preferredFrameworks &&
                devHabits.preferredFrameworks.length > 0) {
                parts.push(`框架偏好：${devHabits.preferredFrameworks.slice(0, 3).join('、')}`);
            }
            if (devHabits.commonTools && devHabits.commonTools.length > 0) {
                parts.push(`常用工具：${devHabits.commonTools.slice(0, 3).join('、')}`);
            }
            // 生活偏好
            if (lifePrefs.dietaryPreferences &&
                lifePrefs.dietaryPreferences.length > 0) {
                parts.push(`饮食偏好：${lifePrefs.dietaryPreferences.slice(0, 2).join('、')}`);
            }
            if (lifePrefs.entertainmentPreferences &&
                lifePrefs.entertainmentPreferences.length > 0) {
                parts.push(`娱乐偏好：${lifePrefs.entertainmentPreferences.slice(0, 2).join('、')}`);
            }
            if (lifePrefs.exerciseHabits && lifePrefs.exerciseHabits.length > 0) {
                parts.push(`运动习惯：${lifePrefs.exerciseHabits.slice(0, 2).join('、')}`);
            }
            // 情绪模式
            if (emotionalPatterns.commonEmotions &&
            ...
            if (taskPrefs.preferredWorkStyle) {
                parts.push(`工作风格：${taskPrefs.preferredWorkStyle === 'focused' ? '专注执行' : '多任务协作'}`);
            }
            // P2增强：从进化引擎获取偏好数据（若有）
            ...
            if (evolutionData) {
                if (evolutionData.communicationStyle &&
                    ...
                    parts.push(`沟通风格偏好：${styleMap[evolutionData.communicationStyle.style] || evolutionData.communicationStyle.style}`);
                }
                if (evolutionData.interactionTimePatterns &&
...
${parts.join('\n')}`;
     ...
     */
    async tryGetEvolutionData(userId) {
        try {
            const evolutionEngine = this.deps.evolutionEngine;
            ...
            if (this._profileSyncCounter % 10 === 0 &&
                ...
                try {
                    ...
                    if (userProfile &&
                        ...
                        if (changed) {
                            Logger_1.Logger.info('🧬 进化数据已写回用户画像', 'ConstitutionPromptBuilder');
                        }
                    }
                }
                catch {
...
exports.ConstitutionPromptBuilder = ConstitutionPromptBuilder;
