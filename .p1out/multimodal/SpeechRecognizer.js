...
class SpeechRecognizer {
    ...
    async initialize() {
        if (this.initialized)
            return;
        try {
            Logger_1.Logger.info(`🎤 初始化 Whisper 模型: ${this.config.model} (${this.config.device})`, 'SpeechRecognizer');
            // 尝试检测 Whisper Python 环境
            try {
                // faster-whisper (Python) — 通过 CLI 检测（无 shell 调用）
                (0, child_process_1.execFileSync)('python', ['-c', "from faster_whisper import WhisperModel; print('ok')"], {
                    stdio: 'pipe',
                    timeout: 5000,
                });
                this.whisperModel = { type: 'faster-whisper' };
                Logger_1.Logger.info('✅ faster-whisper Python 环境可用', 'SpeechRecognizer');
            }
            catch (err) {
                Logger_1.Logger.info(`faster-whisper不可用: ${err?.message}，尝试 openai-whisper`, 'SpeechRecognizer');
                try {
                    // openai-whisper (Python) — 通过 CLI 检测（无 shell 调用）
                    (0, child_process_1.execFileSync)('python', ['-c', "import whisper; print('ok')"], {
                        stdio: 'pipe',
                        timeout: 5000,
                    });
                    this.whisperModel = { type: 'openai-whisper' };
                    Logger_1.Logger.info('✅ openai-whisper Python 环境可用', 'SpeechRecognizer');
                }
                catch (err) {
                    Logger_1.Logger.warn(`Whisper Python 环境均不可用: ${err?.message}，语音识别将运行在降级模式`, 'SpeechRecognizer');
                }
            }
            this.initialized = true;
        }
        catch (error) {
            Logger_1.Logger.error('❌ Whisper 模型初始化失败', error, 'SpeechRecognizer');
            // 降级为模拟模式
            this.initialized = true;
        }
    }
    async recognize(audioData) {
        ...
        if (!this.initialized) {
            Logger_1.Logger.warn('语音识别器未初始化，正在懒初始化...', 'SpeechRecognizer');
            try {
                await this.initialize();
            }
            catch (err) {
                Logger_1.Logger.debug(`语音识别懒初始化失败: ${err?.message}`, 'SpeechRecognizer');
                return {
                    text: '',
                    confidence: 0,
        ...
        void EventBus_1.EventBus.emit('perception_update', {
            traceId,
            modality: 'voice',
            status: 'started',
        ...
        try {
            ...
            Logger_1.Logger.error('❌ 语音识别失败', error, 'SpeechRecognizer');
            void EventBus_1.EventBus.emit('perception_update', {
                traceId,
                modality: 'voice',
                status: 'failed',
    ...
    async transcribeWithWhisper(audioData) {
        const tmpFile = path.join(process.cwd(), 'tmp', `whisper_${Date.now()}.wav`);
        try {
            // 确保临时目录存在
            ...
            const modelType = this.whisperModel?.type || 'faster-whisper';
            ...
            if (modelType === 'faster-whisper') {
                output = (0, child_process_1.execFileSync)('python', ['-c', PY_TRANSCRIBE_FASTER, this.config.model, lang, tmpFile], { stdio: 'pipe', timeout: 60000, encoding: 'utf-8' });
            }
            else {
                output = (0, child_process_1.execFileSync)('python', ['-c', PY_TRANSCRIBE_OPENAI, this.config.model, lang, tmpFile], { stdio: 'pipe', timeout: 60000, encoding: 'utf-8' });
            }
            ...
            Logger_1.Logger.warn(`Whisper 转录失败: ${error.message}`, 'SpeechRecognizer');
            ...
            try {
                ...
                Logger_1.Logger.debug(`临时文件清理失败: ${err?.message}`, 'SpeechRecognizer');
            }
        }
    }
    /**
     ...
     */
    validateWhisperInputs(model, lang, tmpFile) {
        ...
        if (path.extname(resolved) !== '.wav' || !inside) {
            throw new Error(`非法的 Whisper 临时文件路径（路径穿越或非法扩展名）: ${JSON.stringify(tmpFile)}`);
        }
    }
    ...
    addPunctuation(text) {
        const withPeriods = text.replace(/(?<=[\u4e00-\u9fa5]{3,})(?=\s+[\u4e00-\u9fa5])/g, '。');
        if (!/[。！？.!?]$/.test(withPeriods)) {
            return withPeriods + '。';
        }
        return withPeriods;
    ...
    async shutdown() {
        this.whisperModel = null;
        this.initialized = false;
        Logger_1.Logger.info('🎤 语音识别器已关闭', 'SpeechRecognizer');
    }
    ensureInitialized() {
        if (!this.initialized) {
