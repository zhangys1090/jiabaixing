...
class SecurityGuard {
    ...
    setUserRole(userId, role) {
        if (this.userRoles.size >= this.MAX_USER_ROLES && !this.userRoles.has(userId)) {
            const oldestKey = this.userRoles.keys().next().value;
            this.userRoles.delete(oldestKey);
        }
        this.userRoles.set(userId, role);
        Logger_1.Logger.info(`👤 设置用户角色: ${userId} -> ${role}`, 'SecurityGuard');
    }
    /**
     * 获取用户角色
     */
     ...
     * @returns 检查结果
     */
    sandboxCheck(code, _language = 'javascript') {
        const errors = [];
        const warnings = [];
        ...
        // 检查危险代码模式
        for (const [category, patterns] of Object.entries(this.dangerousCodePatterns)) {
            for (const pattern of patterns) {
                if (pattern.test(code)) {
                    ...
                    if (category === 'eval' || category === 'child_process') {
                        errors.push(`检测到危险操作: ${categoryNames[category] || category}`);
                    }
                    else {
                        warnings.push(`检测到潜在风险: ${categoryNames[category] || category}`);
                    }
                }
            }
        }
     ...
     * @returns 是否有权限
     */
    permissionCheck(userId, resource, action) {
        ...
        if (!resourceConfig.allowedRoles.includes(role)) {
            this.logAudit(userId, action, resource, 'deny', `角色 ${role} 无权访问资源 ${resource}`);
            return false;
        }
        // 检查操作是否被允许
        if (!resourceConfig.allowedActions.includes(action)) {
            this.logAudit(userId, action, resource, 'deny', `操作 ${action} 不被允许`);
            return false;
        }
        // 检查用户是否有具体权限
        ...
        if (!hasPermission) {
            this.logAudit(userId, action, resource, 'deny', `缺少权限: ${requiredPermission}`);
            return false;
        }
     ...
     * 记录安全审计日志
     */
    logAudit(userId, action, resource, result, reason) {
        this.auditLogs.push({
            timestamp: new Date(),
            userId,
            action,
        ...
        Logger_1.Logger.debug(`🔐 权限检查: ${userId} ${action} ${resource} -> ${result}${reason ? ` (${reason})` : ''}`, 'SecurityGuard');
    }
    /**
     * 获取审计日志
     */
     ...
     * @returns 验证结果
     */
    validateGeneratedCode(code, language = 'javascript') {
        const errors = [];
        const warnings = [];
        if (!code || code.trim().length === 0) {
            errors.push('生成的代码为空');
        ...
        if (openBraces !== closeBraces) {
            warnings.push(`花括号不匹配: 开 ${openBraces} 个, 关 ${closeBraces} 个，代码可能不完整`);
        }
        if (openParens !== closeParens) {
            warnings.push(`圆括号不匹配: 开 ${openParens} 个, 关 ${closeParens} 个，代码可能不完整`);
        }
        // 3. 检查未完成的代码标记
        ...
        if (language.toLowerCase() === 'typescript' ||
            ...
            if (/new\s+Promise|\.then\s*\(|async\s+function|async\s*\(/.test(trimmedCode) &&
        ...
        for (const fnMatch of functionMatches) {
            ...
            const fnBodyRegex = new RegExp(`function\\s+${fnName}\\s*\\([^)]*\\)\\s*\\{([^}]*)\\}`, 's');
     ...
     * @returns 是否超过限制
     */
    resourceLimitCheck(resourceType, usage) {
        const limits = {
        ...
        if (!allowed) {
            Logger_1.Logger.warn(`⚠️ 安全守卫：资源使用超过限制 - ${resourceType}: ${usage} > ${limit}`, 'SecurityGuard');
        }
     ...
     * @returns 执行结果
     */
    async executeWithProtection(callback, context = {}) {
        const timeout = context.timeout || 30000;
        const userId = context.userId || 'anonymous';
        const traceId = context.traceId || 'unknown';
        Logger_1.Logger.debug(`🛡️ 安全守卫：开始执行保护 - userId=${userId}, traceId=${traceId}`, 'SecurityGuard');
        try {
            // 1. 超时控制
            ...
            if (resultStr.length > maxOutputSize) {
                Logger_1.Logger.warn(`⚠️ 安全守卫：输出大小超过限制 (${resultStr.length} > ${maxOutputSize})`, 'SecurityGuard');
            }
            Logger_1.Logger.debug(`✅ 安全守卫：执行完成 - userId=${userId}`, 'SecurityGuard');
            return result;
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : String(error);
            // 记录安全事件
            Logger_1.Logger.error(`❌ 安全守卫：执行失败 - ${errorMessage}`, error, 'SecurityGuard');
            throw error;
        }
    }
}
