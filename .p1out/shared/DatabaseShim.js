...
exports.nativeAvailable = void 0;
...
exports.nativeAvailable = nativeAvailable;
try {
    // 先验证模块是否能正常加载和使用
    ...
    testDb.close();
    BetterDatabase = mod;
    exports.nativeAvailable = nativeAvailable = true;
}
catch (e) {
    exports.nativeAvailable = nativeAvailable = false;
    console.warn('[DatabaseShim] better-sqlite3 原生模块不可用，将使用内存降级模式:', e.message);
}
/**
 * 创建数据库实例。
...
function createDatabase(dbPath) {
    if (nativeAvailable) {
        try {
            ...
            catch { /* WAL not supported, ignore */ }
            try {
                db.pragma('synchronous = NORMAL');
            }
            catch { /* pragma not supported, ignore */ }
            try {
            ...
            console.warn('[DatabaseShim] 创建数据库失败，降级为内存模式:', e.message);
        }
    }
...
class MemoryDatabase {
    ...
    exec(sql) {
        ...
        for (const stmt of statements) {
            ...
            if (/CREATE\s+VIRTUAL\s+TABLE/i.test(stmt))
                continue;
            // CREATE TRIGGER — 忽略
            ...
            if (alterMatch) {
                ...
                if (table)
                    table.addColumn(alterMatch[2]);
                continue;
            }
            // INSERT INTO ... _fts — 忽略
            if (/INSERT\s+INTO\s+\w+_fts/i.test(stmt))
                continue;
...
class PreparedStatement {
    ...
    parse(sql) {
        ...
        const aggRe = /^SELECT\s+(COUNT\(\*\)|AVG\((\w+)\)|SUM\((\w+)\))\s+(?:as\s+)?(\w+)\s+FROM\s+(\w+)(?:\s+WHERE\s+(.+?))?\s*$/i;
    ...
    all(...args) {
        ...
        if (p.type === 'select') {
            ...
            // DISTINCT
            if (p.distinct) {
                rows = rows.filter((r, i, a) => a.findIndex((x) => x.every((v, j) => v === r[j])) === i);
            }
        ...
        if (p.type === 'aggregate') {
            ...
            if (p.aggFn === 'count')
                ...
                result = vals.length
                    ? vals.reduce((a, b) => Number(a) + (Number(b) || 0), 0) /
                        vals.length
                    : 0;
            else
                result = vals.reduce((a, b) => Number(a) + (Number(b) || 0), 0);
            const r = {};
            r[p.alias || 'result'] = result;
            return [r];
        }
        if (p.type === 'pragma_info')
            return [];
        return [];
    }
}
...
function parseSet(setClause, args) {
    const assignments = [];
...
function resolveValue(placeholder, args) {
    if (placeholder === '?')
        return args[0] ?? null;
...
function resolveArgs(args, valuePatterns, namedColumns, tableColumns) {
    const result = [];
    // 情况1: 命名参数 { id: '...', content: '...' }
    if (args.length === 1 &&
        typeof args[0] === 'object' &&
    ...
    if (namedColumns.length > 0) {
        for (const col of tableColumns) {
            ...
            result.push(idx >= 0 && idx < args.length
                ? args[idx]
                : null);
        }
        return result;
    ...
    for (let i = 0; i < valuePatterns.length; i++) {
        result.push(valuePatterns[i] === '?' && i < args.length
            ? args[i]
            : null);
    }
    return result;
}
function filterRows(table, whereClause, args) {
    if (!whereClause)
        return table.rows;
    ...
    for (const cond of conditions) {
        ...
        let targetVal = condMatch[3].trim();
        // 处理占位符
        if (targetVal === '?') {
...
function sortRows(rows, table, orderClause) {
...
function rowsToObjects(rows, table, selectExpr) {
