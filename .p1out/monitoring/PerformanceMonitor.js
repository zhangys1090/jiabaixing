...
exports.endSpan = exports.startSpan = exports.measureSync = exports.measure = exports.perf = exports.PerformanceMonitor = void 0;
exports.getTracer = getTracer;
exports.getMeter = getMeter;
exports.startOTelSpan = startOTelSpan;
exports.recordOTelRequest = recordOTelRequest;
...
class PerformanceMonitor extends events_1.EventEmitter {
    constructor(config) {
        ...
        this.MAX_SPAN_METRICS = 5000;
        this.MAX_ACTIVE_SPANS = 1000;
        this.maxMetricsPerCategory = 1000;
        this.tokenUsage = {
            totalInputTokens: 0,
            totalOutputTokens: 0,
            totalTokens: 0,
    ...
    startMonitoring(intervalMs = 60000) {
        if (this.monitoringInterval) {
            Logger_1.Logger.debug('性能监控已在运行，跳过重复启动', 'PerformanceMonitor');
            return;
        }
        Logger_1.Logger.info('📊 启动性能监控', 'PerformanceMonitor');
    ...
    stopMonitoring() {
        if (this.monitoringInterval) {
            ...
            this.monitoringInterval = null;
            Logger_1.Logger.info('📊 性能监控已停止', 'PerformanceMonitor');
    ...
    recordRequest(responseTime, success) {
        ...
        if (this.config.performanceTracking) {
            Logger_1.Logger.debug(`📈 请求记录: 响应时间=${responseTime}ms, 成功=${success}`, 'PerformanceMonitor');
        }
    ...
    collectMetrics() {
        ...
        if (currentMetrics.responseTime > 100 ||
            ...
            Logger_1.Logger.info(`性能告警: 响应=${currentMetrics.responseTime.toFixed(0)}ms, 内存=${currentMetrics.memoryUsage.toFixed(0)}MB, 错误率=${(currentMetrics.errorRate * 100).toFixed(1)}%`, 'PerformanceMonitor');
        }
    ...
    checkAlerts() {
        ...
        alerts.forEach((alert) => {
            Logger_1.Logger.warn(`⚠️ 性能告警: ${alert.message}`, 'PerformanceMonitor');
    ...
    cleanupOldMetrics() {
        ...
        if (beforeCount > afterCount) {
            Logger_1.Logger.debug(`🧹 清理旧指标: ${beforeCount - afterCount} 条记录已删除`, 'PerformanceMonitor');
        }
    }
    ...
    reset() {
        ...
        this.errorCount = 0;
        this.startTime = Date.now();
        Logger_1.Logger.info('📊 性能监控数据已重置', 'PerformanceMonitor');
        this.emit('metricsReset');
    }
    updateConfig(config) {
        this.config = { ...this.config, ...config };
        Logger_1.Logger.info('📊 性能监控配置已更新', 'PerformanceMonitor');
    }
    getConfig() {
        return { ...this.config };
    }
    ...
    // ═══════════════════════════════════════════════════════════
    startSpan(name, category = 'default', metadata) {
        ...
        if (this.activeSpans.size > this.MAX_ACTIVE_SPANS) {
            const oldestKey = this.activeSpans.keys().next().value;
            this.activeSpans.delete(oldestKey);
        }
        Logger_1.Logger.debug(`⏱️ 开始追踪: ${name} (${id})`, 'PerformanceMonitor');
        return id;
    }
    endSpan(id, success = true, metadata) {
        const span = this.activeSpans.get(id);
        if (!span) {
            Logger_1.Logger.warn(`⚠️ 未找到活跃的追踪span: ${id}`, 'PerformanceMonitor');
            return null;
        }
        ...
        Logger_1.Logger.debug(`⏱️ 完成追踪: ${span.name} - ${duration}ms ${success ? '✅' : '❌'}`, 'PerformanceMonitor');
        return completedSpan;
    }
    recordSpanMetric(metric) {
        const category = metric.category || 'default';
        if (!this.spanMetrics.has(category)) {
            if (this.spanMetrics.size >= this.MAX_SPAN_METRICS) {
                const oldestKey = this.spanMetrics.keys().next().value;
                this.spanMetrics.delete(oldestKey);
            }
    ...
    async measure(name, fn, category = 'default', metadata) {
        ...
        try {
            ...
            this.endSpan(id, false, { error: error.message });
            throw error;
        }
    }
    ...
    getReport() {
        ...
        for (const name of allMetricNames) {
            ...
            if (nameMetrics.length > 0) {
                const avgDuration = nameMetrics.reduce((sum, m) => sum + m.duration, 0) /
                    nameMetrics.length;
        ...
        for (const name of allMetricNames) {
            ...
            const errorRate = nameMetrics.length > 0 ? failures / nameMetrics.length : 0;
            if (failures > 0) {
    ...
    printReport() {
        ...
        Logger_1.Logger.info('='.repeat(60), 'PerformanceMonitor');
        Logger_1.Logger.info('📊 性能监控报告', 'PerformanceMonitor');
        Logger_1.Logger.info(`🕐 生成时间: ${new Date(report.timestamp).toISOString()}`, 'PerformanceMonitor');
        Logger_1.Logger.info(`📈 总指标数: ${report.totalMetrics}`, 'PerformanceMonitor');
        Logger_1.Logger.info('', 'PerformanceMonitor');
        for (const [category, stats] of Object.entries(report.categories)) {
            Logger_1.Logger.info(`📁 ${category}:`, 'PerformanceMonitor');
            Logger_1.Logger.info(`   调用次数: ${stats.totalCalls}`, 'PerformanceMonitor');
            Logger_1.Logger.info(`   成功率: ${((stats.successCount / stats.totalCalls) * 100).toFixed(1)}%`, 'PerformanceMonitor');
            Logger_1.Logger.info(`   平均耗时: ${stats.avgDuration.toFixed(2)}ms`, 'PerformanceMonitor');
            Logger_1.Logger.info(`   P50/P95/P99: ${stats.p50Duration.toFixed(2)}/${stats.p95Duration.toFixed(2)}/${stats.p99Duration.toFixed(2)}ms`, 'PerformanceMonitor');
        }
        if (report.topSlowest.length > 0) {
            Logger_1.Logger.info('', 'PerformanceMonitor');
            Logger_1.Logger.info('🐢 最慢操作 TOP 10:', 'PerformanceMonitor');
            for (let i = 0; i < Math.min(10, report.topSlowest.length); i++) {
                const item = report.topSlowest[i];
                Logger_1.Logger.info(`   ${i + 1}. ${item.name}: ${item.avgDuration.toFixed(2)}ms (${item.calls} calls)`, 'PerformanceMonitor');
            }
        }
        if (report.topErrors.length > 0) {
            Logger_1.Logger.info('', 'PerformanceMonitor');
            Logger_1.Logger.info('⚠️ 错误率最高的操作:', 'PerformanceMonitor');
            for (let i = 0; i < Math.min(10, report.topErrors.length); i++) {
                const item = report.topErrors[i];
                Logger_1.Logger.info(`   ${i + 1}. ${item.name}: ${(item.errorRate * 100).toFixed(1)}% (${item.failures} failures)`, 'PerformanceMonitor');
            }
        }
        Logger_1.Logger.info('', 'PerformanceMonitor');
        Logger_1.Logger.info('💻 系统负载:', 'PerformanceMonitor');
        Logger_1.Logger.info(`   RSS: ${(report.systemLoad.memoryUsage.rss / 1024 / 1024).toFixed(2)} MB`, 'PerformanceMonitor');
        Logger_1.Logger.info(`   Heap Total: ${(report.systemLoad.memoryUsage.heapTotal / 1024 / 1024).toFixed(2)} MB`, 'PerformanceMonitor');
        Logger_1.Logger.info(`   Heap Used: ${(report.systemLoad.memoryUsage.heapUsed / 1024 / 1024).toFixed(2)} MB`, 'PerformanceMonitor');
        Logger_1.Logger.info(`   Uptime: ${report.systemLoad.uptime.toFixed(1)}s`, 'PerformanceMonitor');
        Logger_1.Logger.info('='.repeat(60), 'PerformanceMonitor');
    }
    clear() {
        this.spanMetrics.clear();
        this.activeSpans.clear();
        Logger_1.Logger.info('🧹 性能监控数据已清空', 'PerformanceMonitor');
    }
    getMetricStats(name) {
    ...
    getTokenUsage() {
        ...
        const avgPerSession = this.tokenUsage.sessionCount > 0
            ? this.tokenUsage.totalTokens / this.tokenUsage.sessionCount
            : 0;
        const estimatedCost = this.tokenUsage.totalInputTokens * 0.00000014 +
...
exports.perf = PerformanceMonitor.getInstance();
exports.measure = exports.perf.measure.bind(exports.perf);
exports.measureSync = exports.perf.measureSync.bind(exports.perf);
exports.startSpan = exports.perf.startSpan.bind(exports.perf);
exports.endSpan = exports.perf.endSpan.bind(exports.perf);
let tracer = null;
let meter = null;
const requestCounter = { value: 0 };
const errorCounter = { value: 0 };
const latencyHistogram = new Map();
const MAX_LATENCY_KEYS = 5000;
/**
 * 获取 OTel Tracer
 */
function getTracer() {
 ...
 */
function startOTelSpan(name, options) {
    if (!tracer)
        return undefined;
 ...
 * 记录请求指标到 Prometheus
 */
function recordOTelRequest(operation, durationMs, success) {
    requestCounter.value++;
    if (!success)
        errorCounter.value++;
    const key = operation;
    if (!latencyHistogram.has(key)) {
        if (latencyHistogram.size >= MAX_LATENCY_KEYS) {
            const oldestKey = latencyHistogram.keys().next().value;
            latencyHistogram.delete(oldestKey);
        }
        latencyHistogram.set(key, []);
    }
    ...
    if (meter) {
        try {
            ...
            Logger_1.Logger.debug(`OTel指标记录失败（非关键）: ${err?.message}`, 'PerformanceMonitor');
        }
    }
}
/**
...
async function shutdownOTel() {
    tracer = null;
    meter = null;
    Logger_1.Logger.info('📡 OpenTelemetry 透传句柄已清理（SDK 由 Python 后端管理）', 'OTel');
}
