...
exports.DialogueGenerator = void 0;
...
class DialogueGenerator {
     ...
     * @returns 生成的回复
     */
    async generate(input, sceneTag, memoryContext, userProfileSummary) {
        try {
            const systemPrompt = this.buildSystemPrompt(sceneTag);
            const userPrompt = this.buildUserPrompt(input, memoryContext, userProfileSummary, sceneTag);
            Logger_1.Logger.info(`📝 生成对话: 场景=${sceneTag}, 记忆数=${memoryContext.length}`, 'DialogueGenerator');
            const response = await this.llm.chat(userPrompt, [], systemPrompt);
            Logger_1.Logger.info(`✅ 对话生成完成 (${response.length} 字符)`, 'DialogueGenerator');
            return response;
        }
        catch (error) {
            Logger_1.Logger.error('❌ 对话生成失败', error, 'DialogueGenerator');
     ...
     * 包含人格摘要 + 场景语气指令 + 边界提醒
     */
    buildSystemPrompt(sceneTag) {
        const personaSummary = this.personaCore.buildPersonaSummary();
        const sceneInstruction = this.personaCore.buildSceneToneInstruction(sceneTag);
...
- 控制回复长度，日常对话不超过50字，技术解释不超过200字`;
     ...
     * - 动态调整记忆数量
     */
    buildUserPrompt(input, memoryContext, userProfileSummary, sceneTag = 'daily') {
        const parts = [];
        ...
        if (memoryContext.length > 0) {
            ...
            if (sortedMemories.length > 0) {
                ...
                sortedMemories.forEach((memory, index) => {
                    const timeHint = config.enableTimeHint && memory.timestamp
                    ...
                    const relevanceLabel = config.enableRelevanceLabel
                        ...
                        : '';
                    parts.push(`${index + 1}. [${memory.type}]${relevanceLabel} ${memory.content} ${timeHint}`);
                });
            }
        }
        // 用户画像摘要
        if (userProfileSummary) {
            ...
            if (userProfileSummary.preferredFrameworks &&
                userProfileSummary.preferredFrameworks.length > 0) {
                profileParts.push(`常用框架：${userProfileSummary.preferredFrameworks.join('、')}`);
            }
            if (userProfileSummary.recentTopics &&
                userProfileSummary.recentTopics.length > 0) {
                profileParts.push(`最近关注：${userProfileSummary.recentTopics.join('、')}`);
            }
            if (userProfileSummary.behaviorHints &&
                userProfileSummary.behaviorHints.length > 0) {
                profileParts.push(`行为模式：${userProfileSummary.behaviorHints.join('；')}`);
            }
     ...
     * 排序和筛选记忆
     */
    sortAndFilterMemories(memories, config) {
        return memories
     ...
     */
    async generateProactiveMessage(reason, sceneTag, context, memoryContext) {
        const systemPrompt = this.buildSystemPrompt(sceneTag);
        const reasonGuidance = {
            long_silence: '用户已经很久没和你说话了。像一位细心的秘书——不要问"你在干嘛"，而是自然地提到你可能注意到的事。',
            negative_emotion_trend: '你注意到用户最近情绪不太好。不需要直接说"你不开心吧"，而是温和地表达关切。',
            morning_greeting: '现在是早晨。一句从容的早安，可以自然地带出今天的重点。',
...
请生成一条简短、自然的主动消息。不超过40字。`;
        try {
            ...
            Logger_1.Logger.error('❌ 主动消息生成失败', error, 'DialogueGenerator');
     ...
     * 降级回复（LLM 不可用时）
     */
    generateFallbackReply(input, sceneTag) {
        const trimmed = input.trim().toLowerCase();
        // 文件读取: "读一下CLAUDE.MD"、"查看日志文件"、"打开README"
        const fileReadMatch = trimmed.match(/^(?:读|读取|查看|打开|展示|显示)\s*(?:一下|取)?\s*([^\s，。]{1,80})/i);
        if (fileReadMatch) {
            ...
            if (fs.existsSync(fullPath)) {
                try {
                    ...
                    Logger_1.Logger.debug(`读取文件失败: ${filePath}: ${err?.message}`, 'DialogueGenerator');
            ...
            if (fs.existsSync(altPath)) {
                try {
                    ...
                    Logger_1.Logger.debug(`读取备用路径文件失败: ${altPath}: ${err?.message}`, 'DialogueGenerator');
                    return `读取 ${filePath} 时出错。`;
                }
            }
     ...
     * 主动消息降级
     */
    generateProactiveFallback(reason) {
        const fallbacks = {
