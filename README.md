...
| **Web 界面 WS 通道未接入 Python 主循环**                                   | 浏览器 UI 发任务**不达 Python 3112**（3112 日志零记录）；UI 走 TS 本地兜底：响应为模板话术、状态栏恒显“0 工具”。已用 UI 发“你好”+ WS 直连双向实证 | 🔴 已定位（`core.processInput` 的 bridge 路由在 WS 入口未生效；`/api/chat`、`/api/process` 直连桥正常），待修复。临时通道：真实任务走网关 HTTP / CLI |
