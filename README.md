...
- **2026-09-16 真实运行修复**（非测试数字，均以真实任务复现 + 修复 + 复验）
- **2026-09-17 生产主循环认知机制接入**（真实任务验证，非测试数字）：
  - **ML4** StateAuthority 注册真实 providers（此前 `registerProviders` 零调用、快照空壳）——现在 world=最近工具结果、context=工作目录、capabilities=工具清单进快照；
  - **ML5** progress 验证驱动 + 按 decisionId 去重（run 与 run_stream 双路径）——并行工具不再虚增进度；
  - **ML6** replan 接入生产循环：`GoalAuthority.replan` 补 `expectedVersion` 参数（修复 TypeError 死代码——此前 run/run_stream 的 replan 全部静默失败）。实测：T12b 构造 3 次连续失败 → 信念下调 ×3 → `replan triggered (consecutive_failures=3)`；
  - **ML8** `LLMProposer.propose` 消费 `snapshot.memory.relevantMemories`（Memory→State→Decision 断链修复）——探针验证 reasoning 携带记忆、无记忆时不伪造。
  - 验证：41 个 authority 单测全过；真实任务 T13（统计 tools 目录 59 个 .py、最大 `code_tools.py`，与磁盘核对一致）与 T12b（失败→belief 下调→replan）均经网关 3111 → WS → Python 3112 实测。
    ：WS 流式收尾 `result` 未绑定、权限校验 `'str' object has no attribute 'value'`、记忆工具 `Logger._log() got an unexpected keyword argument 'error'`、`/health` uptime 负值、`file_search` NameError、文件工具双 python 路径、执行类工具限流；并清理记忆库 73 条测试残留 + daily 数据 1871 条测试残留（任务/会议/提醒/笔记，仅保留 1 条真实任务）。修复后真实任务验证：T1 聊天 / T2 文件 / T3 记忆 / T4 代码分析 / T6 任务 / T7 日程 / T8 多步统计 / T10 验证 / T11 浏览器，均经网关 3111 → WS → Python 3112 真实打通（结果可核验：如 `python/agent/tools` 共 59 个 .py，最大 `code_tools.py` 57,210 字节；example.com 标题 `Example Domain`）。
