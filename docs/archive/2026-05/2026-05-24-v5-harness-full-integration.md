...
describe('PersistenceService 依赖注入', () => {
  it('应正确接收 persistenceDeps', async () => {
    ...
    expect(mockMemoryEngine.storeShortTermMemory).toHaveBeenCalledWith(
      'test content',
      undefined,
      undefined
    );
...
if (this.deps.constraintsService) {
  await this.deps.constraintsService.executeHooks('ON_BUDGET_EXCEEDED', {
...
describe('LoopController + ConstraintsService 集成', () => {
  it('应在预算超限时触发 ON_BUDGET_EXCEEDED 钩子', async () => {
    ...
    const mockConstraints = {
      ...
      checkPermission: jest
        .fn()
        .mockReturnValue({ allowed: true, missing: [] }),
    ...
    const result = await controller.run({ text: '测试', traceId: 'test-1' }, [
  ...
  it('应在计划创建后触发 ON_PLAN_CREATED 钩子', async () => {
    ...
    const mockConstraints = {
      checkBudget: jest
        .fn()
        .mockReturnValue({ withinBudget: true, warnings: [], remaining: {} }),
      checkPermission: jest
        .fn()
        .mockReturnValue({ allowed: true, missing: [] }),
      checkSafetyBoundary: jest.fn().mockReturnValue({ allowed: true }),
    ...
    await controller.run({ text: '测试计划钩子', traceId: 'test-2' }, [
      { role: 'system', content: '你是一个助手' },
    ]);
...
if (this.deps.verificationService && executorOutput.toolCallsCount > 0) {
  const verification = await this.deps.verificationService.verifyToolResult(
    'harness_executor',
    {
      messages: executorOutput.messages,
  ...
  if (
    ...
    Logger.warn(`⚠️ 安全风险等级: ${verification.riskLevel}`, 'LoopController');
  }
}
```
...
if (this.deps.persistenceService) {
  ...
  this.deps.persistenceService.saveConversationMessage(
    'assistant',
    report.response
  );
...
describe('LoopController + PersistenceService 集成', () => {
  it('应在循环开始时保存任务状态，完成时更新', async () => {
    ...
    const mockPersistence = {
      saveTaskState: jest
        .fn()
        .mockImplementation(async (task: Record<string, unknown>) => {
      ...
      saveConversationMessage: jest
        .fn()
        .mockImplementation((role: string, content: string) => {
          messages.push({ role, content });
        }),
      recordEvolutionMetric: jest
        .fn()
        .mockImplementation((metric: Record<string, unknown>) => {
          metrics.push(metric);
        }),
    };
  ...
  it('应在收到消息时调用 core.processInput', async () => {
    ...
    const adapter = (
      manager as unknown as {
        adapters: Map<
          string,
          { onMessageCallbacks: Array<(msg: unknown) => Promise<void>> }
        >;
      }
    ).adapters;
...
if (this.constraintsService) {
  const hookResult = await this.constraintsService.executeHooks('BEFORE_LOOP', {
    input: input.text,
    userId: input.userId,
  });
  if (!hookResult.proceed) {
...
if (this.constraintsService) {
  await this.constraintsService.executeHooks('AFTER_RESPONSE', {
  ...
  it('应在 processInput 前后触发 BEFORE_LOOP 和 AFTER_RESPONSE', async () => {
    ...
    harness.setDeps({
      ...
      constitutionalBuilder: {
        buildConstitutionPrompt: jest.fn().mockResolvedValue('system prompt'),
      },
      memoryInjector: { autoRetrieveMemories: jest.fn().mockResolvedValue([]) },
      dynamicContext: { getDynamicContext: jest.fn().mockReturnValue('') },
      historyProvider: { getRecentHistory: jest.fn().mockReturnValue([]) },
    ...
    (harness as unknown as { constraintsService: unknown }).constraintsService =
      mockConstraints;
    ...
    if (lc) {
      (
        lc as unknown as { deps: Record<string, unknown> }
      ).deps.constraintsService = mockConstraints;
    }
...
describe('Harness 全链路集成测试', () => {
  it('应完成完整的 E-T-C-S-L-V 管控流程', async () => {
    ...
    harness.setDeps({
      ...
      constitutionalBuilder: {
        buildConstitutionPrompt: jest
          .fn()
          .mockResolvedValue('你是家百星，一个温暖的御姐秘书。'),
      },
      memoryInjector: {
        autoRetrieveMemories: jest
          .fn()
          .mockResolvedValue(['用户喜欢简洁的回答']),
      },
      dynamicContext: {
        getDynamicContext: jest
          .fn()
          .mockReturnValue('当前时间: 2026-05-24 14:00，时段: 下午'),
      },
      historyProvider: {
